...
{{% alert type="info" %}}
The preferred Studio Pro version 8 release for apps in production is [8.18](8.18).
{{% /alert %}}
...
* We applied an important security patch that fixed a vulnerability for authenticated users. We strongly advise upgrading your Mendix applications to this version (or to 8.12.5 or 8.17.0, where the issue has also been fixed).
...
* <a name="module"></a>We fixed an issue that caused a crash when committing to an app project after renaming or deleting a module.
...
* We simplified the ability to switch between different languages in your app project. A menu that lists all the languages available is now available on the right of the status bar (at the very bottom of Studio Pro), which allows you to easily change the active language. Also, new keyboard shortcuts have been introduced. For more information, see [Language Menu](/refguide8/menus#language).
