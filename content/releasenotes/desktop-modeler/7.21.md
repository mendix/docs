...
* There is a problem where it is not possible to commit after merging a branch. (Tickets 78468, 78322, 79166, 78542, 76785, 77955, 77245)
	* Workaround – reference the [Getting an Error with the Message SharpSvn.SvnRepositoryIOException: At least one property change failed; repository is unchanged](https://docs.mendix.com/howto/collaboration-requirements-management/troubleshoot-version-control-issues#error-with-message) section of *How to Troubleshoot Version Control Issues* for details on how to fix this.
