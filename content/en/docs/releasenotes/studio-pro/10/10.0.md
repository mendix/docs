...
* To make it possible for a multi-instance user task to have multiple assignees, we renamed the `WorkflowUserTask_Assignee` association to `WorkflowUserTask_Assignees`, and this is a reference set from now on.
...
* We updated the `StrictChangeBehaviour` setting name to `ObjectManagement.StrictChangeBehavior`. This setting will be removed in Mendix 11. The behavior will not be configurable henceforth. [**REVIEW FOR COMBINATION/DELETION**]
