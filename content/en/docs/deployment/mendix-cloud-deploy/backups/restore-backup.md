...
The dump must be created with `pg_dump` version 1.16 or below, which is currently bundled with PostgreSQL 17 and below. If it is created with a later version, then the upload will fail.
