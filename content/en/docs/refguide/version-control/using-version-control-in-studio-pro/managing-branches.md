...
4. Test that things work as intended.
5. Create a new deployment archive with a higher version number (increase patch or minor version).

{{% alert color="info" %}}
Mendix recommends merging the fixed maintenance branch into the main line quickly, if required. If the merge is too complicated to be made automatically because the main line has changed too much, you will know how to apply the fix manually to the main line, as the changes will still be fresh in your mind.

Not all maintenance fixes need to be merged to the main line. Sometimes, it is a fix of something that was completely redesigned or eliminated in the main line. In this case, merging is unnecessary.
{{% /alert %}}
...
Select **Version Control > Manage Branch Lines...** and create a branch from a revision of the main line.
...
Use the second method if the first method is not possible and you want to overwrite the main line with your branch line.
{{% alert color="warning" %}}
This method permanently replaces all files in your main line with those from the branch line. Ensure you have committed or backed up any important changes in your main line before proceeding.
{{% /alert %}}

Follow these steps:

1. Check out both the main line and the branch line locally.
2. Close Studio Pro if it is open.
3. Delete all the files in the main line app directory (except for the *.git* directory).
4. Copy all the files from the branch line directory into the main line app directory (except for the *.git* directory).
5. Reopen the main line app in Studio Pro.
6. Commit your changes using Studio Pro.
...
In a future release, Mendix plans to introduce a separate command as part of the [mx Command-Line Tool](/refguide/mx-command-line-tool/) to allow merging on the command line.
...
Mendix recommends starting trunk-based and adopting trunk-based with feature branches next, if needed. Getting many merge conflicts or releasing a first version to production are sensible triggers to move away from solely trunk-based development.
...
In the [tips and tricks](#branching-tricks) section, you will find suggestions on how to work with and manage branches effectively.
...
* **Difficulty mitigating issues** – When encountering issues in production, it is not possible to deploy a hotfix without also publishing other changes to your app unless you create a branch.
...
* Periodically merge higher-level branches, such as development or main, to lower-level branches, such as feature branches. This ensures you take the most recent stable work into account when developing a feature, preventing larger merge conflicts down the road.
...
* Ensure old branches are cleaned up to prevent accumulating them over time. Ideally, delete a branch as part of the process of completing a feature. In cases where branches are not merged in the end, consider cleaning them up periodically.
