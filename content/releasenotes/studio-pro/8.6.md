...
* **KI: 93385: Changes in a change object activity...**
* **KI: 2212: The Core.execute(IContext context...**
* **KI: 93869: Using a reference over associations in offline apps... & Retrieving objects through...**
* **KI: 1929: On MySQL 8.0.19...** 
* **KI: 1254: There is an issue with deployment...**
* 
## 8.6.0 {#860}
...
* It is now possible to select a default color for each microflow activity type separately. This setting is stored in your app project, which means that all the App Team members working on the app will see the same colors. The [Project Settings](/refguide/project-settings) dialog box presents an overview that allows you to change the default color for all activity types. Also, it is now possible to set a current activity's color as default by right-clicking it in the microflow editor and selecting the new option **Set as default color** in the context menu. Finally, when changing a default color, it is possible to reset any custom colors that are assigned to activities in your project so they will use the default color as well.
* In native mobile apps, you can now use classes on pages to add custom styling for the header and status bar.

#### New Navigation Patterns for Native Mobile Apps

Native layouts have a new **Layout type** property, which is similar to the one for web layouts. This property enables the following new navigation patterns for your native app:

* Top-level layouts – pages with a top-level layout never show a back button, so when such pages are opened, the other open pages are closed; using top-level layouts for home pages and bottom bar items is recommended
* Pop-up layouts – a page with a pop-up layout slides in from the bottom and has a close icon instead of a back icon; it is possible to open other pop-up pages inside the pop-up window; when opening a normal page from the pop-up window, the pop-up window will be closed first
* Sidebar – on top-level layouts, you can enable the **Sidebar** property; the sidebar can be opened via the icon that will appear in the header of the page or by swiping from the left side of the screen; when the end-user navigates to a new page, the sidebar will automatically be closed; for more custom control you can also use the new **Toggle sidebar** nanoflow action, which is part of the [Nanoflow Commons](https://appstore.home.mendix.com/link/app/109515/) module

### Improvements

* We optimized the waiting time that occurs after you commit to a Studio-enabled branch.
* We rebuilt the [Connector](/refguide/view-menu#connector) to give it the same look and feel as the new [Project Explorer](//refguide/project-explorer). Highlighting via the **Filter** now works in the Connector. Should you find any issues, you can switch back to the old version of the Connector via **Edit** > **Preferences** > [New Features](/refguide/preferences-dialog#new-connector).
* We simplified the ability to switch between different languages in your app project. A menu that lists all the languages available in your app project is now available on the right of the status bar, which allows you to easily change the active language. Also, new keyboard shortcuts have been introduced. For more information, see [Language Menu](/refguide/menus#language).
* We improved the logging of published OData services. They no longer log on the error level when a client performs an invalid request – this is not an error, it is the correct and valid behavior of the service. That logging is now done on the debug level.
...
* When removing a reference in a Reference set selector, the confirmation will now show you a removing message instead of a deleting message. The message can be customised in the project system text. (Ticket #82316)
...
* We improved accesibility for keyboard navigation and screen readers. You can now set a **Tab index** and **Alternative text** for all images. (Ticket 70196)
	{{% alert type="info" %}}This is based on an [upvoted idea from Patrick Hurley](https://forum.mendixcloud.com/link/ideas/1587) submitted to the [Mendix Idea Forum](https://forum.mendixcloud.com/link/ideas). Thank you, Patrick!
	{{% /alert %}}
...
* <a name="93385"></a>We fixed an issue that was introduced in version 8.5 where the changes in a microflow [change object](/refguide/change-object) activity were not executed sequentially. (Ticket 93385)
...
* <a name="2212"></a>We fixed an issue in the `Core.execute(IContext context, String name, Object... params)` method which threw a `NullPointerException` when a list containing `null` values was passed as a parameter.
...
* <a name="1254"></a>We fixed an issue where apps with **Atlas UI Resources** version 2.4.0 using the new flex layout grid features caused the deployment to fail.
...
* We fixed a bug for the date picker running in Internet Explorer 11. Now, a date can selected again in the calendar. (Ticket 91976)
...
* Double-clicking a row in the left-hand grid of the zoomed-in level of the [Changes](/refguide/changes-pane#zoomed-in-level) pane now has the same effect as clicking **Go to**, which includes opening the relevant document.
...
It is no longer possible to upload microflows to and download microflows from Model Share, because this service is now deprecated and will be discontinued on March 31st, 2020. Note that models uploaded to Model Share before March 31st will still be available for download in the MPK format.

Because many users have indicated that copy/pasting microflows between app projects was their main reason for using Model Share, cross-project copy/paste will be added to Studio Pro very soon!
