...
6.  After focusing the `input` element, enter the text. When entering text in an `input` element, use the [*Send Keys*](/addons/ats/refguide/rg1/send-keys) action. Add the action, connect the input element from step 1, and give it a proper description:

    ![](attachments/ht2-create-unsupported-widget/ht2-cab-03-textbox/text-box-sendkeys-add.png)

7.  The last action you add is [*Mendix Wait*](/addons/ats/refguide/rg1/mendix-wait). You trigger a possible event in the widget by entering text, so you need to ensure that ATS waits for all the background processes to finish:
