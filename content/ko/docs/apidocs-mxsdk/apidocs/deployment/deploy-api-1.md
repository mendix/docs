...
description: "Deploy API v1은 라이선스 노드에 Mendix 앱을 배포하고, Mendix Cloud에서 애플리케이션 환경을 관리하며, 상태를 조회하고, 애플리케이션을 시작 및 중지하고, 애플리케이션 환경에 새 모델 버전을 배포하거나 전송합니다."
restapi: true
weight: 40
...
이 이미지는 아래에서 논의되는 개념과 그 관계를 도메인 모델로 표현합니다:
...
API는 자동화된 시스템을 위해 설계되었으므로, the Deploy API does not require two-factor authentication, which is normally required to make changes to production environments. This is a potential security risk. Therefore, API access must be explicitly allowed for team members who want to use the Deploy API.
...
모든 앱을 조회합니다 to which the authenticated user has access as a regular user that have environments created on Mendix Cloud. This includes all licensed apps and any Free Apps that have been deployed.
...
요청된 프로젝트 ID에 대한 Free App을 생성합니다.
...
특정 앱을 조회합니다 to which the authenticated user has access as a regular user. This app can be found via the "Nodes overview" screen in the Mendix Platform.
...
### 환경 조회{#list-environments}

#### 설명

모든 환경을 조회합니다 that are connected to a specific app to which the authenticated user has access as a regular user. These environments can be found via the "Nodes overview" screen in the Mendix Platform.
...
### 환경 조회 {#retrieve-environment}

#### 설명

특정 환경을 조회합니다 that is connected to a specific app to which the authenticated user has access as a regular user. These environments can be found via the "Nodes overview" screen in the Mendix Platform.
...
특정 환경을 시작합니다 that is connected to a specific app to which the authenticated user has access as a regular user. These environments can be found via the "Nodes overview" screen in the Mendix Platform.
...
### 환경 시작 상태 조회 {#get-start-environment-status}
...
특정 환경을 중지합니다 that is connected to a specific app to which the authenticated user has access as a regular user. These environments can be found via the "Nodes overview" screen in the Mendix Platform.
...
### 환경 패키지 조회 {#retrieve-environment-package}

#### 설명

배포된 패키지를 조회합니다 of a specific environment that is connected to a specific app to which the authenticated user has access as a regular user. These environments can be found via the "Nodes overview" screen in the Mendix Platform.
...
### 환경으로 배포 패키지 전송 {#transport-deployment-package}

#### 설명

특정 배포 패키지를 특정 환경으로 전송합니다. After the deployment package has been transported, it will not replace a currently running app automatically. You will need to [stop](#stop-environment) and [start](#start-environment) the environment to activate the new package.
...
특정 환경에서 모든 데이터를 제거합니다: including files and database records. This action requires the environment to be in the status *NotRunning*.
...
현재 값을 가져옵니다 of custom runtime settings, constants, and scheduled events used by the target environment.
...
값을 변경합니다 of existing environment settings, such as custom runtime settings, constants, and scheduled events. These changes are applied after restarting the environment.
...
환경의 메모리와 인스턴스를 스케일링합니다. Only those environments that run a package that uses a supported version of the Mendix Runtime can spread the total memory over multiple instances.
...
태그는 임의의 문자열입니다 that are not interpreted by the Mendix Portal. Users are able to set tags on environments. Tags serve two purposes:
