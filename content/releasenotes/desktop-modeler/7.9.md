...
* After their introduction in the Web Modeler, we have now added building blocks to the Desktop Modeler! Building blocks offer a convenient way to add a pre-configured widget to the page as easily as you would add a normal widget. This speeds up page creation, as it enables the re-use of recurring UI patterns.
...
* We deprecated the theme ZIP file setting, because UI resources packages are the future of theming. To get rid of the deprecation, you can extract the ZIP file to your theme folder and set the setting to `None`.
...
We added support for [mathematical functions](/refguide/mathematical-function-calls) as well as `parseInteger`, `parseDecimal`, and `formatDecimal` to the client expressions used in conditional formatting and validations.

### Other Improvements

* We upgraded our internal web server Jetty to version 9.4.7. When retrieving [server statistics](/refguide/monitoring-mendix-runtime#5-server-statistics), the `max_idle_time_s_low_resources` information is not returned anymore, because Jetty does not provide this information anymore. Also, the shutdown procedure of Mendix apps has been improved in the process of upgrading.
...
* We fixed the issue where import mappings using `find by key` to find existing objects tried to also update the key attribute(s) of the found object.
* It is now allowed to specify headers with the same name more than once for a Call REST action.
