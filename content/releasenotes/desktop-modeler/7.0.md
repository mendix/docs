...
When an object is changed, but there are no actual changes the system attributes (`changedDate` and `changedBy`) would still be updated. This does not make sense when there are no changes, so now the system attributes are only updated when at least one attribute or association is changed.
