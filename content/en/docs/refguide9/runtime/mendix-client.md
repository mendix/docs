...
| **clear_cache** | Runtime | Used to instructs the client to clear the cached session data whenever a new user login to an offline application—*introduced for apps created in Mendix version 9.24.24 and above* | `/` | until client starts or user closes their browser | `false` | 
