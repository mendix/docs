...
For professional operations solutions, there is often an agent per node that is shipping data in near-real time towards an application performance monitoring ([APM](/addons/apm-addon/index)) system (such as Data Dog) that is used for root-cause analysis, trend analysis, sizing metrics, and alarms.
