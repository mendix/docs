 ...
 * In the microflow editor, you can now use **Loop** activities. This is a powerful feature that allows you to add all sorts of logic, e.g. logic based on items in a list. For example, if you have a list of order lines and you want to set the delivery date for every order line, you can use a **Loop** activity, containing a **Change Object** activity that sets the delivery date for each order line.

#### Improvements

 * We improved the expression editor that is part of the microflow editor. When writing expressions, e.g. conditions for an **Exclusive Split** activity or attribute values for a **Change Object** activity, the suggestions that you will see in the expression editor are now categorized. This improves readability and helps you write the right expression more easily.
