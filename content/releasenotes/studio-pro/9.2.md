...
* We implemented the functionality to revert committed changes for [Git projects](/refguide/version-control). [CORRECT CROSS-REFERENCE?]
* We added a feature to the [Call REST service activity](/refguide/call-rest-action). When modeling a form data request, you can now specify HTTP headers for the parts.
...
* We added quick navigation from a [menu widget](/refguide/menu-widgets) to a navigation document.
	{{% alert type="info" %}}
	This is based on [an idea from Fabian Recktenwald](https://forum.mendixcloud.com/link/ideas/678) submitted to [the Mendix Idea Forum](https://forum.mendixcloud.com/link/ideas). Thanks, Fabian!
	{{% /alert %}}
...
* We improved the MxAssist Performance Bot recommendation MXP007 by limiting it only to attribute types that may benefit from adding an index.
* We implemented performance optimizations of the [MxAssist Logic Bot](/refguide/mx-assist-logic-bot) when used with large apps.
...
* When a pluggable widget explicitly requests no items from [ListValue](/apidocs-mxsdk/apidocs/client-apis-for-pluggable-widgets#listvalue) are passed to it by calling `setLimit(0)`, we now avoid triggering a network request for the underlying [datasource](/apidocs-mxsdk/apidocs/property-types-pluggable-widgets#datasource). Using this mechanism, pluggable widgets can build lazy loading features that do not fetch data until some condition is met.
* You can now use ad database source for the [selectable objects](https://docs.mendix.com/refguide/input-reference-set-selector#selectable-objects) of an input reference set selector.
...
* We enabled the `owner`, `changedDate`, `changedBy`, `createdDate` attributes in the `WorkflowContext` entity.
* We now prevent starting a workflow using [Call workflow](/refguide/on-click-event#call-workflow) if its workflow context entity is not committed.
...
* <a name="1655"></a>We fixed the issue with [Full Screen](/refguide/view-menu#full-screen) mode not being available.
...
* Fixed an issue where running an offline-first app in a cluster produced warnings when trying to remove a non-existent session on another node. (Ticket 108018)
...
* We fixed the issue that occurred when a conditional editability expression referenced a non-existent attribute when the widget was non-editable due to its parent container (for example, list view/data view). (Ticket 120665)
...
* We fixed an issue where the <kbd>Ctrl</kbd> + <kbd>Enter</kbd> key combination did not close a dialog box in certain cases. (Tickets 116222, 120182)
...
* The [file manager](/refguide/file-manager) widget when configured to with **Type** > **Upload** no longer causes an unexpected error when there are validation errors in certain scenarios. (Ticket 120116)
...
* We fixed an issue where you saw an error message when pressing <kbdShift</kbd> + <kbdEnter</kbd> at the sign-in screen. This key combination made the browser control attempt to open a new browser window, and that failed. This key combination no longer leads to an error. (Ticket 119314)
...
* We fixed the MXP006 recommendation from the MxAsist Performance Bot so that converting microflows to nanoflows should NOT allow a call from a grid action button.
...
* We fixed an issue where resolving a conflict using **mine** did not hide the (orange) conflict status bar in editor tabs and the editor was not reopened when the conflict resolution was active and resolving the whole document using mine was used.
* We fixed an issue that occurred while changing the attribute type between string and number in the [text box](/refguide/text-box) widget, which showed the validations from the previous selection in Studio Pro.
* In some situations, the **Update** action in the [Update Widgets](/refguide/project-menu#update-widgets) tool did not update the widget, although it was showing that it had the latest version in the overview. This has been fixed.
