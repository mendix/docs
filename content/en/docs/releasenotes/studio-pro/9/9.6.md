...
**Release date: April __th, 2022**

{{% modelerdownloadlink "9.6.11" %}}

### Improvements

To make management of file storage more flexible we extended the performDeleteFromStorage runtime setting. This change also allows getting rid of unnecessary log messages about unreferenced files delete failures. This is a backport from Mendix 9.12.0 (Ticket 139266).

### Fixes

We fixed an issue when offline PWA users lose their valid sessions upon a new deployment (Ticket #144836).
We fixed an issue where the import mapping would select an incorrect version of the Xerces library that would result in a Java error. (Ticket #141679)
We fixed an issue when navigation can be broken if a user try to navigate during a page transition (Ticket #105484)
We fixed an issue with Native List Views and pagination. Native List Views do not become scrollable if the initial number of items is not overflowing. This behaviour renders "scroll to load more" unusable in such cases. Now, Native List Views will attempt to load as many items as they can to fill their content. The pagination limits will be respected for the any "scroll to load more" event thereafter.
We fixed a NullPointerException during startup that could occur after renaming a module with a remote entity that was created before Mendix 9.6.3 (Ticket #142150).
...
* <a name="31"></a>We fixed an improper access control vulnerability. This behavior can be enabled by using the Mendix Runtime setting `DataStorage.EnableOrderByEntityAccess`. Please note this can lead to reduced performance in cases where a UI sorts on conditions that are more restrictive than the result set you are requesting. (3.1 – CVSS:3.1/AV:N/AC:H/PR:L/UI:N/S:U/C:L/I:N/A:N/E:P/RL:O/RC:C – for more information, see [Security Advisories](/releasenotes/security-advisories/))
