...
* We fixed an issue where calling a microflow from an offline-first application might trigger an unnecessary reload of data sources on the opened pages. (Ticket 114660)
* We fixed an issue in native mobile apps where in some cases a focused text input could be hidden behind the keyboard on Android. Make sure to also update Make It Native and Native Builder. (Ticket 113397)
* We fixed an issue where errors might happen during operations such as moving documents when certain custom widgets exist in the project.
