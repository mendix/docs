...
description: "이 문서는 Mendix로 문서를 만드는 방법에 대한 통찰력을 제공합니다."
---
...
Mendix로 나만의 문서를 만드는 방법이 궁금하셨나요? 이 참조 문서에서 그 방법을 알려드립니다!

Mendix를 사용하면 다양한 방법으로 문서를 생성할 수 있습니다. 여기에서는 자체 애플리케이션에서 문서를 생성하는 기본 사항을 배우게 됩니다.

시작하기 전에, 다음 페이지를 읽는 것을 권장합니다:

* [Document Template](/refguide9/document-templates/)
* [Generate Documents](/refguide9/generate-document/)

## 문서 파악하기

Mendix로 문서를 생성하기 전에, 생성하려는 문서의 초안 버전을 만드는 것이 좋습니다. 종이에 스케치하거나 고객에게 예시를 제공해 달라고 요청할 수 있습니다. 어떤 방법이든, 달성하고자 하는 것을 마음속에 두는 것이 좋습니다.

원하는 문서를 사용하여 생성 전략을 선택할 수 있습니다. Mendix는 기본 제공 Document Template 기능을 사용하여 문서를 생성하는 다양한 옵션을 제공합니다.

예시를 통해 이것이 어떻게 작동하는지 살펴보겠습니다.

## 비즈니스 사례

이 애플리케이션에서 고객은 제품을 구매할 수 있습니다. 주문을 생성하고 구매하려는 제품을 선택합니다. 고객에게 주문 개요를 제공하기 위해 PDF가 생성되어 확인 이메일의 첨부 파일로 전송됩니다. 주문에는 고객 세부 정보, 회사 로고, 가격이 포함된 제품, 주문 총액이 표시되어야 합니다.

## 기본 제공 Mendix 기능으로 문서 생성
...
**Customer**는 주소 정보와 선호 커뮤니케이션 언어를 보유합니다. **Order**는 모든 주문 라인의 날짜와 합계를 소유합니다. **OrderLine** Entity에는 **Product**에 대한 고객별 가격이 있습니다. 문서를 생성하려면 **OrderDocument** Entity가 추가되었습니다. 이 Entity는 **System.FileDocument** Entity를 상속합니다.

{{% alert color="info" %}}
**System.FileDocument** Entity를 직접 사용하지 마십시오. **System** 모듈의 해당 부분에서 보안을 제어할 수 없기 때문입니다.
{{% /alert %}}

### Microflow

이제 Domain Model이 설정되었으므로 새 문서 생성 기능을 위한 Microflow를 만들 준비가 되었습니다.

모든 주문 문서 생성 관련 소스를 구성할 새 폴더를 만드십시오:
...
현재 Microflow는 기본 시작점과 끝점만 있습니다:
...
Microflow를 만든 후 문서에 사용할 정보를 결정하십시오. 주문을 입력으로 시작하겠습니다. 나중에 주문 인스턴스를 통해 추가 데이터를 검색할 수 있습니다.

{{% alert color="info" %}}
Microflow를 만들 때 재사용을 촉진하기 위해 입력 매개변수의 수를 제한하는 것이 모범 사례입니다.
{{% /alert %}}
...
다음 단계에서 새 OrderDocument를 만듭니다. 이 객체는 실제 문서를 저장합니다. **Order** 객체에 대한 참조와 문서 이름을 설정하십시오:
...
이제 **Language** 객체가 필요합니다. 이 경우 **Customer**가 선호 커뮤니케이션 언어와 연결되어 있습니다. Microflow 예시에서는 먼저 **Order**를 통해 **Customer**를 검색한 다음 해당 **Customer**에서 **Language**를 검색합니다:
...
다음 단계는 **Generate document** 액티비티를 사용하는 것입니다. 이 액티비티 내에서 사용 가능한 객체를 사용하고 Document Template을 선택하여 문서를 만들 수 있습니다. 그러나 Document Template이 아직 존재하지 않으므로 폴더에 만들어 배치해야 합니다:
...
{{% alert color="info" %}}
Document Template에서 사용되는 Entity와 해당 속성에 대한 올바른 Entity 접근 설정을 확인하십시오. 템플릿에 표시되는 속성에 대해서는 읽기 접근이 필수입니다. 다음은 **User** 모듈 역할에 대해 **Read, Write**로 구성된 **Customer** Entity입니다:
...
이 예시에서는 다음 Document Template을 사용할 수 있습니다:
...
이 Document Template에서는 주문 세부 정보가 포함된 Data View로 시작합니다. 이 주문에서 고객 정보를 가져오고, 주문 라인에서 구매한 제품에 대한 정보를 가져올 수 있습니다.

Data View는 테이블, 테이블 셀, 라벨, 이미지, 줄 바꿈 및 Template Grid를 사용하여 문서를 구성합니다.

이제 Document Template을 만들었으므로 오류 독에 오류가 있는 것을 확인할 수 있습니다:
...
이 오류를 해결하려면 Microflow의 **Generate document** 액티비티를 여십시오. 액티비티가 열리면 매개변수 매핑이 업데이트되고 매핑 매개변수에 할당됩니다.
...
Document Template이 구성되었고 Microflow를 사용할 준비가 되었습니다. 이 Microflow를 하위 Microflow로 호출하면 메인 Microflow에 다운로드 액티비티를 추가할 수 있습니다. 이 Microflow는 다음을 수행할 수 있습니다:
...
이 예시에서는 **Entity (path)** 데이터 소스를 통해 **OrderLine** 정보를 검색했습니다. 이를 수행하는 대안적인 방법은 목록 프레젠테이션을 위한 객체를 반환하는 Microflow를 사용하는 것입니다. 이 경우 Document Template 내에서 데이터 소스 Microflow로 사용되는 Microflow에 올바른 사용자 역할을 추가해야 합니다.
