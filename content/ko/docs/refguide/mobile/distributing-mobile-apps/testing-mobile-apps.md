...
weight: 50
description: "Appium을 사용하여 두 플랫폼 모두에서 엔드투엔드 테스트를 한 번 작성하고 실행하는 방법을 안내합니다."
aliases:
...
* Set up Appium Inspector to spy on a Mendix native iOS app
* Set up Appium Inspector to spy on a Mendix native Android app
...
## Setting Up Appium Inspector to Spy on a Mendix Native iOS App

To set up Appium Inspector for native iOS testing, do the following:
...
1. Install and start Appium Server (you can use `appium` CLI or Appium Server GUI).
1. Start Appium Inspector and configure it to connect to your Appium Server (default: `http://localhost:4723`).
1. In Appium Inspector, add the following capabilities:
...
## Setting Up Appium Inspector to Spy on a Mendix Native Android App

To set up Appium Inspector for native Android testing, do the following:
...
Once you have the capabilities you want that work with Appium Inspector, you can also use them with Appium CLI to run automated tests. Everything you need to create your first automated test can be found [here](https://appium.io/docs/en/2.0/quickstart/). You can use Appium Inspector to find the right locators.
