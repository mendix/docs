...
* We added a [module-import command](/refguide/mx-command-line-tool/module/#module-import) to the **mx** command-line tool. [**TW: PR for new section?**]
...
* We improved the UX of the [Commit](/refguide/using-version-control-in-studio-pro/#pushing) dialog box for pushing when remote changes exist on the remote server. [**TW**]
...
* An external enumeration now shows the source of the enumeration and allows you to **Detach** it from the source. [**TW**]
...
* DONE: We fixed an issue where importing large XML schema documents was too slow. (Ticket 170962, 185085)
* DONE: We fixed an issue where a text area ignored a value autofilled in Google Chrome. (Ticket 176195)
* DONE: We fixed an issue with calling a microflow action where retrieving a specialization over an association did not work if the other side was the generalization. (Ticket 187667)
* DONE: We fixed an issue that caused the [Batch Update Button Icons](/refguide/app-menu/#batch-update-button-icons) dialog box to throw an exception. This includes addressing the scenario of adding an extra native icon selector when there are native documents and the selected icon is not supported on native. (Ticket 191511)
* DONE: We fixed an issue in the `$metadata` of published OData services using OData v3 where a self-referencing association resulted in two roles with the same name. (Ticket 193467).
* DONE: We fixed an issue where an XPath data source with a constraint that used the `id` attribute of a variable (for example, `[Module.Person_House/Module.House/id=$currentObject/id]`) resulted in an error during deployment. (Ticket 194055)
* DONE: We fixed an issue where setting a new AuthToken was not applied until the native app was restarted. (Ticket 195005)
