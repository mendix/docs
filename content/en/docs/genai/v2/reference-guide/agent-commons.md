---
title: "Agent Commons"
url: /agents/agents-kit-2/reference-guide/agent-commons/
...
aliases:
    - /agents/reference-guide/agent-commons/
...
* For [OpenAI](https://marketplace.mendix.com/link/component/220472), the configuration of OpenAI Deployed Models is part of the [configuration](/agents/agents-kit-2/reference-guide/external-connectors/openai/#general-configuration).
...
Besides microflow tools, tools exposed by MCP servers are also supported. To add MCP tools to an agent version, select an MCP server configuration from the [MCP Client module](/agents/agents-kit-2/reference-guide/mcp-modules/mcp-client/). You can then choose one of two ways to add MCP tools: 
...
* [Mendix Cloud GenAI Connector](/agents/agents-kit-2/mx-cloud-genai/mxgenai-connector/#configuration)
* [Amazon Bedrock Connector](/agents/agents-kit-2/reference-guide/external-connectors/bedrock/#sync-models)
* [OpenAI Connector](/agents/agents-kit-2/reference-guide/external-connectors/openai/#azure-ai-search)
* [PgVector Knowledge Base](/agents/agents-kit-2/reference-guide/external-connectors/pgvector/#general-configuration)
...
Note that [user access approval](/agents/agents-kit-2/reference-guide/commons/#enum-useraccessapproval) can only be set to `HiddenForUser` or `VisibleForUser` for knowledge base retrievals.
...
5. Pass both the `Request`, Agent, and optionally the context object to the `Call Agent (Runtime) w/ History` activity.
...
4. Optional: You can [create a file collection and add files](/agents/agents-kit-2/reference-guide/commons/#initialize-filecollection) to it that can be sent along with the user message to the model. Check the documentation of the underlying LLM connector for support of files and images.
5. Pass Agent and, if relevant, the optional request and context objects to the `Call Agent (Runtime) w/o History` activity.
