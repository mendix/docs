...
* In Mendix 9.23 we changed the mechanism to parse XPath constraints, which introduced a couple of issues. The following issues are found and resolved:
  * We fixed an issue where negative numbers resulted in an "Unknown function '-'.". error (Ticket 180161)
  * We fixed an issue where special characters (such as !, #, %, ^, &, * etc) got lost while typing.
  * We fixed an issue where the autosuggestion list would not contain '[%CurrentUser%]', '[%CurrentObject%]', or '[%UserRole_...%]' suggestions where it could suggest them
  * We fixed an issue where the autosuggestion list would show the wrong suggestions after a `System.owner/`, or `System.changedBy` association
