...
linktitle: "Jumping to Different Activities"
weight: 80
...
For the workflow to jump to other activities, you need to update the **System.WorkflowJumpToDetails** and the **System.WorkflowCurrentActivity** object members. You need to do the following:
...
When the **Action** attribute is set to *JumpTo*, the **System.WorkflowCurrentActivity_JumpToTarget** association should refer to the **System.WorkflowActivityDetails** object that is associated through the **System.WorkflowCurrentActivity_ApplicableTargets**. This ensures you select an applicable target. Referring to other **System.WorkflowActivityDetails** objects will lead to errors.
...
Jumping to other activities has a limitation in parallel splits: it is not possible to jump into or out of a current parallel split path. However, it is possible to jump to other activities in a current parallel split path. Activities in other parallel split paths and activities outside of the current parallel split path will not be available in the **System.WorkflowCurrentActivity_ApplicableTargets** association.
