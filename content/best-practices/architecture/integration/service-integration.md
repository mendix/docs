...
### 3.3 Copy data or Get it in real-time?

If data generated by one app is needed in another app, there are two main directions:

* Store a copy of the data in the other app
* Retrieve data on demand whenever it is needed

In coming releases Mendix will make Remote Objects available, using OData to get data from another app in real-time and seallessly. This will be great for apps that work tightly together in a microservices system.

But in general, with microservices architecture, it is more common to copy required data over in the background to make apps as operationally independent as possible. At least if the app that needs the data uses it frequently and combines it with other local data to display in a grid.

After the data is copied over, it needs to be kept up-to-date and then the option is for the source to Push data or the destination to Poll for changes.

### 3.4 Push or Pull to keep data up-to-date?
...
To create autonomous microservices and apps the pull-based synchronization between Mendix apps is most common. However when updating SaaS solutions we almost always have to adapt to their published API, so Push-updates are more common
