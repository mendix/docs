...
7. Copy the **Token secret** to your clipboard by clicking the storage icon below the secret.
    ...
    {{% alert type="info" %}}For every request that is made to the Data Hub API, you must include the following key value pair as a header `Authorization: MxToken <your_PAT_Token>`.
