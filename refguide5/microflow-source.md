...
In most cases, the intended object or objects used to fill a [data widget](/refguide5/data-widgets) is obvious: an edit form opened from a data grid is going to require an object selected in the data grid, a data grid gets its objects from a database query, and a nested template grid will retrieve its objects over an association. However, sometimes the target objects need to adhere to very specific criteria, or different objects are shown under different circumstances that cannot be handled by [XPath](/refguide5/xpath-constraints). In these situations a microflow data source may be required.
