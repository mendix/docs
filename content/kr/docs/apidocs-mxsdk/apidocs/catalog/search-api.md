...
description: "Search API를 사용하면 앱 개발에 사용할 수 있는 등록된 자산에 대한 정보를 검색하고 가져올 수 있습니다."
weight: 50
type: swagger
---
...
Search API를 사용하면 사용자가 지정된 검색 기준을 만족하는 Catalog에 등록된 자산을 검색하고 가져올 수 있습니다.

오프셋(offset)을 사용하여 검색 결과를 페이지네이션할 수 있으며, 이를 통해 결과 수를 제한하고 건너뛸 수를 지정할 수 있습니다.
...
검색 쿼리에 제공할 수 있는 내용과 제공할 수 없는 내용에 대한 자세한 내용은 아래의 [API 레퍼런스](#api-reference) 섹션을 참조하십시오.
...
검색어가 Customer인 요청의 예시는 다음과 같습니다:
...
성공적인 `GET` 호출은 `200` 상태 코드와 검색 결과에 대한 세부 정보가 포함된 JSON 응답 본문을 반환합니다:

<details><summary><b>JSON 응답 본문 보기 클릭</b></summary>
