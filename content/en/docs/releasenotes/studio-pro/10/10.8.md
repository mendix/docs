...
For more information on the Runtime behavior of data sources, see [here](/refguide/datasource-runtime). {TW}
* You can now publish entities in OData services without the *Read* capability. This allows you to use entities without exposing the data, and also non-persistable entities, as parameters or return types for your published microflows. On the consume side these non-readable external entities can be imported where they will behave like non-persistable entities, and can be passed to a **Call external action** activity as parameters or used as its return type. {TW}
...
* We now directly add a default output variable to the **Create Variable** action in nanoflows and microflows. {TW}
* We added the ability to navigate through styling properties by using keyboard shortcuts, such as tab and the arrow keys. {TW}
...
* Data grid, static image, and dynamic image widgets, which are not supported in the React client, can be converted to their modern counterparts as Data Grid 2 and Image widgets. This conversion can be performed through the context menu of the mentioned widgets when the React client is enabled. {TW}
* We now support HTTP Method `PUT` in consumed REST service (beta). {TW}
* We now made the body content text editor as always editable for Consumed REST service. {TW}
* We now show a banner in body structure tab indicating whether the edited body content matches existing mapped structure, if any, for Consumed REST service. {TW}
* You can now prevent published OData services from including a `@context` attribute in the response by default, by changing the default behaviour with a toggle in Studio Pro. {TW}
* A dialog box for external entities in Studio Pro now shows the service from which the entity originates, with a Show button that navigates to the service. {TW}
* We upgraded Swagger UI from 4.10.3 to 5.11.2. The Runtime uses this to show and interact with the OpenAPI documents of published OData services and published REST services. {TW}
* For microflows published in OData services, Studio Pro now has a **Show** button for entity, list and enumeration parameters and return types. That button navigates to the entity or enumeration. {TW}
* We made microflow editor connection points and resize handles easier to click. {TW}
...
* We changed the default Java version for new projects to 17.
