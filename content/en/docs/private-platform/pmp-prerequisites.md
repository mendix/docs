...
Private Mendix Platform provides an end-to-end Mendix developer experience to customers who need to enjoy it on their private infrastructure. It is dependent on Kubernetes, and integrates with tooling in the customer's landscape.
...
[Microsoft Azure Government](https://azure.microsoft.com/en-us/explore/global-infrastructure/government/) (also known as Azure US Government Cloud) is a fully supported deployment environment for Private Mendix Platform, using Azure Kubernetes Service (AKS) provisioned in Azure Government regions.
...
{{% alert color="warning" %}}
If the cluster is running nodes with multiple architectures (for example, *x86-64* and *ARM64*), the namespace where Private Mendix Platform is installed should use a fixed (specified) architecture. One way to do this is by configuring a [PodNodeSelector](https://kubernetes.io/docs/reference/access-authn-authz/admission-controllers/#podnodeselector) for that namespace, and only using nodes with a specific architecture (for example, `amd64`).
...
It is not possible to use Private Mendix Platform in [OpenShift Online](https://www.openshift.com/products/online/) (all editions, including Starter and Pro) or [OpenShift Developer Sandbox](https://developers.redhat.com/developer-sandbox) because they don't allow the installation of Custom Resource Definitions.

Kubernetes included with [Docker Desktop](https://docs.docker.com/desktop/kubernetes/) is not officially supported.

#### Container Registries{#container-registries}

Private Mendix Platform builds container images for every app and pushes them to the registry. It needs credentials to access the registry and permissions to push images into the registry.

Images are pulled from the registry by Kubernetes, not by Private Mendix Platform.
The configuration script for Private Mendix Platform can configure Kubernetes image pull secrets and use the same credentials it uses for pushing images (for all registries except EKS).
...
{{% alert color="info" %}}
While Private Mendix Platform supports all Postgres versions listed above, the Mendix Runtime might require a more specific Postgres version.
...
If Strict TLS is enabled, Private Mendix Platform will connect to the PostgreSQL server with TLS and validate the PostgreSQL server's TLS certificate. In this case, the connection will fail if:
...
{{% alert color="info" %}}
By default, Private Mendix Platform will not enforce encryption. Encryption can be enforced in SQL Server if required.
...
[Amazon S3](https://aws.amazon.com/s3/) is supported. Private Mendix Platform supports multiple ways of managing and accessing S3 buckets: from creating a new S3 bucket and IAM account per environment to sharing an account and bucket by all environments in a namespace.
...
Private Mendix Platform will use the existing ingress controller.
{{% /alert %}}
...
Private Mendix Platform is compatible with the following ingress controllers:
...
Private Mendix Platform can create Services without an Ingress. In this way, the Ingress objects can be managed separately from Private Mendix Platform.

Private Mendix Platform can create Services that are compatible with:
...
Starting with Mendix Operator v2.5.0, the following service meshes can be enabled for the entire Private Mendix Platform namespace:

* [Istio](https://istio.io/)
* [Linkerd](https://linkerd.io)

If service mesh sidecar injection is enabled, all communication between pods in the Private Mendix Platform namespace will happen through the service mesh.
