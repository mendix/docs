...
* We fixed an issue where scheduled events did not execute in a horizontally-scaled environment and in some cases in a single-node environment. (Ticket 146867)
...
* Scheduled events are not triggered on horizontally-scaled environments and in some cases in a single-node environment.
