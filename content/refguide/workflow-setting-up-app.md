...
*   **WorkflowDashboard** - When you only want to use the workflow dashboard as your admin go to page. Add this to the navigation instead of the *WorkflowAdminCenter*. This page will give you workflow/task based metrics. And direct access to all *WorkflowInstance* as well as *WorkflowUserTask* data.
*   **MyInitiatedWorkflows** - A useful page for those users that initiate workflows. This page will give them an overview of all their initiated workflows, determined by the owner association set upon starting a workflow. On this page they may see the current state, task timeline and have the ability to withdraw (abort) workflows that are in progress, paused or incompatible.
