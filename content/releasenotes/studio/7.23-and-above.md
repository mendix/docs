...
* You can now import a spreadsheet to an app and we will generate a fully functional app based on your data!  The data of the spreadsheet will be analyzed and converted into entities, attributes, and associations. Also, pages and navigation items based on your data can be created. Click **Create App**  and select **App from a spreadsheet** in the Developer Portal to get started. For more information, see [Starting With Your Own Data](start-with-data) in the *Studio Guide*. 

#### Fixes

* We fixed an issue where a guidance tour crashed on the preview.
* We fixed an issue where the list view **Properties** tab with a filter would result in an error when the selected entity was removed.
