...
### 6.3 Impact on using non-persistent entities and changed entities in microflows and java actions
When a user calls a microflow from the client, a copy of the state is send with the request to the runtime. This copy of the state stays at the runtime and is updated by the runtime during processing of this request. After the request finished processing, it will return to the client and the client will update it's client state with the information returned by the response.

On the server side however, this state can only be accessed by that request handling action. This means that it is no longer possible to query the state of an NPE when it is updated by another request, other than via the client (as the other request needed to return this NPE to the client, which needs in turn send this with a subsequent request to the server). 

When a request triggers an asynchronous action on the runtime, this asynchronous action gets the state as it was from the moment it initiated. Only if this action was asynchronously executed by the client, it can be tracked by the client and the client can get the updated state when the asynchronous action is finished. When the asynchronous action is triggered on the runtime using a Java Action, the state updates can not be tracked by the client (similar to refresh, datavalidation and other instructions).
