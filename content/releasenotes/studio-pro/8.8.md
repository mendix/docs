...
* Calling a microflow from a nanoflow in an offline app with an NPE that has an association to another NPE that is not used in the microflow, will cause an error. The workaround is to either use all associations in your microflow, or to have no associations in the NPE.
