...
The [Mendix Make It Native](https://play.google.com/store/apps/details?id=com.mendix.developerapp) app is currently in beta phase as part of the Mendix 8.0.0 Beta. Throughout the beta phase, we will continue to improve and extend the functionality. However, you may encounter some limitations:

* Mendix native mobile apps can only be viewed using the Mendix Make It Native app, available in the Google Play Store and Apple App Store. Throughout the beta program, we will add support to create standalone apps (for example, Android APK or iOS IPA) that can be distributed using your preferred distribution channel (for example, Google Play).
...
The **View mode** in the page editor has been upgraded to the **Design mode**! With this update, the feature is not limited to just previewing your app, as you can now edit the page without having to switch modes. Combined with significant improvements in resource management, we do not know why you would ever want to go back! 
...
* We improved the performance of Stduio Pro when you are adding or deleting documents. The algorithm ensuring consistency now performs fewer searches through the app project.
...
* The check-box CSS class is now set on the wrong `div` if the read-only style is set to control and the form orientation is horizontal. It was missing if the form orientation was vertical.
...
* We removed the client APIs marked as to-be-removed in Mendix 8. Check the currently supported list [here](https://apidocs.mendix.com/8/client/).
...
* **aws-java-sdk-s3** from 1.11.525 to 1.11.534
