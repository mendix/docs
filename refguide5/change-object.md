...
Change member action _set_ on _zipcode_ | toUpperCase($InputCustomer/zipcode) | A [microflow expression](/refguide5/microflow-expressions) can be used to set all characters to upper case (capitals)
...
Option                     | Description
-------------------------- | -----------------------------------------------------------------------------------------------
Yes with event handlers    | The object is saved in the database and the [event handlers](/refguide5/event-handlers) are triggered
Yes without event handlers | The object is saved in the database, but the [event handlers](/refguide5/event-handlers) are not triggered
