...
|---|---|---|
| MS-1144 | None | Upgrade to TypeScript 3.1. |
| MS-1156 | None | Add onEventProcessed hook to AbstractModel. |
| MS-1157 | None | Add optional param to deepCopy() so client can map from original ID. |
| MS-1162 | None | Removed the `utils.serializeToJs(unit)` method, it's replaced by `JavaScriptSerializer.serializeToJs(unit)`. |
