...
#KI: "Cookies set by the client" UICORE-879
...
Cookies set by the client do not fully comply with the new requirements with regards to the SameSite attribute, which are being rolled out by Chrome (you can find a good explanation of SameSite cookes in [SameSite cookies explained](https://web.dev/samesite-cookies-explained/) on the _web.dev_ website. Some users may experience problems opening their application when running locally and when using Chrome. This issue will be resolved with 8.11.1. Until then, we advise you to disable the flag that controls this behavior by navigating to chrome://flags/ in Chrome, and disabling the #cookies-without-same-site-must-be-secure flag. 
