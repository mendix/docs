...
With Studio Pro 8.15 we are releasing Mendix Native Mobile Builder. Mendix Native Mobile Builder is a UI tool to build your Mendix native mobile apps with just a few clicks. The UI replaces the CLI workflow and adds new enhanced functionality (such as signing your apps and setting up capabilities like push notifications) that was difficult or impossible to realize in a user-friendly way via a CLI. 
...
* We improved the screen for adding or editing a [consumed web service](/refguide/consumed-web-services). It is now a tabbed document instead of a pop-up window.
...
* We improved the customization of styles for [text box](/refguide/text-box), [text area](/refguide/text-area), and [drop-down](/refguide/drop-down) for native widgets in their focused state. Now, you are able to add specific styles when these widgets are focused. For more information, see [Native Mobile Styling](https://docs.mendix.com/refguide/native-styling-refguide) in the *Studio Pro Guide*.
...
* <a name="108885"></a>We fixed a known issue in [8.14.0](8.14#8140) and [8.14.1](8.14#8141) with deleting a branch from the **Branch Manager**. (Tickets 108885, 109072) [**UPDATE KIs**]
* <a name="2562"></a>We fixed a known performance issue in [8.14.0](8.14#8140) and [8.14.1](8.14#8141). Now, the [Delete object(s)](/refguide/deleting-objects) action is not slowed down if there are microflow variables containing large lists of entities in scope. [**UPDATE KIs**]
* <a name="2554"></a>We fixed an issue in the [debugger](/howto/monitoring-troubleshooting/debug-microflows) where an error was thrown when breakpoints were added to microflows from scheduled events. [**UPDATE KIs**]
...
* When a pluggable widget has an `attribute` property and a `widgets` property linked to a `datasource` property, rendering additional items updates the items already rendering with the last-used object. (Tickets 105333, 108482)
* We fixed a bug where the **Checking for errors** message in the status bar did not complete. (Ticket 108549)
...
* We fixed an error in the expression of a [log message](/refguide/log-message) action that is created by [Mendix Assist](/refguide/mx-assist-studio-pro). (Ticket 108966)
* We fixed an issue where on certain conditions, [data grid](/refguide/data-grid) tooltips threw errors when you hovered on them. (Tickets 103882, 106171)
