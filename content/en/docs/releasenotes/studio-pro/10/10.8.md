...
* We fixed an issue where the Studio Pro suddenly shut down while using an Expression or XPath editor (new text editor). A side effect of the fix is that the Accessibility Cursor is not supported by the control anymore. (tickets #205853)
...
* We changed the minimum object size in the logic editors. {TW}
