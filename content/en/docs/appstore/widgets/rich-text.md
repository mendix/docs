...
* When the keyboard input set to a language which utilises Input Method Editors (IME), like Japanese or Chinese, which can offer suggestions for the input through a dropdown, using "Spacebar" to select a suggestion can cause the dropdown to close without selecting any suggestion, causing the entered value to have no value in CKEditor to read. If the user wants to submit/save this value, CKEditor misses handling this IME behaviour, therefore this may print errors in the browser console, and cuase rendering issues in older Rich Text versions.