...
| Building the app Teamcenter trows an error: **Execution failed for task ':app:processDevDebugGoogleServices'. > No matching client found for package name 'com.mendix.myapp.testlocal.developerapp'** | The google-services.json contains an package_name this should match with the Native app package identifier | [Add an app](setting-up-google-firebase-cloud-messaging-server#native-apps) with the correct package identifier to Firebase and update the google-services.json in your GitHub repository |
