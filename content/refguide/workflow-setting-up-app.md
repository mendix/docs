...
*   **WorkflowDashboard** – This page gives you workflow/task based metrics, and direct access to all *WorkflowInstance* as well as *WorkflowUserTask* data. When you only want to use the Workflow Dashboard as your admin go-to page, add this to the navigation instead of the *WorkflowAdminCenter*. 
*   **MyInitiatedWorkflows** – A useful page for those users that initiate workflows. This page gives them an overview of all their initiated workflows, determined by the owner association set upon starting a workflow. On this page they can view the current state, task timeline and have the ability to withdraw (abort) workflows that are in progress, paused, or incompatible.
...
*   **Workflow_TaskDashboard** – May be used to generate your own *TaskDashboard*, for example, when you want to use your own metrics or add context information.
