...
- We reverted the fix for the issue where created files are not deleted when an error occurs in a microflow. (Reintroduces Ticket 199712) 
- We reverted updating the `changedBy` attribute when `storeFileDocumentContent` or `storeImageDocumentContent` from the public API is used, or when uploading files.
- We enabled users to set any name for the applications being imported from solutions.
- We improved the performance of the Microflow Editor by reducing the time it takes to create the list of suggestions for adding new elements. (tickets #204887, #203936)
