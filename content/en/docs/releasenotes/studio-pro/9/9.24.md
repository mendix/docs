...
- In the Studio Pro Preferences you can configure a different JDK for each supported version of Java.
...
- We changed StudioPro installer for windows to check for currently installed Git CLI version and upgrade it if needed (perviously installer didn't change anything if any Git CLI vesion is installed).
