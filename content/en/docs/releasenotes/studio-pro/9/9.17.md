...
* In the App Explorer's [styling node](/howto/front-end/customize-styling-new/#styling-editor) we added file and folder operations such as create, rename, and delete. Simply right-click a file or folder to see the available options.
...
* We fixed an issue that caused unnecessary warning pop-up messages to appear in the Make it Native app when running in development mode. (Ticket 148162)
...
* We fixed an issue that occurred with the [pow expression](/refguide/mathematical-function-calls/pow) in a nanoflow when the exponent was not an integer number. (Ticket 153289)
* We fixed an issue where decimal values were displayed in exponential notation (for example, 0E-8). (Ticket 153332)
* We fixed an issue where a reference selector as the first input element on a page did not get the focus when the page was first loaded. (Ticket 153456)
