...
{{% alert color="info" %}}
For more information on using Studio Pro 10, see [Upgrading from Mendix Studio Pro 9 to 10](/refguide/upgrading-from-9-to-10/).
{{% /alert %}}
...
* We have deprecated SVN version control. In Studio Pro 10, SVN apps are treated as non-version controlled, so no version-control operations are allowed (meaning, update, commit, revert, merge, publish, and all other operations related to version control). Studio Pro shows a deprecation warning with the link to [How to Migrate to Git](/developerportal/collaborate/migrate-to-git/).
