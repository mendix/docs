...
{{% alert color="info" %}}
This section applies only if you have access to AI Hub and exported your results to an AI Hub repository. If you stored results locally, you cannot deploy your model for integration purposes.
{{% /alert %}}
...
If the test was successful, there is nothing stopping you from integrating the model into your Mendix app. 
...
1. **Create input data representation**: Design your domain model to represent the input data required by the model
   * You can derive the required structure from the test page in AI Hub, which shows the expected JSON format. It does not need to match the structure exactly. It is recommended to keep API structures separated from your app's entities
...
1. **Create JSON Structure**: Configure a JSON structure that maps to the input format required by your AI Hub endpoint
   * Use the JSON format shown in the AI Hub test page as your reference (as described [above](#testing-endpoint)). In the example app, an `EmployeeId` was added which is not used for the prediction, but helps identifying the objects in the response later
   * Ensure all required fields are included and properly typed

2. **Prepare data structure** (optional): If your master data doesn't fit the structure required by the REST call:
...
2. **Add Call REST (POST) action** in your microflow that triggers the prediction:
...
After successfully calling your AI Hub endpoint, you need to process the response and integrate the prediction results into your Mendix application:
...
4. **Import response in your microflow**: In your microflow that calls the REST endpoint, add response processing immediately after the REST call:
   ...
   * Use the object(s) to postprocess, for example changing a status of your Mendix data

5. **Handle batch processing and data matching**: If you're processing multiple predictions and need to update existing data:
