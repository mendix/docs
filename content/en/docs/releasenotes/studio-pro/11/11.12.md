...
* We fixed an issue where Studio Pro could freeze while a progress dialog is shown, for example, while opening an app.
...
* We fixed an issue where renaming in Studio Pro (for example, a module or an entity) could lead to consistency errors in microflow expressions.
...
* We fixed an issue where design properties from an excluded module could incorrectly trigger an error that they had been renamed. (Ticket 281790)
...
* We fixed an issue where placing a widget with a selection, such as a **Combobox**, inside a repeatable container widget (for example, the **Gallery** widget) could prevent expressions from using `$currentObject` or cause a client error when selecting an attribute from the data source. (Tickets 283186, 283708)
