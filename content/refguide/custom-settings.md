...
| com.mendix.core.SameSiteCookies | The [SameSite property](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie/SameSite) to include in all cookies that are returned by the embedded HTTP server. Possible values are `"Strict"`, `"Lax"` and `"None"`. At present `"None"` is the default, but this will change to `"Strict"` in  the next major release. Setting it to `"None"` is typically needed only when an application is embedded in an `iframe` of another application with a different domain. Newer browsers may require the connection to be secure (HTTPS) when set to `"None"`. If the connection is plain HTTP, then this setting must be changed to `"Strict"` (recommended) or `"Lax"`. This setting is present since Mendix 8.11.
