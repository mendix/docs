...
* In MxAssist Logic Bot, when you are not logged in or you are offline, the AI-generated suggestions are now not available. You will be notified through the Logic Bot menu.
* MxAssist Logic Bot now shows parameterized recommendations for a Change list activity.
* MxAssist Logic Bot now shows **Retrieve** list before **Retrieve** first for a parameterized suggestion. [**TW: MAKES SENSE?**]
...
* In the logic editors, we fixed the error handler connection points on activities that have custom error handlers.
* In the logic editors, we fixed showing the MxAssist Logic Bot suggestion flow with the error handler.
* In the logic editors, we fixed an issue with MxAssist Logic Bot not opening on a newly created microflow, nanoflow, or rule.
