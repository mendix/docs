...
* The state of the **Show purely visual visualchanges** toggle in the [Changes](/refguide/changes-pane#modified) pane is now remembered when switching between documents or modes.
* We updated the splash screen, about screen, empty state background, and various icons in Studio Pro.
...
* We fixed a bug for Tree selection in [MxAssist Performance Bot](/refguide/mx-assist-performance-bot) changed to be full width.
