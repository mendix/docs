---
title: "Agent Commons"
url: /agents/agents-kit-1/genai-for-mx/agent-commons/
...
Note that [user access approval](/agents/agents-kit-1/genai-for-mx/commons/#enum-useraccessapproval) can only be set to `HiddenForUser` or `VisibleForUser` for knowledge base retrievals.
...
4. Optional: You can [create a file collection and add files](/agents/agents-kit-1/genai-for-mx/commons/#initialize-filecollection) to it that can be sent along with the user message to the model. Check the documentation of the underlying LLM connector for support of files and images.
