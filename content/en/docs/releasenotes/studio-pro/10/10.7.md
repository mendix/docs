...
Studio Pro on Mac is now in [beta](/releasenotes/beta-features/). This allows you to run Studio Pro natively on Mac without Parallels. With the Mac version of Studio Pro, you can edit your apps on Windows using the functionality you are used to.
...
We added a mechanism to register microflows as event handlers for events that occur to workflows. The event-handler microflows are provided with detailed workflow data that can be used to create audit trails or performance diagrams or be used for analytical purposes.

### Improvements

* [MxAssist Logic Bot](/refguide/mx-assist-logic-bot/) now supports parameter matching for the **Show Page** activity.
* We introduced the new selection property for the [Pluggable Widgets API](https://docs.mendix.com/apidocs-mxsdk/apidocs/pluggable-widgets/) which allows widgets to read and set selections that can be used in actions, expressions, or a **Listen to** data source of a data view.
* We changed the keyboard support in [MendixChat](/refguide/mendix-chat/) for building and sending a prompt. <kbd>Enter</kdb> sends while <kbd>Shift</kdb> + <kbd>Enter</kdb> adds a new line. <kbd>Ctrl</kdb> + <kbd>Enter</kdb> works in the same way as <kbd>Enter</kdb>, because <kbd>Ctrl</kdb> is not taken into account anymore.
...
* When moving documents that have roles configured for them (for example, roles like pages or microflows) to a different module, Studio Pro now tries to re-map the configured roles to roles with the same name from the module to which you are moving the documents.
...
* We updated the [Data Importer](/appstore/modules/data-importer/) module to read, parse, and import CSV (comma-separated values) files. The existing Data Importer menu can be used to configure your CSV files. The configured settings can be utilized with the existing [Import data from file](/refguide/import-data-from-file/) activity.
...
* We fixed an issue with the `System.ArgumentOutOfRangeException` in microflow expressions using if-then-else expressions. (Ticket 204039)
...
* We fixed an issue that occurred when you opened MendixChat for the first time and had to click **Sign in** even though they were already signed in to Studio Pro.
...
* We fixed an issue with the schema import wizard for published web services in Studio Pro on MacOS.
* We corrected the `@context` URL returned for OData actions that return an object or a list of objects. This now points to the entity type metadata instead of that of the entity set.
