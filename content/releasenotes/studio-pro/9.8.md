...
* We increased the node memory limit when bundling custom widgets to 1GB (from 512MB). This should prevent running out of memory when bundling large apps with lots of custom widgets. (Ticket 88774)
* A microflow with a [Show home page](/refguide/show-home-page) activity will now send used objects in memory when called. (Ticket 127576)
* We no longer invalidate sessions when the deletion of existing sessions fails after an app model change. (Ticket 127632)
* We added the option to enable checking security for pages only accessible by page URL. This is enabled by default for new apps and will also become the new default in the next major Mendix version. (Ticket 129205)
...
* When you right-click on a [non-persistable entity](/refguide/persistability#non-persistable) in the domain model, the option to publish it as an OData resource is now disabled. This avoids a consistency error, as non-persistable entities cannot be published in OData services.
...
* We removed the redundant **OK** button from the [History](/refguide/history-dialog) form, leaving only the **Close** button.
...
* We changed how the names of the external entities are displayed in the Data Hub pane so that they are now consistent with the dataset names in the Data Hub Catalog.
...
* It is now possible to change multi-valued associations via [published OData services](/refguide/published-odata-services).
...
* As of this release native mobile apps will encrypt the authentication token and session cookie on Android to further enhance security. New native mobile apps will apply this encryption automatically. For existing apps, the native mobile app needs to be updated and built, based on Native Template version 6.2.9 or higher. (Ticket 129891)

#### Default Values in Offline-First Apps

With this release, we added support for default values in offline-first apps. This means that when an object is created, it uses the default values as defined in your domain model, just like web apps do. This makes life easier, as it is aligned with the online/web apps and requires less effort to create new objects.

#### Microflow Calls in Offline-First Apps

We have reworked the internals of how non-persistable objects are passed as microflow parameters in offline-first apps, resulting in better performance, increased stability, and more flexibility.

A microflow call is now a single request instead of multiple network calls, which improves the performance and stability of calling microflows. Depending on the amount of objects sent, this can be a great boost.

Is that it? Not yet. Do you recall the error message, “Non-persistable objects created in a microflow can’t be passed to another microflow”? Well, you can forget this! Because we are also happy to announce that you can now pass non-persistable objects to microflows, even if they were created inside another microflow.
...
* We fixed an issue where Mendix Runtime did not start after replacing a module containing a remote entity. (Ticket 131331)
* We fixed an issue when invalid SVGs in the image collection crashed Studio Pro while an app was being published. (Ticket 131378)
...
* We fixed an issue in the generated metadata of a [published OData serivce](/refguide/published-odata-services). The capability annotation that indicated which (navigation) properties are not updatable used a string instead of `(Navigation)PropertyPath`.
...
* We fixed another issue in the app selector so branches are ordered so that the main branch is displayed first.
