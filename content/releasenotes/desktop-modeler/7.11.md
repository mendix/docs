...
* We fixed the issue where the search field over a reference on a data grid did not show all the available values. (Tickets 58579, 59803, 60132, 60257, 60436)
...
* We simplified data synchronization for applications. Previously we were trying to filter the data by analyzing your model, but as models get complicated this approach became complex as well. Now all instances of entities used in offline applications will be synchronized to the local database. We fixed an issue where modeler crashes during deployment. (Ticket 60235)
...
* JSON structures do not support the characters `:`, `|`, or `^` in property names, so we added a validation message when you try to use them.
...
* When navigating back to a page with a data grid inside a non-default tab, the current value of the search field of that grid was not remembered correctly if the field was the attribute of an associated entity. We fixed this.
