...
APD Performance Recorder는 Mendix 애플리케이션 내의 각 개별 액션을 기록할 수 있게 합니다. 두 가지 기록 옵션이 있습니다:

* 런타임 기록은 서버 액션(Microflow 및 클라이언트 API)을 기록합니다
    * 느린 Microflow를 조사하거나 시간이 많이 소요되는 액션을 정확히 파악하는 데 유용합니다
* 브라우저 기록은 사용자의 애플리케이션 연결을 기록합니다
    * 사용자 관점에서의 성능과 서버 응답의 결과로 브라우저가 수행하는 후속 액션을 보여줍니다
...
**Browser** 유형의 기록을 클릭하면 [브라우저 레코더 결과](/appstore/partner-solutions/apd/rg-three-browser-recorder-results/) 화면이 열립니다. **Server** 유형의 기록을 클릭하면 [서버 기록 결과](/appstore/partner-solutions/apd/rg-three-runtime-recorder-results/) 화면이 열립니다.
