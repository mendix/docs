...
* We added a checkbox in the user targeting section of a workflow user task allowing to automatically assign a user task when a single user is targeted. For more information, see the [Auto Assign When Result Is One User](/refguide/user-task/#auto-assign) section in *User Task*.
...
* We fixed an issue that prevented workflows built in versions below 9.7 to be upgraded to the latest version.
