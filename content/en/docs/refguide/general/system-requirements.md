...
Only devices running on these operating system versions receive up-to-date security fixes from their vendors and thus minimize the risk of being vulnerable to known exploits.

You can build native apps with Mendix that run on older operating system versions than the ones we support. However, to receive official Mendix support you must demonstrate that the problem also occurs on a supported operating system version.
...
Depending on the complexity of your application, these hardware requirements might not be sufficient and should be adjusted.
