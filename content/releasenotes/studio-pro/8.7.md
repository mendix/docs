...
* **KI: "Export mappings" = IN-3267 = #93998**
* **KI: "When you use a JSON structure" = IN-3414 = #95390**
* **KI: "MySQL 8.0 cannot" = DAT-1530 = #1530**
...
* We added a new event type for the [text box](/refguide/text-box) widget. You can now connect a action that is trigger when the user presses <kbd>Enter</kbd>. This is useful for triggering searches.
...
To accomplish this, link the [widgets property type](/apidocs-mxsdk/apidocs/property-types-pluggable-widgets#listvalue) to the new [datasource property](/apidocs-mxsdk/apidocs/property-types-pluggable-widgets#listvalue). This will enable selecting a [list data source](https://docs.mendix.com/refguide/data-sources#types). You can then add widgets to the drop-zone defined by the widgets property in the context of the data source.

#### Suppress Warnings

Even though model warnings are useful, you now have the option to suppress them in Studio Pro. You can decide which warnings will not be shown. This can be a specific warning, all the warnings in a module, or even all the warnings from [App Store content](/developerportal/app-store/app-store-content).
...
* We improved the performance of the new version of the [Project Explorer](/refguide/preferences-dialog#new-project-explorer) for expanding and collapsing a folder or module as well as scrolling and filtering the documents. If you previously switched it off, you might to give it a try by enabling the option in **Edit** > **Preferences** > **New features** > **New version of the Project Explorer**.
* We improved the performance of export mappings that export persistable associated objects. The [Mendix Runtime](/refguide/runtime) now retrieves batches of associated objects instead of individual associated objects. This reduces the the number of calls to the database. (Ticket 89189)
* We greatly improved the performance of the last step of [committing](/refguide/commit-dialog) and updating. (Ticket 93773)
...
* We fixed an issue where problems with the appearance of elements were not always correctly reported.
* We fixed an issue where issues with missing or outdated custom widgets were not reported on [page templates](/refguide/page-templates) and [building blocks](/refguide/building-block). In rare cases, this can trigger consistency errors for existing projects. If this happens, please follow these steps:<br />
    ...
    3. Replace the content of the **theme** folder with the content of the **theme_old** folder to restore your app project theme.
...
* We fixed the resize behaviour for pop-up windows that contains an image. When the parent window is resized, it now has the correct height.
