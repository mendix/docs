...
To access these changes, update the [Atlas Core](https://marketplace.mendix.com/link/component/117187) Marketplace module, where we have included the new design properties.
...
Enable the new UI via the option in the [Preferences](/refguide/preferences-dialog/#new-features) dialog box.
...
* We added a feature that allows for switching between portrait and landscape rendering of the workflow editor.
* We have implemented a new text editor that is enabled by default (with the option in **Preferences** > [New Features](/refguide/preferences-dialog/#new-features) to go back to the legacy text editor). This new text editor will also work on MacOS.
...
* We added a **Wait for timer** event for workflows that stops the process execution until the defined period of time has elapsed.
...
* MxAssist Logic Bot now shows **Retrieve** list before **Retrieve** first for a parameterized suggestion.
...
* We changed the dialog box for changing an entity published in an OData service. The fields about the structure and the fields about the data are now in two separate group boxes.
...
* We made changes to provide more error and label consistency in the microflow and workflow actions form when a variable is deleted (if it was being used).
