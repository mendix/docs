...
* We prioritized "Check if object exists" decision recommendation after Retrieve object operation in Logic Bot menu.
* Logic Bot now always shows a parameterized list for the "Calculate from a list" menu item.
...
* We fixed an issue when there was an import mapping which was not committed and the mapping method was "Find by key", then the child objects couldn't be retrieved by association. (Ticket 126464)
* We fixed an issue when Logic Bar toggle was shown for non editable microflow.
* We fixed an issue where client certificates with empty passwords could not be used. (Ticket 137950)
* We fixed an issue that in certain cases, a change inside a document does not show up in the the updated ‘Find Usages’ results until another change happens or the app is re-opened.
* We fixed an issue where the target namespace in XML schema import is not stored in Mendix project, resulting in XML validation failure during import and export mappings in Runtime. (Ticket 123183)
* We fixed an issue where Logic Bot generates incorrect aggregate action for Calculate maximum and minimum suggestions.
