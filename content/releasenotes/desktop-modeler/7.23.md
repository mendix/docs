...
* Custom widgets which subscribe using the `this.subscribe` function and unsubscribe using `mx.data.unsubscribe`, get an error when calling `mx.data.unsubscribe`. The workaround is to call `this.unsubscribe` instead, which is also the recommended way. 
(Tickets 79881, 80075, 80078 and 80174)
