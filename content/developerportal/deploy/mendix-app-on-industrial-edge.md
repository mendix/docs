...
* The Industrial Edge App Publisher UI/CLI installed on your development PC
   ...
   You need the following versions of teh Industrial Edge App Publisher:
...
### 3.2 Create Docker Image of Your Mendix App  
...
1. Log into your IEM.
2. Create a project and the according Mendix app inside the project.
...
The container will search the location, specified in the environment variable: `IEM_CONFIG_PATH`, for files with the extention `.env`. The `.env` files can contain one or more environment variables, which will be added to the environment variables of the container. This can be used to set Edge device specific constants, scheduled events, or custom runtime settings. Check in the [configuring constants](https://github.com/mendix/cf-mendix-buildpack#configuring-constants) section of the Mendix buildpack for the syntax to use. 

See the next section for an example compose file including the `IEM_CONFIG_PATH`.
...
5. In the wizard, select the Docker image which you have created in Step 3.2.
...
7. Click **Validate & Create**.
   ...
   This will create the version metadata in your workspace you selected earlier.
...
1. Create an empty folder, and run the following command inside that folder to initialize a workspace:
   ...
   ```
2. Connect the Industrial Edge App Publisher to an Industrial Edge Management:
   ```shell
...
3. List the project including its apps and fetch the 'applicationId' which will be needed for creating an app version:
   ...
   ```
4. Create a version of your Mendix app:
   ```shell
