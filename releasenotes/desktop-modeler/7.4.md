...
* Fixed native library support which caused runtime to crash when trying to load user native libraries that were already loaded (e.g. SQL Server authentication library). (Ticket 46846, 48052, 50498)
* Fixed an issue that causes an undesired autocommit of an NPE's reference. (Ticket 51186)
* We fixed an issue that left old thumbnails on the file storage when replacing an existing image.
* Fixed regression in Mendix 7.3 where 'max' and 'min' aggregate actions on empty list resulted in NullPointerException. (Ticket 52967)
* Fixed error while executing admin action get_logged_in_user_names. (Ticket 52147)
* Extended support for JPEG images with CMYK color space. (Fixes 40140)
