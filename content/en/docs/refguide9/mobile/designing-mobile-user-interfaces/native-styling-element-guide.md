...
The drop-down widget has a style property called `useUniformDesign: boolean` which enables the uniform design in both platforms.
...
| `validationMessage` | This has all TextStyle properties. | Styles the validation message.|
...
| `value`  | `placeholderTextColor: string` | If placeholder is selected, placeholderTextColor will be applied.|
| `valueContainer` | This has all ViewStyle properties and rippleColor | Styles the value button's container.|
| `valueContainerFocused` | Same properties as `valueContainer` | Overrides `valueContainer` styles if the dropdown box is focused.|
...
| `valueFocused`  | Same properties as `value` | Overrides `value` styles if the dropdown box is focused.|
