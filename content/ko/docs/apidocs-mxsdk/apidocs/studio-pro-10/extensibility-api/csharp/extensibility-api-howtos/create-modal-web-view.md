---
title: "C#을 사용하여 모달 대화 상자에 호스팅된 웹 뷰 만들기"
linktitle: "모달 웹 뷰"
...
이 사용 방법에서는 모달 대화 상자 안에 호스팅되는 새 웹 뷰를 만드는 방법과 새 메뉴 항목에서 모달을 여는 방법을 설명합니다.

이 사용 방법의 예제는 [이 GitHub 리포지토리](https://github.com/mendix/ExtensionAPI-Samples)에서 다운로드할 수 있습니다.

## 새 모달의 뷰 모델 추가하기

모달 웹 뷰의 라이프사이클과 메시징을 관리하기 위한 뷰 모델을 만드십시오.
...
작업을 위임하는 컨트롤러를 만들어 메뉴 항목에 관련 없는 비즈니스 로직이 포함되지 않도록 하십시오(예: 기본 설정 수행 및 웹 뷰 표시). 이 접근 방식은 권장되지만 필수는 아닙니다.

{{% alert color="info" %}}
`Height`와 `Width` 속성을 모두 지정하십시오. 그렇지 않으면 기본값 0,0이 됩니다.
{{% /alert %}}
...
`WebServerExtension` 라우트를 사용하여 확장 기능에 정적 웹 콘텐츠를 제공하십시오.
...
`MyMenuExtension.cs`의 내용을 아래 코드로 바꿔 대화 상자를 여는 메뉴 항목을 추가하십시오:
...
{{% alert type="info" %}}
이 예에서 대화 상자가 모델과 상호 작용해야 하는 경우 `currentApp` 매개변수가 필요합니다. 또한 `WebServerBaseUrl`은 매우 중요합니다. 기본 경로 없이는 웹 서버 확장에서 정의된 라우트로 이동할 수 없습니다.
{{% /alert %}} 
