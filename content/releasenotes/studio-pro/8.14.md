...
* When microflows are executed in a context without an authenticated user, i.e. as a background job or as a REST service which does not require authentication, debugging fails. An error message appears in the log and breakpoints would not be triggered.
