...
* The Native Dynamic Image widget can now show the default image as a fallback when there is no dynamic image available. *Please note*; for consistency all images (static and dynamic) are are now contained in a `View`. This might break your styling. This can be fixed by set the styling in the [container property](/refguide/native-styling-refguide#image).
