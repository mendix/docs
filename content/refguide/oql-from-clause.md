...
This constrains the specified entity in the `JOIN` component of the `FROM` clause. The constraint syntax is similar to that of the `WHERE` clause. Only the entities and `from` aliases from the current `JOIN` element and each previous one can be used in the constraint.
...
This constrains the specified entity in the `JOIN` component of the `FROM` clause. The constraint syntax is similar to that of the `WHERE` clause. Only the entities and `from` aliases of the current and preceding `JOIN` elements can be used in the constraint.
