...
{{% alert color="info" %}}
From Studio Pro 11.15, the module version is displayed next to the module name in the **App Explorer** tree. Regular modules show the version in brackets (for example, `[1.0.0]`). Marketplace modules that have an AppStore version set show it with an `mp` prefix (for example, `[mp 3.2.1]`).
{{% /alert %}}
