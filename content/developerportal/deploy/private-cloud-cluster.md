...
ImageNameTemplate is a go template that is used to generate the image name and tag and allows both use of OpenShift-style "repository per app" and ECR-style "tag per app". For example a value of imageNameTemplate may be `registry.example.com/mendix-apps/{{.Name}}-{{.Version}}-{{.UnixTimestamp}}` and an example of the generated image from the imageNameTemplate for the build will be `registry.example.com/mendix-apps/pgv9gw71-0.0.1.2-1640699175.392`

The imageNameTemplate is generated by mxpc-cli when you update the registry configuration. **The changes made in the imageNameTemplate is overwritten when you update the registry configuration.**

An example of the imageNameTemplate in operator configuration manifest is given below.
...
You can customize the registry imageNameTemplate in OperatorConfiguration with these available templates:
