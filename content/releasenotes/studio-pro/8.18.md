...
* The [Commit](/refguide/committing-objects) activity now includes **Find usages** results.
...
* We fixed an issue where certain operations on binaries were only possible within a sudo context.
    ...
    {{% /alert %}}
* We fixed an issue regarding the [Change list](/refguide8/change-list) activity: If a list is `SET` to itself, the contents are no longer cleared. (Ticket 109243)
...
* We fixed an issue where dynamic classes were not applied for [layout grid](/refguide8/layout-grid) rows and cells, [table](/refguide8/table) rows and cells, [scroll container](/refguide8/scroll-container) regions, and [data grid](/refguide8/data-grid) columns. We also fixed an issue where in some situations, these were also not applied to the container widget. (Tickets 111048, 111810, 112312)
...
* App services are no longer supported. Opening an app project that contains consumed or published app services will result in a consistency error.
