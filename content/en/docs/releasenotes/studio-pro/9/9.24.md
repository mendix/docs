...
* We fixed an issue in XPath constraint suggestions that gave incorrect suggestions when having whitespace characters after comparison operators (=,<,>,<=,=>,!=). (Ticket 181673, 181908)
...
* We fixed an issue in XPath constraint suggestions that gave incorrect suggestions when having multiple predicates (expressions between the square brackets).
...
* We updated the [Make It Native](/refguide9/mobile/getting-started-with-mobile/prerequisites/) app to make it compatible with this Studio Pro version. This means it is no longer compatible with older Studio Pro versions. We recommend upgrading your app to 9.24 or [building a custom developer app](/refguide9/mobile/distributing-mobile-apps/building-native-apps/how-to-devapps/).
