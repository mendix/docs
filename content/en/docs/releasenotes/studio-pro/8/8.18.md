...
weight: 82
#DEPRECATION REMINDER: 8.18.20 deprecations for 8.18.22 removal
---
...
{{% modelerdownloadlink "8.18.22" %}}

### Improvements

* We now show consumed OData services on the Data Hub Pane regardless if you are logged in or logged out.
* Downloading files with a custom extension is now only allowed for people with write-access to the file itself. (Ticket 167137)

### Fixes

* We upgraded OpenJDK to 11.0.16.1+1 in the Studio Pro installer. (Ticket 164386)
* We fixed an issue with loading the app in IE11. (Ticket 167520)
...
* We fixed an issue where a missing widget inside a pluggable widget container caused an error dialog in Studio Pro while running consistency checks. (Ticket 138573, 138582)
