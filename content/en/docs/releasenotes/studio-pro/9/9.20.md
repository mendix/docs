...
* We fixed an issue that occurred when trying to debug Swagger UI served as interactive REST documentation by Mendix Runtime. As the JavaScript and CSS files refer to a source map, Mendix Runtime now serves that source map as well. [**TW: HERE AND BELOW, DH API?**]
