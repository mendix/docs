...
* Applications in Mendix Cloud v3 that were running for more than 248 days started to use 100% CPU due to an unknown bug in either the Mendix Runtime, the JVM or a combination. A very small number of applications are impacted, as most applications are updated much more often, and every deployment restarts the application. However, this problem is causing performance issues for both the affected applications and our infrastructure. As a workaround we will automatically restart apps that have been running non-stop for 247 days between 01:00 and 07:00 local time of the cloud region. If this happens to an application you will see a message in the application log. 
