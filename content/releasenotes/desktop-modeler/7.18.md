...
* Unpacking and opening project packages (*.mpk* files) does not work when double-clicking in Windows Explorer. (Ticket 68263)
  * Workaround: Open the Desktop Modeler and import the project package via **File** > **Import Project Package**.
* In this version of the Mendix Modeler we introduced a means to prevent entity instances to be committed with invalid autonumber values (fix for ticket 63071). This happens when the user has no access rights to the AutoNumber attribute and the entity instance had to be sent to the client before its initial commit. This measure, however, caused the commit to fail in the runtime with the exception stating "Value of auto-number attribute 'AttributeName' not available, could be caused by insufficient access rights to serialize the mendix object to the client." This causes the Mendix application to fail for their end users. Therefore we will revisit the solution in an upcoming release.
