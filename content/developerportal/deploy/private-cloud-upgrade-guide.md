...
This procedure allows to upgrade to any supported (v1.9.0 and later) version of the Mendix for Private Cloud Operator.
...
Once you've installed a certain version of the Mendix Operator into any namespace in the cluster, you should not install older versions of the Mendix Operator into the same cluster, even into other namespaces.
{{% /alert %}}

If you're using your own private registry, follow the [Migrating to Your Own Registry](private-cloud-migrating) guide first
...
{{% alert type="info" %}}
Manual upgrade is an alternative to automatic upgrade mode. If you have already performed an automatic upgrade, you don't need to follow instructions in this section.
{{% /alert %}}
...
Stop the `mendix-operator` Deployment (replace `{namespace}` with the namespace where the Mendix Operator is deployed):

```shell
kubectl -n {namespace} scale deployment mendix-operator --replicas=0
```

If you've installed the cluster in Connected mode, stop the `mendix-agent` Deployment (replace `{namespace}` with the namespace where the Mendix Agent is deployed):
...
Run the following command to upgrade to dependency versions for the Mendix Operator:

```shell
kubectl -n {namespace} apply -f upgrade_manifest.yaml
```

Open the `upgrade_patches.txt` file and run commands the commands printed in that file.
The `upgrade_patches.txt` file contains a list of `kubectl` commands (or `oc` commands for OpenShift) that you will need to execute in a commandline terminal to complete the upgrade process.

{{% alert type="warning" %}}
The `upgrade_patches.txt` file uses Bash escaping rules. If you're using another terminal such as the Windows command prompt, you will need to adjust the commands so that they're compatible with your commandline terminal's escaping rules.
{{% /alert %}}
...
If you've installed the cluster in Connected mode, start the `mendix-agent` Deployment (replace `{namespace}` with the namespace where the Mendix Agent is deployed):
