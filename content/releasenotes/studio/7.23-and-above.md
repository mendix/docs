...
* You are now able to preview your app without publishing it. The **Preview** feature allows you to to verify your own changes and to see how your app will look like for different end-users. As it has a different environment as the published app, you can use the **Preview** to experiment with changes and test them. For more information, see [Previewing & Publishing Your App](/studio/publishing-app) in the *Mendix Studio Guide*.

  ![](attachments/preview.png)

* To improve the user experience , we added an ability to filter your data for pages and microflows. In microflows, you can add a filter to the **Retrieve** activity, and in the page editor, it is possible to filter data in a list view and in a data grid.  For more information, see [Data Filters](/studio/filters) in the *Mendix Studio Guide*.

* You can secure your app from Studio now. Before publishing your app, you need to enable security and you can restrict access to your app for different type of users. When you enable security, all the necessary settings are performed automatically for your app. For more information, see [Security, Roles & Permissions](/studio/settings-security) in the *Studio Guide* and [Model Changes When Security Is Enabled in Studio](/refguide/studio-security-enabled) in the *Studio Pro Guide*.

#### Improvements

* To avoid confusion, we cleared naming between variables and objects in the microflow editor. For example, a "variable" could be used in different ways: as a name for a list of objects or as an object itself. Such confusing naming is cleared. 
* To make the microflow editor more user friendly, we added more tooltips for properties. 

#### Fixes

* We fixed an issue where app publishing was not working in Studio.
