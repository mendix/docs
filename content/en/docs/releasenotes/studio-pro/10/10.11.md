...
* You can now configure your application to use Java 21 in the project settings.
...
We recommend that projects and marketplace modules upgrade to Java 21.

* Pass a render mode to the pluggable widgets api to indicate which mode the page editor is in.

### Improvements
    
* We've added a CanApplyJumpTo attribute to the System.Workflow entity that indicates whether it is allowed to apply a jump-to option to the workflow in its current state.
...
* We upgraded react-navigation to version 6.x.x and extended navigation transitions options for StudioPro.
* We have added support for Hermes engine.
* Studio Pro Version Selector has been visually overhauled and now supports dark and light modes.
* Studio Pro is now based on a .NET 8 instead of .NET 6. Therefore, .NET 8 is a new prerequisite for the application. This can be seen in the system requirements.
* With the new .NET 8 update, extension developers will need to upgrade their respective extensions to target the new version of the framework.
...
* We updated some phrases in the XPath editor for more clarity to the user.
* We made sure that on right click the context menu with cut, copy, paste, and delete options is also available on the MendixChat feedback page.
...
* We improved relayouting in the logic editors: When adding objects to a loop, it will only expand in the relevant direction(s)
* We have made a couple of small performance improvements to the Logic Bot.
* The menu items in the 'View' main menu of Studio Pro are now again ordered alphabetically, and they all have working keyboard accelerators assigned.
* We improved consistency checks for parameters in Consumed REST Request (beta) microflow action. Parameters with different casing are also marked as duplicate now.
* We added support for parameters in Path URL inside Consumed REST Service (beta) document
* We now support parameters for string body content in a Consumed REST service (beta).
* We improved the integration pane in Studio Pro. For associations between persistable and non-persistable entities where the non-persistable entity is the owner it now indicates that these are not supported. Before, these would appear supported, but using them would always result in a consistency error.
...
* We have improved interaction with loops in the microflow and nanoflow editors. When a loop is not selected, you can now drag a selection rectangle from inside the loop to select its containing elements. To move the loop, simply click the loop to select it and then start dragging it to start moving.
...
- We fixed an issue where Data grid 2 didn't show data in some columns when different columns were configured with an attribute path using the same association but different specialization or generalization entities. (Ticket 203959)
...
We fixed an exception that happened upon closing a non-version controlled app (after we previously opened a version controlled app & didn't restart studio-pro).
...
-  Fixed issue where disabling a Scheduled Event through Model Options in Sprintr wouldn't work. (Ticket 214392, 215703, 215843)
