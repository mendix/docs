...
- We changed the default setting of the Best Practice Bot to disabled by default. The user can change this setting in the current session, but the bot will be disabled again in the next startup of Studio Pro.
...
- We fixed an issue where the Merge bar doesn't appear after solution upgrade so it is impossible to commit without Studio Pro restart (ticket #206204)
...
- We fixed an issue where you wouldn't be redirected to the page you were initially trying to access after login in via a modeled login page (Tickets 205091 and 212002).
...
- We fixed an issue where importing an OData contract containing an action referencing an entity type as a parameter would cause an error.
