...
### Mendix Developer App functionality embedded in the Mendix App
...
* Client API `session.getUserRoles()` has been removed. That API was never supposed to be public or used externally, but unfortunately it was. Any usages should be changed to `session.getUserRoleNames()`, which is the new and officially public API.
...
* We fixed the issue when date tokens like `[%BeginOfCurrentWeek%]`were evaluated in client-side expressions but did not respect the [first day of the week](/refguide/project-settings#first-day-of-the-week) project setting.
