...
{{% alert color="info" %}}
Note: The **PUT** operation call executed more than once overwrites the details for all the published endpoints at the specified environment. If there's a collection of endpoints on the environment, it's possible to create, update, and delete different endpoints all in one **PUT** call.
{{% /alert %}}
