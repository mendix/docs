...
A data set can be used to define the data shown in [reporting widgets](/refguide6/report-widgets) in [pages](/refguide6/pages).

A data set is defined using either an [OQL query](/refguide6/oql) or a custom [Java action](/refguide6/java-actions). To constrain a data set parameters can be defined, which can be used in the OQL query or Java action.
...
*   _OQL query_: The [OQL query](/refguide6/oql) which defines the data set.
*   _Java action_: The interface of the Java action which returns a data set. The columns and [data types](/refguide6/data-types) of the columns needs to be specified in the modeler. Based on this specification the modeler will create a template for this action.
...
*   _Type_: The type of the parameter: Object, Enumeration or a primitive (e.g. DateTime, Float, Integer, Boolean, etc). See [Data Types](/refguide6/data-types) for the possible parameter types.
...
An XPath constraint can be defined using [XPath](/refguide6/xpath). Multiple constraints can be defined on a parameter and each constraint can be associated with a [user role](/refguide6/user-roles).
