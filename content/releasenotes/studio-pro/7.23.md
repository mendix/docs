...
* **KI fixed in this version: IN-3267 - Export mappings that use XML choice or XML inheritance throw exceptions during runtime. (Ticket 93998)"
* We fixed identifier reserver timeout error happening due to long garbage collector pauses (Ticket 88181)
* We introduced parameter `JdbcLoginTimeout` to limit database connection establishment time, with a default of 5 seconds (Tickets 88010, 92169, 93573)
* We now use network timeout in Mendix identifier reserver for PostgreSQL and DB2 instead of jvm-based timer. It can be disabled by setting `UseNetworkTimeout` parameter to `false`. (Tickets 88010, 92169, 93573)
