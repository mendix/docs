...
{{% alert type="warning" %}}
We are currently investigating an issue related to committing new objects with a one-to-one association that might result in data loss. For that reason, we advise you not to use versions 8.2.1 or 8.2.0 until further notice. We have disabled downloads of these versions until this investigation is resolved.
{{% /alert %}}
