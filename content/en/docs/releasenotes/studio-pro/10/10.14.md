...
- We have extended the error response of published OData services for action calls that contain parameters with invalid object references. It now includes a path property to indicate which node in a parameter's object tree caused the error.
...
* We corrected handling of commits with time offset in the **History** dialog box.
