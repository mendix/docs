...
`IMicroflowService.CreateMicroflow` 메서드는 `IModel.Create<IMicroflow>`를 사용하여 마이크로플로우를 수동으로 생성한 다음 `IFolderBase` 컨테이너에 수동으로 추가할 필요가 없습니다. 마이크로플로우를 생성하는 방법에 대한 포괄적인 예는 [C#을 사용하여 계산을 위한 마이크로플로우 만들기](/apidocs-mxsdk/apidocs/csharp-extensibility-api-11/create-microflows-for-calculations/)를 참조하십시오.
