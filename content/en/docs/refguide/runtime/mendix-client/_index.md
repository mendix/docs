...
1. If enabled, the Mendix Client checks if an updated bundle is available on the runtime. If it is, the updated bundle is downloaded and stored, and the app is restarted. This enables the app to keep up-to-date without needing to download new versions of the app from the app store. Please consider the limitations listed [here](/refguide/mobile/distributing-mobile-apps/overtheair-updates/#4-when-to-use-ota-updates) when updating native apps this way.
