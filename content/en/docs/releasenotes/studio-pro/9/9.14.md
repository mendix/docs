...
* We now do inline highlighting of the search terms in the **Go to** and image-selector dialog boxes to help you to find things more quickly.
* We added support for dark mode for the **Edit Building Block** and **Edit Page Template** dialog boxes.
...
* We introduced three new metrics — `mx.odata.publish.created`, `mx.odata.publish.updated` and `mx.odata.publish.deleted` — that count the number of objects created, updated, and deleted through published OData services.
...
* We added a confirmation pop-up window when moving a data view with footer widgets to a place where the footer is not allowed, preventing widgets from being silently deleted. This pop-up window can be set to not show again.
 ...
 * We deprecated the **Serbian (Serbia and Montenegro)** language with the code **sr_CS**. It is replaced with **Serbian (Serbia)**, code **sr_RS** and **Serbian (Montenegro)**, code **sr_ME**. (Ticket 80642)
...
**Review**
* We fixed the `MetaAssociation#hasMetaDataAccess` function in the **Java API class** to work for specializations of **System.User** as well. This might have an effect on which attributes can be shown in the UI. Whether the content of an attribute is readable has not changed. 
...
* We now block changes to the Password attribute of User and User-derived objects in client commits (for example the save button in forms). The old behaviour is deprecated but can be restored until Mendix 10 by setting the `BlockPasswordInClientCommit` custom setting to `false`.
* Pluggable widget data sources with a linked expression or text template property will no longer show updated values if an object is changed, unless a widget which requires full objects is linked to the same data source. To reload the data source and get the old behaviour, use "Refresh in client" in a microflow or "Refresh entity" (from the Nanoflow Commons module) in a nanoflow.
