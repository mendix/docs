---
title: "Runtime 사용자 정의"
url: /refguide8/custom-settings/
description: "Mendix의 서버, 로그 파일, 데이터베이스, Amazon S3 스토리지 서비스, IBM Cloud Object Storage, Microsoft Azure, IBM Bluemix 객체 스토리지, 웹 클라이언트, 프록시 서버에 대한 커스텀 설정을 설명합니다."
...
커스텀 서버 설정을 사용하여 Studio Pro에서 제공하는 표준 가능성 이상으로 Mendix Runtime을 구성할 수 있습니다.

{{% alert color="warning" %}}
정확히 무엇을 하는지 알고 있는 경우에만 이 기능을 사용하십시오. 잘못된 값은 Mendix Runtime이 시작되지 않을 수 있습니다.
{{% /alert %}}

각 커스텀 설정은 이름과 값으로 구성됩니다. 예를 들어, 영구 세션을 활성화하려면 이름이 `PersistentSessions`이고 값이 `true`인 커스텀 설정을 추가합니다. 설정 및 예시 값의 자세한 목록은 [full-documented-m2ee.yaml](https://github.com/mendix/m2ee-tools/blob/master/examples/full-documented-m2ee.yaml)을 참조하십시오.

Mendix Cloud에서 앱을 실행하는 경우, **Apps**에서 **Environments** > **Environment Details** > **Runtime** > **Custom Runtime Settings**를 통해 이러한 설정에 접근할 수 있습니다. 자세한 내용은 *Environment Details*의 [Runtime 탭](/developerportal/deploy/environments-details/#runtime-tab) 섹션을 참조하십시오.

SAP Cloud에서 실행하는 경우, `MXRUNTIME_` 접두사가 붙은 User-Provided Variables로 커스텀 설정을 추가할 수 있습니다. 설정에 점 `.`이 포함된 경우 변수에서 밑줄 `_`을 사용할 수 있습니다. [참조](https://github.com/mendix/cf-mendix-buildpack#custom-runtime-settings)
...
아래 설정은 로그 파일의 동작에 영향을 줍니다. 이러한 설정은 온프레미스에서만 사용할 수 있습니다. 클라우드에서는 이러한 설정이 동작을 변경하지 않습니다.
...
아래 설정은 데이터베이스 연결 풀링 동작을 정의하는 데 사용됩니다. Mendix Runtime은 재사용 가능한 데이터베이스 연결 풀을 사용합니다. 예를 들어, 사용할 수 있는 연결 수를 정의할 수 있습니다. 연결 풀링은 [Apache Commons Object-pooling API](https://commons.apache.org/pool/)를 사용하여 구현됩니다.
...
아래 설정은 모든 데이터를 주 데이터베이스로 복사해야 하는 소스 데이터베이스를 정의하는 데 사용됩니다. 아래 설정은 한 번만 지정하면 됩니다. 주 데이터베이스는 존재해야 하며 비어 있어야 합니다. 앱 시작 시 아래 설정이 지정되면 데이터가 복사됩니다. 더 이상 필요하지 않으므로 나중에 설정을 제거하십시오.

데이터 복사 프로세스가 시작되기 전에 주 데이터베이스처럼 소스 데이터베이스도 모델에 맞게 조정됩니다. 이는 문제 없이 모든 데이터를 복사할 수 있도록 하는 데 필요합니다.
...
## S3 스토리지 서비스 설정 {#amazon-s3-storage-service-settings}

아래 설정은 Amazon S3 Storage Service 모듈의 동작에 영향을 줍니다. 이 모듈은 Amazon S3 Storage와 IBM Cloud Object Storage 모두에 사용할 수 있습니다. Mendix Cloud에서 이러한 설정을 수동으로 사용하는 것은 강력히 권장하지 않습니다. 외부 시스템에 저장된 파일은 백업 생성 및 복원에 포함되지 않기 때문입니다.
...
이러한 설정을 변경하여 Mendix 애플리케이션에 Microsoft Azure SQL 데이터베이스를 사용할 수 있습니다.

먼저 Azure SQL 데이터베이스를 생성해야 합니다(이 방법에 대한 정보는 이 [SQL Database Tutorial](https://azure.microsoft.com/en-us/documentation/articles/sql-database-get-started/)을 참조하십시오). Azure 방화벽 설정이 Mendix 애플리케이션이 Azure SQL 데이터베이스에 도달할 수 있도록 허용하는지 확인하십시오(기본적으로 Azure 방화벽은 외부 연결을 허용하지 않습니다).
...
## Microsoft Azure Blob 스토리지 설정

이러한 설정을 사용하여 Microsoft Azure Blob 스토리지 서비스를 통해 파일을 저장할 수 있습니다. 서버 측 암호화는 Azure Portal을 통해 구성할 수 있습니다(자세한 내용은 [Azure Storage encryption for data at rest](https://azure.microsoft.com/en-us/documentation/articles/storage-service-encryption/) 참조).
...
{{% alert color="warning" %}}
Azure Blob 스토리지의 기본 연결 프로토콜은 기본적으로 안전한 연결을 장려하기 위해 HTTPS입니다. 이것은 매우 권장되는 모범 사례입니다(자세한 내용은 [Configure Azure Storage Connection Strings](https://docs.microsoft.com/en-us/azure/storage/common/storage-configure-connection-string) 참조). 커스텀 도메인 이름을 사용하지 않는 한 이제 투명해야 합니다(자세한 내용은 [Require Secure Transfer](https://docs.microsoft.com/en-us/azure/storage/common/storage-require-secure-transfer) 참조). 이 경우 위의 `UseHttps` 설정을 사용하여 이전 기본 동작으로 되돌리고 HTTPS를 비활성화해야 합니다.
{{% /alert %}}

## 웹 클라이언트 설정{#web-client-settings}

아래 설정은 Mendix 웹 클라이언트의 동작에 영향을 줍니다.
