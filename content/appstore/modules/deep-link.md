...
* **SSOHandlerLocation** – When a deep link is configured to support anonymous users, the SSO handler is requested before redirecting users to the destination
  ...
  * The original deep link location will be appended to the login location when the login location ends with `=` (for example, in the case of Mendix SSO: `https://login.mendix.com/oidp/login?ret=`)
