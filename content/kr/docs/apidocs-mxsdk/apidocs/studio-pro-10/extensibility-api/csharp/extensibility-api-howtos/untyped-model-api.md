---
title: "C#을 사용하여 Untyped Model Access API 사용"
...
Untyped Model Access API는 Mendix 플랫폼의 내부에 익숙한 고급 사용자를 위해 설계되었습니다. 이를 통해 모델 요소의 풍부한 데이터에 액세스할 수 있습니다.

* 모델 요소, 단위 및 속성 이름에 대해서는 [Mendix Model SDK](https://apidocs.rnd.mendix.com/modelsdk/latest/index.html)를 참조하십시오. 
* Untyped Model Access API에서 사용하는 유형 이름은 모든 모델 요소의 [structureTypeName](https://apidocs.rnd.mendix.com/modelsdk/latest/classes/Structure.html#structureTypeName) 속성에서 확인할 수 있습니다.

{{% alert color="info" %}}
Untyped Model API의 모든 메서드는 콘텐츠를 검색하는 데 필요한 API 호출 양을 줄이기 위해 재귀적입니다.
{{% /alert %}}
...
* 작업이 포함된 `MyFirstLogic`이라는 이름의 마이크로플로우를 만드십시오.
* 도메인 모델에 엔티티를 추가하십시오.

## 시작하기

Untyped Model Access API 서비스를 가져오는 것으로 시작합니다:
...
Untyped Model Access API를 사용하여 모델 `Root`에 액세스하십시오:
...
`Apps` 및 `Modules`와 같은 모델 요소에 액세스하려면 시작점을 선택하십시오:
...
마이크로플로우 작업이나 도메인 모델의 엔티티와 같은 모델 요소의 자식 요소에 액세스할 수 있습니다.

`GetElements` 또는 `GetElementsOfType`을 사용하여 요소 속성을 분석하고 사용자 지정 유효성 검사 규칙을 구현하십시오. 
...
모델 요소 또는 단위에서 데이터를 추출하려면 해당 속성에 액세스하십시오:
