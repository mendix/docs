...
We are adding an experimental feature to make working in **Design mode** easier: X-Ray Mode. This feature adds a special border to certain widgets, such as layout grids and containers. You can also change their sizes. This mode makes it easier to visualize the structure of your page.

When the X-Ray Mode experimental feature is enabled, the X-Ray Mode itself can be toggled on or off with a button the top bar. Turning X-Ray Mode off returns the page to the WYS/WYG editor of **Design mode**.

As **Structure mode** is not supported on Mac, we are enabling the X-Ray Mode feature on Mac by default. 

On Windows, it can also be enabled in the **New features** section of **Preferences**. You can opt out of the beta by disabling the feature in **Preferences**. {TW}

#### Other New Features

- We added a **Continue All** button to the debugger. When there are multiple paused microflows, nanoflows, and/or rules, they can all be continued with pressing a single button, instead of having to continue them one-by-one. {TW}
...
Token expiry is now controlled by the com.mendix.webui.HybridAppLoginTimeOut runtime setting, with a default expiration set to one year if left unspecified. {TW}

### Improvements

- We improved the XPath constraint Builder to format the XPath whenever a change is made.
- We improved the feedback shown to the user in the XPath constraint Builder when unsupported association or attribute references are used in the XPath expression editor.
- We improved the feedback shown to the user in the XPath constraint Builder when advanced token expressions are used in the XPath expression editor.
- We improved the order of the dropdown list items in the XPath constraint Builder to better match the autocomplete list in the XPath expression editor.
- We now support the copy, cut, paste, and delete actions in MendixChat through a right click context menu.
- We updated the fonts for MxChat, data filters and **Start from spreadsheet** to those native to the operating system it is currently running on. {TW}
- We extended logging during synchronization by tracking execution time of commands. Warnings are printed when a command takes a long time, which is controlled with the custom runtime setting `LogMinDurationQuery`. History of all commands is logged when an exception occurs during synchronization. {TW}

- For React Client, we improved the time it takes to bundle when rerunning an app.
...
- For React Client, we improved the building time by exporting nanoflows in parallel. Moreover, when rerunning an app, nanoflows are selectively exported based on changes.
...
- Now request body for OData Actions are marked as required in openapi only if it has at least one required parameter.
