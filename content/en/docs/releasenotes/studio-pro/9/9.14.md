...
* Have you ever been lost trying to find your widget package on a page in Studio Pro? We have got you covered! Say hello to the **Show widget in Explorer** button now available from the context menu. 
* We added the **On change behavior** property to the [text area](/refguide/text-area/) native widget, which lets you select how the on-change behavior is handled.
* We enabled support for on-change events for the association property in the [Pluggable Widgets API](/apidocs-mxsdk/apidocs/pluggable-widgets/). It is now possible to link an action property against an association property through the `onChange` attribute in the widget definition XML. 
