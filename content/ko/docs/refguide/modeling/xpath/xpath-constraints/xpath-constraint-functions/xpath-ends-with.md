...
`ends-with()` 함수는 문자열 Attribute가 특정 문자열로 끝나는지 여부를 확인합니다.
...
이 쿼리는 이름이 부분 문자열 `sen`으로 끝나는 모든 고객을 반환합니다:
...
예를 들어, 이름이 "Jansen" 또는 "Isaacsen"인 고객은 두 이름 모두 "sen"으로 끝나므로 반환됩니다.
