...
* We now support keyboard hotkeys to copy, cut, paste, undo, redo, and delete text in MendixChat (Windows only).
...
* We fixed an issue in the new expression editor when the cursor was jumping to the end of the expression upon typing. This cursor issue could also lead to unexpected behavior with IME characters. (Ticket 205514, 216915)
...
* We fixed an issue where disabling a Scheduled Event through **Model Options** in the Developer Portal did not work. (Tickets 214392, 215703, 215843)
...
* We fixed a regression where using Boolean microflow variables (or Boolean attributes from an object microflow variable) in an XPath constraint resulted in a runtime exception *Unexpected expression in predicate position*. (Ticket 216792)
* We fixed an issue when the new expression editor was getting stuck upon selecting a long suggestion in the auto-completion popup.
...
* We fixed the positioning issue of extracting sub-microflows in a loop in the logic editors.
