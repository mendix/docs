...
* [클라우드에서 Mendix 네이티브 앱 빌드](/howto8/mobile/deploying-native-app/)에 있는 Mendix Native Mobile Builder 마법사를 완료하십시오

## Developer App 빌드 {#build-your-developer-app}
...
1. Mendix Native Mobile이 시작되면 홈 화면이 표시됩니다:
...
1. *Build app for local development*를 선택하십시오
1. 초기 마법사를 적어도 한 번 완료했다면 *로컬 개발용 앱 빌드* 구성 화면이 표시됩니다: 
...
1. *Build developer app* 버튼을 클릭하십시오
1. 도구가 GitHub 저장소를 설정하고 변경 사항을 커밋하고, 필요한 경우 App Center를 iOS용과 Android용 두 개의 새 프로젝트로 구성한 다음 앱 빌드를 계속합니다.
...
Mendix Native Mobile Builder로의 빌드는 시뮬레이터 아티팩트가 제거됩니다. 따라서 Xcode의 Simulator에서 실행하려면 소스에서 developer 브랜치를 로컬로 빌드해야 합니다:
...
Native Builder가 사용자 지정 Developer App에 서명하게 하려면 *클라우드에서 Mendix 네이티브 앱 빌드 방법*의 [빌드 서명](/howto8/mobile/deploying-native-app/#signing-a-build) 섹션의 단계를 따르십시오.
