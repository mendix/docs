...
* Accesrules on entity attributes are not always parsed proparely to the UI, allowing users to edit fields they have no access to and resulting in an error when the user tries to commit the object. (#52930)
* In certain situations associations can be unexpectetly be set to empty when passed to the client and back to the server again. (#52974) 
* Conditional visibility on TextAreas does not work properly in all situations, which may result in the label appearing without the actual input field. (#53027)
