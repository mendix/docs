...
* For **App from a spreadsheet**, we fixed an issue where an entity and an enumeration had the same name and it resulted into consistency errors. Now when an entity and one or more enumerations share the same name, after the import the enumeration is named as "name_N". For example, if both the entity and enumeration are named *Customer,* the enumeration name will be *Customer_1*. If there is one more enumeration named *Customer*, it will be named *Customer_2*, etc. 

* We fixed an issue where you could still open the disabled project for Studio in Studio by copying the URL. 
