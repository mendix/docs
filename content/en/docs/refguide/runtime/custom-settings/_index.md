...
| **DatabaseUseIntegratedSecurity** | This setting defines whether integrated security will be used to authenticate to SQL Server. If true, user name and password will not be used. If the runtime is not running on Windows, it must be instructed to use JavaKerberos authentication: add `;integratedSecurity=true;authenticationScheme=JavaKerberos` to the **DatabaseJdbcUrl** (for versions < 9.23.0). In addition, ensure that the proper domain user for accessing the SQL Server is authenticated on the runtime server using the `kinit` command. | false |
