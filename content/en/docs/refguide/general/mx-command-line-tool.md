...
From version 9.17 onwards, optionally you can also specify the path to an exported Suppress Warnings file (JSON file).
This means that `mx check -w` will use the list of suppressed warnings in that JSON file, instead of the default behavior which is to read from the `project-settings.user.json` file within the app directory.
