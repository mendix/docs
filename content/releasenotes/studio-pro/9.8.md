...
The existing code used for this will not work anymore with the encryption improvements and therefore the code must be updated to make sure the module/solutions will work again. Previously it was possible to read or write the token using the AsyncStorage dependency (in a JS action). With the introduction of encryption for the enhanced security this is not possible anymore. Reading is fully disallowed, while writing is still possible with the following approach.
