...
| ClientCertificateUsages | Defines which service must use which client certificate. The value of this setting must be a comma-separated list of key/value items. A key/value item must be specified as [`"identifier": "path to certificate"`] without brackets. For web services, use the imported web service name as the identifier. For REST services, use the hostname of the remote server as the identifier. Please note that any backslash in the path must be doubled. The whole value must be enclosed by braces. Example: ![](attachments/Custom+Settings/code_snippet.png) |  |
