...
The **Execute callable statement** microflow action is used to execute stored procedures and functions in the database engine. In addition to the **JDBC Url**, **Username** and **Password**, this action also expects an input object of type `DatabaseConnector.Statement` which should define the contract to perform the execution and retrieve the results:
...
The **Content** attribute of the `DatabaseConnector.Statement` type should contain the statement body (i.e. the SQL content). An association with a `DatabaseConnector.Parameter` type is also available to define, if applicable, the input and output parameters that the stored procedure expects. 
...
For a type-safe representation of stored procedures' **IN**, **OUT** or **INOUT** parameters, the `DatabaseConnector.Parameter` type has pre-defined specializations that should be used.

##### a. Primitive Types
The `DatabaseConnector.ParameterDecimal`, `DatabaseConnector.ParameterLong`, `DatabaseConnector.ParameterDateTime` and `DatabaseConnector.ParameterString` can be used to refer to the SQL primitive types for decimals, natural numbers, dates and character types respectively.

![](attachments/database/primitive-types-parameters.png)

The **Value** attribute defined in these specializations will be handled differently depending on the parameter mode used. In case of input parameters, the attribute must hold the value to pass to the stored procedure. In case of output parameters, it will be set to the output from the stored procedure.


##### b. ParameterObject Type
Some database vendors support creating complex SQL object types, these can be referred to using the `DatabaseConnector.ParameterObject`. The **SQLTypeName** attribute should be set to the underlying SQL object type name.

![](attachments/database/parameter-object-parameter.png)

Attributes of the object can be represented by associated `DatabaseConnector.Parameter` objects using the `ParameterObject_Parameter` association. You can use any specialization of `DatabaseConnector.Parameter` for the associated parameter objects; this allows for flexibility defining nested object hierarchies as well, i.e. objects with attributes of type object.

{{% alert type="info" %}}
Attributes within an object are identified by their position in the object and not by their name. It is therefore necessary to set the **Position** attribute correctly for all object attributes.
{{% /alert %}}

##### c. ParameterList Type
...
The many-to-many-association with `DatabaseConnector.Parameter` is also used here for the same reasons mentioned in the previous section about lists parameters.
