...
The flag `canExecute` indicates if an action can be executed under the current conditions. This helps you customize your security settings by managing which user roles can execute microflows and nanoflows. For more information, see the [Module Security Reference Guide](/refguide/module-security). You can also employ this flag when using a **Call microflow** action triggering a microflow with a parameter. Such an action cannot be executed until a parameter object is available, for example when a parent Data view has finished loading. An attempt to `execute` an action that cannot be executed will have no effect except generating a debug-level warning message. 
