...
We added **Create object** to the available actions of the Action Button. This means that you have more possibilities to create new objects. For instance, you can create a new object and show a page from a menu item.

#### Show Page

We now support the **Pages per specialization** setting for the **Show a page** action of the Action button. When a context entity with specializations is available, this setting becomes visible and can be configured.
...
As you all know, data grids and template grids allow you to place a **Delete** button on the control bar, which deletes the selected objects. The Delete button has been freed from the control bar of these grids, and you can now put it in data views, list views, and template grids as an Action button.
...
In our effort to improve the quality of the Mendix Client, we have rewritten the Action button widget. During the rewrite of the Action button, we made a number of improvements:
...
* An import mapping in a REST operation that reports that it cannot find the object by the key will now result in an HTTP response 400 rather than 500.
...
* We fixed the issue that caused the View mode to crash when the page contained a list view widget with templates.
...
* We fixed the issue with displaying the debug variable of dates formatted as `yyyy-MM-dd'T'HH:mm:ss.SSS'Z'`. (Ticket 63317).
* We fixed the issue causing the `$current_user` variable in the microflow scope to be different from the **current user** object in the state. This led to a situation where changes made to the associations of the **current user** object in the state were not reflected on the `$current_user` object in the microflow scope. (Ticket 64582)
...
* We fixed the issue where the time zone drop-down menu showed incomplete information on the location and area. Now, the drop-down menu has the correct description of **(GMT-x) location/area** instead of only **(GMT-x) location**. (Ticket 65115)
...
* We fixed the issue in the import mappings where it was not possible to pass the parent object to the microflow to obtain the object. (Ticket 65708)
* We fixed the issue in export mappings where using a reserved namespace led to an error. Now, `http://www.w3.org/XML/1998/namespace` will always have the prefix `xml`, and `http://www.w3.org/2000/xmlns/` will always have the prefix `xmlns`. (Ticket 64512)
* We fixed the issue in import mappings that used **Find by key**, wherein if the key appeared after a property with the value null, the import mapping reported that the key could not be null. (Ticket 65836)
* We fixed the issue in the mappings where the properties tab did not show all the mapping sources. (Ticket 65585)
* We fixed the issue where a runtime error during an offline synchronization was not reported to the user. (Ticket 64709)
...
* We fixed the issue where microflows depending on the data grid selection did not send all the related state to the server. (Ticket 65114)
* We fixed the issue where `mx.data.get` with `filter.references` containing an entity generalized from `System.FileDocument` did not return all the satisfying results. (Ticket 64052)
* We fixed the issue where rapidly calling REST services that use cookies passed cookies back to the server in a subsequent request. (Ticket 66627)
* We fixed the issue where sorting on data grids led to an exception. (Ticket 65276)
* We fixed the issue where opening an entity that contained an index based on a system attribute resulted in an error. (Tickets 64349, 65679, 66003)
* We fixed the issue where creating two indexes on an attribute with different directions resulted in a model synchronization error. (Ticket 66758)
...
* We fixed the issue where adding multiple widgets in an empty layout placeholder (for example, in a building block or via copy-and-paste) resulted in duplicate widget names.
...
### Known Issues
* There is an issue when a New button is placed on a Data grid and configured to create a specialization of the grid entity. In this case, an instance of the grid entity will be created instead of the requested specialization. For example, when a grid has been configured to list `Animal`s and there is a new button which should create a new `Dog`, it will still create an `Animal`. (Ticket 67014)
* The `createXPathQuery` in the Core Java API doesn’t support adding variables at the moment. (Ticket 67019)
