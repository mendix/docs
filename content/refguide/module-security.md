...
For each combination you can indicate whether or not the page is visible for the module role.
You can also edit this information in a page using the Visible for property.
...
Page access settings do not restrict users from navigating to a page through other means, e.g. through a deeplink or through a button that is forced to be visible (see also the [common widget properties](common-widget-properties) documentation).
If you want to ensure that specific roles cannot access parts of your data and/or logic, than this must be expressed through e.g. **entity access** and **microflow access** constraints.
