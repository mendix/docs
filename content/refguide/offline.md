...
Synchronization consists of two steps: uploading new and changed objects and recreating the database by downloading all necessary objects from the runtime.
...
Because synchronization depends on the regular runtime APIs, the models of the app and the runtime should be compatible. This is important when deploying a new version of your app. For example, if you remove the `Brand` attribute of an offline visible entity `Car` someone using an old version of the offline app will get an error during synchronization if he changes the brand of a car. Therefore, as a rule of thumb, never remove, rename or change the type of an offline visible entity or its attributes.
