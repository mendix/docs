...
* We have added support for multi-instance user tasks. It is now possible to configure user tasks that require multiple approvals. These user tasks will be completed based on the completion criteria specified. Completion criteria can be **Consensus** or **Veto**.
...
* We added a column  to the [Open App](/refguide/open-app-dialog/ ) dialog box displaying the Mendix Studio Pro application **Version** (including for local apps that are stored on disk).
...
* We fixed the documentation links in Studio Pro to point to the correct version 10 pages.
...
* We fixed an issue where attribute widgets (for example, text box) did not trigger a consistency error if the selected attribute had an invalid source, causing an error when deploying the app. (Ticket 183379)
