...
icon 타입의 속성은 사용자가 [버튼](/refguide/button-properties/#icon)에서 사용되는 것과 유사한 아이콘을 구성할 수 있게 합니다. `DynamicValue<IconValue>` prop으로 클라이언트 컴포넌트에 전달됩니다. 자세한 내용은 *플러거블 위젯에 사용 가능한 Client API*의 [IconValue](/apidocs-mxsdk/apidocs/pluggable-widgets-client-apis/#icon-value) 섹션을 참조하세요.
