...
**Release date: June __th, 2022**

{{% modelerdownloadlink "9.12.3" %}}

### Fixes

* We fixed an issue in the _Headers from a microflow_ feature of consumed OData services. In runtime memory, the headers returned by this microflow are now marked as _deleted_ to make sure header values are not communicated to the client.
* We added a missing `OData-Version` header to the response of calls to the metadata document and the service document of Published OData 4 Services. This resolves an error that is thrown when importing the OData feed into SAP. (Ticket 145894)
* We fixed an issue where a conversion dialog would appear when loading an MPR even though the app was already converted. (Ticket 136910)
* We fixed an issue which caused an error during deployment if the _On Change_ action of a widget was configured as an _Open Page_ action with the _Close pages_ option set to something other than _None_. (Ticket 148391)
* We fixed an issue where self-referencing associations and associations with long names are truncated when domain model is exported as image (Ticket 105595)
* We fixed an issue where server could not determine correct time zone and it was causing issues with XPath date functions on some databases (Ticket 144700, 131707, 119712)
* We fixed an issue where layout arguments would overlap, making it difficult to add widgets to them. (Ticket #149524)
* We fixed an issue which could cause an error in the runtime and the client if objects associated to the resulting objects of a data source were filtered out because of security constraints. (Ticket 144498)
* We fixed an issue where .svn or .git files would be carried over to exported modules by blacklisting them
* We fixed an issue where scheduled events did not execute in a horizontally scaled environment. (Ticket 146867)
* We fixed an error that appeared when closing a dialog with the [X] button while changing a grid value. (Ticket 146714)
...
* We fixed an issue where Ok, Cancel AND Help buttons are constantly "shaking" in the Application Security dialog.
* We fixed an issue where dropdown is opening outside the parent dialog.
...
* We fixed the tab sequence in dialogs: Pressing 'Tab' now navigates between focusable controls as expected (from top to bottom, left to right), without unexpected jumps. (Ticket 147204)
