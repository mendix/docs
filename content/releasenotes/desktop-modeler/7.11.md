...
* In offline apps when using datasources with paths containing association(s), entities referred to by the association might not be sycnhronized if they are not referred anywhere else in the offline app. This can result in missing data. To resolve this issue you can place a hidden DataGrid listing the missing entity and refer to it in an offline page, to make sure that the entity is sycnchronized. (Tickets 61768, 61916)
