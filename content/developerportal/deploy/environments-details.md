...
* **USAGE_METRICS_EMAIL_FIELDS** – if your app has specialisations of the `System.User` entity to store users, then you should use this variable to point to the them. This will help us in identifying Internal and External users of your app. The value of this variable should be in the `Module.Entity.Attribute` format where `Module` is that module of your app which contains the `Entity` that is a specialisation of `System.User` and `Attribute` is the attribute that stores the email address of the user. If you have multiple specialisations of `System.User` then you can specify the values in a comma separated format. Something like `Module1.Entity1.Attribute1,Module2.Entity2.Attribute2,…,ModuleN.EntityN.AttributeN`. For example: `Administration.Account.Email,MendixSSO.MendixSSOUser.EmailAddress`.
