...
* In OQL, `Attribute = $Parameter` where the parameter is a collection containing only a `null`, will check that the attribute is `empty`. This now also works with the parameter on the left hand side.
