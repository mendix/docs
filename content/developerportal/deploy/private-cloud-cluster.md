...
Any configuration values from `runtimeMetricsConfiguration` can be overridden in an environment (`MendixApp` CR).
...
You can set the following metrics configuration attributes:

* `mode`: metrics mode, `native` or `compatibility`.
* `duration`: Prometheus scrape duration interval in ISO 8601 Duration format (e.g. 'PT1M'), should be aligned with your Prometheus configuration; can be left empty, which defaults to 1 minute (matching the default Prometheus scrape interval). This attribute only works when `mode` is set to `native`.
* `mxAgentConfig`: configuration for the [Java instrumentation agent](https://github.com/mendix/mx-agent); collects additional metrics such as microflow execution times; can be left empty to disable the instrumentation agent. This attribute only works when `mode` is set to `native`.
* `mxAgentInstrumentationConfig`: instrumentation configuration for the [Java instrumentation agent](https://github.com/mendix/mx-agent); collects additional metrics such as microflow execution times; can be left empty to use the default instrumentation config. This attribute only works when `mode` is set to `native`, and `mxAgentConfig` is not empty.
