...
* We fixed the issue where selecting multiple rows at once from a reference set selector did not update the selector's grid properly. (Ticket 58626)
...
* We fixed the issue where synchronization failed when creating a new attribute with a unique constraint for an entity, while also changing one of the system members. (Ticket 59926)
