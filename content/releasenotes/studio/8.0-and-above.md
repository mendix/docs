...
*  Now you can create an association just by drawing a line between two entities: simply drag a dot from one entity to another, and an arrow (association) between two entities will be created automatically. This will make the process of creating associations faster. 
	![](attachments/8.0/drawing-an-association.png)
	
*  We added **Aggregate Function** on a data grid. With this property, you can calculate a sum, count, average, minimum, or maximum value that will be calculated and shown at the bottom of the data grid column. **Aggregate Function** is only available for columns with a numeric attribute, that is an attribute that represents quantity.  
    ![](attachments/8.0/aggregate-function.png)
    
#### Fixes

* We fixed an issue where a full-screen error was showed when the loading or reloading of Studio took too much time.
