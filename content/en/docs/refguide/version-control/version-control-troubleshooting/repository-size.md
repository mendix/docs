...
There are several reasons why your repository may be large. The most common reasons for a Mendix app to have a large repository are the following:
...
The *.mpr* storage format will be changed in H1 2024 to reduce the rapid repository growth. This will reduce the repository size for new apps, but will not reduce the repository size for existing apps. Upgrading to a new storage format will happen automatically and will not affect be noticeable for users in terms of functionality.
...
#### 5.1.6 Pushing Your Results to the Server
