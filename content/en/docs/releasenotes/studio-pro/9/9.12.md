...
We added modularity capability to the platform. [ISV program](https://www.mendix.com/marketplace-vendor-program) members and [private beta partners](/releasenotes/beta-features/) are able to create new types of modules which have IP protection options and offer different distribution mechanisms that allow for easier upgrades management on consumer side. For more information, see [Modules](/refguide/modules/) and [Creating Content](/appstore/creating-content/). 

The following is introduced for the new capability:
* Modules on the publisher side now have a new **Settings** option where module type and version can be managed.
* We added a new module package format *.mxmodule* for add-on modules and solution modules.
* We added a new **Export Level** setting to documents to define access level to the document on consumer (customer) side. 
