...
* Configure `GET`, `POST`, `PATCH` and `DELETE` requests
...
* This feature supports the following HTTP request methods:
  ...
  {{% alert color="info" %}}`DELETE` methods are only supported for [Mendix Studio Pro 10.9](/releasenotes/studio-pro/10.9/) and above.{{% /alert %}} 
...
Use the Consumed REST service to configure a `GET`, `POST`, `PATCH` or `DELETE` request for your app. 

### 3.1 Basic Configuration {#configure-a-request}

Create a `GET`, `POST`, `PATCH` or `DELETE` request to send data to your server by doing the following:
