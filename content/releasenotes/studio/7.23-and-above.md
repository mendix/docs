...
* In the page editor, you can now use the **Drop Down** widget as input widget for selecting both enumerations as well as associations. For the latter, the Reference Selector widget was required before. Now, the Drop Down widget can be used in all cases. This improvement, based on user feedback, makes building data input pages easier and faster.
* We made the following improvements for **App from a spreadsheet**:
  * Users will now be able to horizontally scroll tab pages in the **Data Import Preview** screen in case a spreadsheet with many sheets was selected as starting point
  * We increased the maximum size of spreadsheets that can be imported in Studio to 500 KB

#### Fixes

* For apps started from a spreadsheet, we fixed the issue that importing data was failing in case of very long header names in the spreadsheet
* For the **Checks** panel, the following issues were fixed:
  * Selecting an error would not scroll the page editor or microflow editor to the related widget or activity, but result in an error popup
  * When previewing or publishing the app, the Checks panel didn't display anything the first time consistency errors were reported
...
* On pages, you can now quickly configure the most important properties of many widgets. Simply drag and drop a widget on a page and configure properties in a pop-up window. This feature not only saves your time but also helps you avoid consistency errors.  For more information, the [Quick Configuration of Widget Properties](/studio/page-editor-widgets#quick-config) section in Widgets. 

	![](attachments/quick-config.gif)	

#### Improvements

* We made the following improvement for **App from a spreadsheet** (for more information on how to create an app with your own data, see  [Starting With Your Own Data](/studio/start-with-data)):
  ...
  * When you import an empty spreadsheet, you can now try again and import another spreadsheet. 
  ...
  For more information on how to create an app with your own data, see  [Starting With Your Own Data](/studio/start-with-data).
