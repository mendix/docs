...
3. The service definition—the **metadata contract**— of these services are published according to the OData standard that includes the information about the datasets such as the **entity sets**,  **attributes** and any **associations** and provides the necessary location of the datasets for access by **consumers**.
4. The developer usually assigns a **version number** to each service that is deployed. Services from an app are usually deployed to the **environments** that the app is deployed to. The different environments that the services are deployed usually indicate the quality or purpose of the data: **non-production** environments will have test data, **production** environments will have the live data.
5. There are several ways to register the services in the Catalog:
   ...
   2. For Mendix apps deployed to a non-Mendix cloud and for other business applications, a registration "pipeline" can be established using the **Data Hub APIs** to ensure that data sources published by apps are registered.
   3. For other business applications: the **Connectors** on the Data Hub **Home** screen can be used to register data sources.
6. Once registered, the services and the constituent datasets, can be further In Data Hub, the developer who is the **technical owner** or an assigned **curator** **curates** the registered assets. Curation includes adding further information to the **metadata** such as  **tags** that indicate the relevancy of the dataset. They may also indicate, for example, that datasets are the "official", "reliable" datasets for this kind of data by setting the sources as **validated**, and identify the source app that the data sources originated from by adding a customized logo.
