...
When you click a row in the list on the [Active](#active) or [Archived](#archived) tab of the [Survey Overview](#survey-overview) page, the survey details page opens.
...
The **Setting** tab is available for all the mini surveys. It gives an overview of all the survey details. 
...
On this tab, you can find instructions on how to implement the mini survey in Studio Pro. For a more step-by-step procedure, see the [Implement the Mini Survey in Studio Pro](#implement-survey) section.
...
5. After you fill in all the information, you can perform one of the following actions by clicking the corresponding button at the bottom of the page:
   ...
   * **Mark as Ready** – If you click **Mark as Ready**, the survey is finalized and it is [ready to implement](#survey-status). The [survey details](#survey-details) page opens. Save the **Survey ID** because you need it when you implement the mini survey in Studio Pro. Then you can proceed to implement [implement the mini survey in Studio Pro](#implement-survey)

### 4.3 Editing a Mini Survey {#edit-survey}

To edit a mini survey, go to the [Settings](#survey-details-settings) tab of its [survey details](#survey-details) page. Depending on the [status](#status) of the mini survey, you can edit different sections:
...
* If a mini survey has the status of **Ready to Implement**, **Running**, or **Evaluate Responses**, you can only edit the **Settings** section (**Runtime** and **Toaster Placement**) in the survey. To edit the **Setting** section, click **Edit Settings** in the **Settings** section, and then make changes.
...
8. Set up the **TriggerSurvey** nanoflow as follows:
   ...
   1. Double-click **Create String variable**, fill in the survey ID of the mini survey, and then save the changes.
      ...
      {{% alert color="info" %}}You can find the survey ID on the [survey details](#survey-details) page in Product Insights in Developer Portal.{{% /alert %}}

   2. Double-click **CHANGE ME**, add your microflow or nanoflow action there, and then save the changes.

10. Create a button that calls the **TriggerSurvey** nanoflow on the page where **Template_SurveyWidget** is placed. We recommend renaming the flow to something that describes the original action.

11. In **App Explorer**, double-click **Security**, and add the **ProductInsights_Survey.Use**r user role to all the system roles which might use the mini survey in the **App Security** dialog box.

## 5 Viewing and Exporting Responses
