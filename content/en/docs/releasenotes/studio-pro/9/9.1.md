...
* <a id="2048"></a>We fixed an issue where updating the local branch of an app that had [collaborative development](/refguide9/collaborative-development/) enabled failed if an editor pane (for example, the domain model editor) was open.
...
* When Studio Pro is displayed on a secondary screen, it is currently not possible to select an entity from the [Integration pane](/refguide9/integration-pane/) and drag it to the domain model editor.
