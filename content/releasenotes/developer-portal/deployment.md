...
* We now only monitor the selected Endpoint type (OpenShift Routes or Ingess), no longer resulting in an error message when starting the Operator in a non-OpenShift cluster
* We've added several features which make possible to use [cert-manager](https://cert-manager.io/) with Ingress - to have TLS certificates generated on-demand
* We added an option to specify an environment's TLS certificate, or load it from a Kubernetes secret
* When TLS is enabled, Mendix for Private Cloud Portal will now display the App URL with an https:// schema
