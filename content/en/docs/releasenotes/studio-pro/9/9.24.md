...
* We fixed an issue where executing a DeepLink microflow with a close page event navigated you outside of the app. (Ticket 205701)
* We fixed the initialization of hybrid apps on iOS. (Ticket 213273)
* We fixed an issue in the OpenApi 2.0 (swagger.json) document for published REST services, where operations that return an object using an export mapping based on an entity message definition had and `xml` property in the response schema that should not have been there.
