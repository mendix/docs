...
`ListValue`는 [datasource](/apidocs-mxsdk/apidocs/pluggable-widgets-property-types-9/#datasource) 속성에 대한 객체 목록을 표현하는 데 사용됩니다. 해당 리스트 항목 값은 [datasource](/apidocs-mxsdk/apidocs/pluggable-widgets-property-types-9/#datasource) 속성에 연결된 다양한 타입의 속성을 나타냅니다.

## ListValue {#listvalue}

[`datasource`](/apidocs-mxsdk/apidocs/pluggable-widgets-property-types-9/#datasource) 속성이 `isList="true"`로 위젯에 구성되면, 클라이언트 컴포넌트는 `ListValue`로 표현된 객체 목록을 받습니다. 이 타입은 데이터 소스에 대한 세부적인 접근과 제어를 허용합니다.
...
### 다시 로드 {#listvalue-reload}

데이터 소스의 항목을 다시 로드할 수 있습니다. `reload()` 메서드는 기존 `filter`, `offset`, `limit`, `requestTotalCount`, `sortOrder` 속성을 유지하면서 기본 데이터 소스에서 새로 가져오기를 트리거합니다. `reload()` 메서드는 인수를 허용하지 않습니다.
...
## 필터 헬퍼{#filter-helpers}

### 값 헬퍼 {#filter-value-helpers}
