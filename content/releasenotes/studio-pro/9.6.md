...
**Release date: December __nd, 2021**
...
* We fixed an issue where a reference selector which sets a reference to an uncommitted object, this object could not be retrieved over association in a microflow. (Ticket 131593, 133345, 134576, 135463)
* We fixed three issues in the generated metadata of published OData services in Studio Pro. Due to these changes, previous Studio Pro versions and versions 9.7.x and 9.8.0 are not able to read metadata produced by this Studio Pro version. The runtime is unaffected. When updating the contract of a consumed OData service we advise is to upgrate to Studio Pro 9.6.4, 9.8.1 or 9.9.0 or later. The three fixes are:
   * The NonUpdatable(Navigation)Properties are now specified as a collection of (Navigation)PropertyPath, and not as a collection of String.
   * The UpdateMethod is now specified as an enum value, and not as an Int.
   * The SupportedFormats is now a collection of String, and not a simple String.
* We fixed an issue when reading the metadata of a consumed OData service in Studio Pro, where (navigation)properties marked as non-updatable in the metadata would appear to be updatable.
* We fixed an issue where the [Auto load more](https://marketplace.mendix.com/link/component/50323) widget wouldn't work as expected together with a list view. (Ticket 132953)
* We fixed an issue in Published OData while generating metadata XML. In the `EntityContainer` section the `Name` property, now, can not contain `/`(slash).
* When the user opens a project from a network path or from a drive in a Mac (both of which will contain double backward slashes in the beginning of the file path), a warning is shown to tell the user about potential issues that might cause and asks if he wants to proceed anyway.
* Fixed a bug with some dialog boxes increasing in height after any edit the user does on the dialog itself, if the project if version controlled
