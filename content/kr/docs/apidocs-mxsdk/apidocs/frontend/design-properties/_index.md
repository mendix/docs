...
{{% alert color="info" %}}
이 문서는 Mendix 11에 해당합니다. 다른 Design Properties API 문서에 대해서는 이 카테고리의 다른 문서를 참조하십시오:
...
위의 예에서 볼 수 있듯이 공통 필드는 다음 표에 정의된 대로 모든 디자인 속성에서 사용할 수 있습니다.

| 필드          | 필수 | 유형   | 설명                                                                                   |
| -------------- | -------- | ------ | --------------------------------------------------------------------------------------------- |
| `name`         | 예      | String | UI에 표시되는 디자인 속성의 이름 및 식별자입니다.            |
| `description`  | 예      | String | 디자인 속성과 그것이 제공하는 것에 대한 짧은 설명입니다.                              |
| `type`         | 예      | String | 속성의 유형을 정의합니다.                                                              |
| `category`     | 아니요       | String | UI에서 디자인 속성을 그룹화하는 데 사용될 식별자입니다(기본값은 **General**). |
...
다음 디자인 속성 제한 사항에 유의하십시오:

* `category`는 **Common**이 아니어야 합니다. 이는 예약된 카테고리이기 때문입니다. `category`를 **Common**으로 설정하면 Studio Pro에서 경고가 발생하고 해당 디자인 속성이 기본 카테고리 내에 표시됩니다.
* `type` 속성은 `Toggle`, `Dropdown`, `Colorpicker`, `ToggleButtonGroup` 또는 `Spacing` 디자인 속성 유형 중 하나여야 합니다.
