...
### Version control: pre-check of the size of files being commited

We have introduced a new step during commit process that checks for large files that are being added or changed.  We want users to be informed about them as large files can become problematic for git.
