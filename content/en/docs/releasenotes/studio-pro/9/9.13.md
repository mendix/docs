...
#KI: "Scheduled events" - RTI-579 (9.14)
#KI: "When you update a consumed OData service" - DTML-358
# FIXED
#KI: "An error occurs" - SPVC-1515 - 9.13 - NEED NOTE
#KI: "When you double-click" - CTRL-2047 - 9.13, 9.12.1, 9.12.0 ? - NEED NOTE
---
...
* An error occurs when you import translations from an Excel file for batch translate or batch replace operations.
	* Workaround: Add an empty sheet to your Excel file and put it first (so the translations sheet is second).
...
* In published OData services you can now allow clients to [delete objects](/modeling/integration/published-odata-services/published-odata-resource/#deleting-objects). This is off by default. When this feature is switched on, entity access applies, so only clients with the right credentials that are allowed to delete.
...
* We changed the paths format in the *package.xml* file to Unix style. Now, the packages exported using command line tools on Linux and Windows have the uniform paths.
* We improved the MxAssist Logic Bot recommendation of Delete so that the AI filters by object type and sets refresh and error-handling. We improved the text for generic menu items, and variables are now ordered by last used, not just last created. Variables in use down-stream are not included (to avoid errors in the microflow). [**TW - ADD CROSS-REFERENCE**]
...
* For published OData services, we added a tab to edit the public documentation (summary and description). These fields used to be available via the properties tab only, which made them easy to miss. [**TW**]
...
* The task queue now supports a cluster-wide thread limitation per task queue, which allows you to limit parallel executions regardless of your cluster setup. A new option on the task queue creation form has been added to specify the scope of the thread count. [**TW**]
...
* We fixed a known issue that prevented selecting a line in multi-column trees when double-clicking a column that was not the first.
...
* We fixed an issue in the data view widget where it threw an error when nanoflow selected as the data source returned an empty object. (Ticket 138186)
...
* We fixed a NullPointerException during startup that occurred after renaming a module with a remote entity that was created before Studio Pro version [9.6.3](/releasenotes/studio-pro/9.6/#963). (Ticket 142150) [**TW - check "remote" vs. "external" entity, also for 9.12.1, 9.10.4, 9.6.11**]
...
* We fixed the issue where `SELECT *` from a subquery always returned only the first column of the subquery. Please note these improvements currently work with a single relation (a table or subquery) in the `from` clause.
...
* Scheduled events are not triggered on horizontally-scaled environments.
	* Workaround: Redeploy your app with the [buildpack](/refguide/runtime-deployment/) release of [4.27.4](https://github.com/mendix/cf-mendix-buildpack/releases/tag/v4.27.4), which fixes this issue.
