...
The Mendix Platform is good for building all types of apps and integrating them virtually any other technology. This section provides an overview of how Mendix integrates with other systems and technologies, using clear contracts as is prescribed for microservice architectures.

## 2 Mendix Integration Characteristics

### 2.1 Internal Integration Consistency Checked
...
For almost any business use-cases there will be less alignment between development teams and less integration testing using Mendix than with other technology choices, providing the low-code advantage of speed, security and flexibility.

### 2.2 External Integration by Contract {#external-contract}

Microservice architectures prescribe clear contracts for integration between components, which is the way Mendix operates. All external integration occurs via the app's runtime server, and requires clear contracts defined in the Mendix app model to allow for integration. The app model is aware of all external integration contracts before deployment into production.

If, for example, Mendix allowed direct database connections to the internal DB using SQL defined outside the Mendix app model, it would mean a risk to the functioning of the app in production. Testing would be unable to predict all possible interactions. Therefore, in the [Mendix Cloud](/developerportal/deploy/mendix-cloud-deploy), all connections to the Mendix internal database and file store have to use an agreed contract executed by the Mendix app server.

![](attachments/mendix-integration/runtime.png)

The diagram shows the following points:
...
* **OData** – This is a newer format already supported by the Mendix Platform. The transport is also REST, but instead of JSON, there is a direct OData contract from the endpoint to the domain model, often used with BI, SAP and between Mendix apps.
...
Alternatively when investing in this legacy intergration, there could be a Mendix adapter or "Integration App" thet provides other apps and systems with required access to old systems using modern protocols such as REST or OData. If the legacy system is om premises this adapter can also be installed on premises, while the other apps are on the Mendix Cloud or VPC.
...
<<Updated with the new chapters>>
* 2 REST to Call Other Systems
* 3 REST Pull to Transfer Data
* 4 OData Retrieve
* 5 Database Integration
* 6 SOAP Integration
* 7 RPC Integration & Adapters
* 8 Queue Integration
