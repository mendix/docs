...
### Fixes
* We fixed an issue that prevents the Mendix Runtime from starting if a client or server certificate is provided using the **CACertificates** or **ClientCertificates** [custom settings](/refguide/custom-settings)s (Tickets 109744, 110215, 110312, 110351, 110465, 110487, 110489).
* We fixed that when running an app project that contains a `FileDocument` created in Developer Portal version 5 or below, an error occurs in the `NodeManagementAction` that shows as the root cause that `System.TestFileDocument` is not a valid entity type (Ticket 110317).
...
* The Mendix Runtime cannot start if a client or server certificate is provided using the **CACertificates** or **ClientCertificates** [custom settings](/refguide/custom-settings).
	* Fixed in [8.15.2](#8152).
