...
**Release date: January __th, 2022**

{{% modelerdownloadlink "9.6.7" %}}

### Fixes

* We fixed an issue in Logic Bot when microflow call action cannot be inserted if exposed microflow presented in a project.
* We fixed a backward compatibility issue when a native application with a version prior to 9.4.0 is not starting up when connecting to a runtime with 9.4.0 or a later version.
* We fixed a bug with the auto-synchronize option in online profiles, that would crash the app when the action is used. (Ticket 138115)
...
* We fixed an issue where changes to an app theme with many files would no longer be automatically reloaded in the client. (Ticket #137980)
* We fixed an issue where Studio Pro would become unresponsive if compiling the app's theme never finished
* We fixed a bug where when a list of an Entity is sorted on a attribute of its generalization in a Nanoflow, deploying the project would wrongly error. (Ticket 138310)
* We fixed a bug where deployment fails when a List operation activity with sort operation is used in a nanoflow. This issue happens when the list is sorted based on an attribute that is defined on one of the generalisations of the sorted entity. (Ticket 138310)
* We fixed an issue with native mobile apps that could prevent an app from starting when a user has unsynchronized changes and was implicitely logged out. In such a case it was possible that startup synchronization could fail to complete due to user access rights with no actionable mitigation path.
...
* We fixed an issue where re-rendering of a [widgets property](https://docs.mendix.com/apidocs-mxsdk/apidocs/pluggable-widgets-property-types#widgets) linked to a [datasource](https://docs.mendix.com/apidocs-mxsdk/apidocs/pluggable-widgets-property-types#datasource) of a pluggable widget would no longer render its contents properly. (Tickets 132842, 134061)
...
* When accessing contents of file documents access rules on the Contents attribute are now checked instead of relying entity access alone.
