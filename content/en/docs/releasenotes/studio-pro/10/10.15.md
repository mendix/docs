...
X-Ray mode is generally available now for both Windows and macOS and is enabled by default. X-Ray mode is a way to better visualize the structure of your page while in Design Mode. It adds a special border around certain widgets, like layout grids and containers, while keeping most of your apps styling the same. We also added data source information to data widgets in this release. For more information, see [X-Ray Mode](/refguide/page/#x-ray-mode).

#### Other New Features

- We added new items into **Support Tools** under the **Help** menu option. For more information on **Project Context** and **Background**, see [Profiling](/refguide/profiling/). {TW}

- We added new options to **Help** menu >**Support Tools**:
  - **Collect version control data** – This option helps you collect basic version control data kept in the logs of Studio Pro.
  - **Profiling** > **Capture Memory Snapshot** – This option captures a snapshot of memory consumed by Studio Pro. The snapshot is written to disk, which you can then send to the Mendix Support teams.
...
- We optimized the performance of the Properties Tool Window when editing pages, enhancing responsiveness. {TW}
...
* We upgraded the JDBC driver for Microsoft SQL Server to version 12.8.1.
    * If you connect to SQL Server using integrated security in a module such as [Database Connector](https://marketplace.mendix.com/link/component/2888), you may need to update the JDBC driver in `userlib` as well. {TW}
- We added hyperlinks for collecting feedback on published-GraphQL services via survey. {TW}
- We now clean up anonymous users after a REST call finishes.
- The **Commit** dialog box now always displays all the tabs. The tab titles indicate the number of changes, and the default focus is set to the tab with more than zero changes. {TW}
...
- We fixed an issue where a data validation error triggered on a rollback or a delete. We now remove data validation errors for objects that are rolled back or deleted, also data validation that are manually added in a microflow. (Ticket 192090) {TW}
...
- We fixed an issue when trying to filter by an attribute of an associated entity when the association is self-referencing many-to-one (Ticket 224145) {TW}
...
- We fixed an issue where building failed when an app that was created with Studio Pro 9.24.0 or below containing jar exclusions is opened with newer versions. (Ticket 227102) {TW}
...
- Currently, boundary events are scheduled when the workflow activity with the boundary event is started. For user tasks this means that scheduling will happen before the targeting microflow finished running, which has implications on the timing of the **UserTaskStarted** event.
