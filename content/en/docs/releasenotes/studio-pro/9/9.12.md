...
weight: 88
#KI: "When you navigate between dockable panes" - SPX-2661 - fixed only in 9.21
#DEPRECATION REMINDER: 9.12.7 deprecation for 9.12.9 removal
---
...
* Starting with version 9.12.11, we will drop support for MariaDB 10.2 because it is no longer supported by the vendor
...
* We dropped support for SQL Server 2017 due to it no longer being supported by the vendor
...
* There is a known issue for the scenario where there are multiple instances of the same identical object (meaning, with the same ID) assigned to different microflow variables, this object has a one-to-one association to another entity, and no changes have been made to the associated object. When you then set the association to the same value, only a single microflow variable points correctly to the new association, and the other variables have an empty association. (Ticket 168024)
    * Fixed in [9.12.9](#168024), 9.18.4, and 9.22.0. 
...
* Starting with version 9.12.9, we will drop support for SQL Server 2017, which is no longer supported by the vendor.
...
* Starting with version 9.12.8, we will drop support for PostgreSQL 10, which is no longer supported by the vendor.
...
* <a name="ki-163193"></a>When you navigate between dockable panes using <kbd>Ctrl</kbd> + <kbd>Tab</kbd> while in the microflow editor, page editor, or **Properties** pane, releasing <kbd>Ctrl</kbd> does not close the pane-selection dialog box. (Ticket 163193)
    * Fixed in [9.21.0](/releasenotes/studio-pro/9.21/#163193).
* There is a known issue for the scenario where there are multiple instances of the same identical object (meaning, with the same ID) assigned to different microflow variables, this object has a one-to-one association to another entity, and no changes have been made to the associated object. When you then set the association to the same value, only a single microflow variable points correctly to the new association, and the other variables have an empty association. (Ticket 168024)
    * Fixed in [9.12.9](#168024), 9.18.4, and 9.22.0. 
