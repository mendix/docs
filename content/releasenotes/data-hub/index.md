...
The [Data Hub Transform API](/apidocs-mxsdk/apidocs/data-hub-apis#transform) is available for Mendix users deploying their apps to non-Mendix environments.  The Transform API extracts the information from the the app’s dependencies.json file to generate the request bodies that are required when using the [Data Hub API](/apidocs-mxsdk/apidocs/data-hub-apis).
