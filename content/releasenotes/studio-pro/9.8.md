...
#### Changes in how authtoken can be accessed in native mobile

Even though the token mechanism is an internal mechanism, for some solutions like SSO/Authentication modules this token was manually set.
The existing code used for this will not work anymore with the encryption improvements and therefore the code must be updated to make sure the module/solutions will work again. If authentication is broken after upgrading to 9.8, please make sure your modules are up to date.

Previously it was possible to read or write the token using the AsyncStorage dependency (in a JS action). With the introduction of encryption for the enhanced security this is not possible anymore. Reading is fully disallowed, while writing is still possible with the following approach.
...
Any native app built with Mendix Studio Pro 9.8 and that is using the `setValue` method provided by the new auth token utility, supports encrypted and unencrypted tokens alike with no extra modifications.

For modules or JS actions that require to be backwards compatible with versions of Mendix Studio Pro prior to 9.8, use the following snippet to ensure you are getting the correct setter based on the availability of the auth token util:
