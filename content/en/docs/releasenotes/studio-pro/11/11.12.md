...
* We removed the **Other** option for single and multiple-choice questions in Maia because all choice questions already have custom free-text responses, making the **Other** option redundant.
...
* We improved the message shown during version control operations when Git is not installed in the system.
...
* We fixed a performance issue when filtering over an association with multiple selected values, such as when using a drop-down filter on a Data Grid 2. (Ticket 284484)
...
* We fixed an issue where the **Properties** pane stayed empty when an import mapping or an export mapping document was selected. (Ticket 286678)
* We fixed an issue where Strict mode did not work correctly with certain Client APIs and nanoflow actions in offline apps. (Tickets 287100, 282233)
* We fixed an issue where canceling dialogs triggered an error check.
...
* We fixed an issue in the microflow editor where the loop was selected when trying to select elements inside the loop.
* We fixed the *Variable name in use* error that appeared in the Microflow and Nanoflow editors when closing the dialog box after double-clicking a Java action or a JavaScript action, respectively.
...
* We fixed an issue in the modernized App Explorer where navigating to a document not visible in the tree (for example, a hidden styling node) made Studio Pro unresponsive.
...
* We now prevent the use of the selection of a widget as an argument for snippet calls because it is not supported in the client.
...
* We improved the security of the module import process. ([CVE-2025-40592](https://cert-portal.siemens.com/productcert/html/ssa-627195.html))
