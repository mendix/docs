...
* **WorkflowEvent** - A representation of the workflow event data at a specific execution moment. It contains the following attributes:
    * **EventTime** is the time at which the event is triggered.
    * **EventType** is the type of the event being triggered. See [Workflow Events](/refguide/workflow-events/#workflow-event-types)
    * **WorkflowEvent_Initiator** is the identifier of the current user object which is logged in. The activity workflow events will have an initiator in cases where a user takes some action otherwise it will be empty. For example, when a user specifies the outcome of a user task or when a user notifies a suspended workflow then in this case this user becomes the initiatior. As a special case, it will also be empty for the `WorkflowInitiated` event of a sub-workflow because a sub-workflow is directly related to the [CallWorkflowActivity](/refguide/call-workflow/).
