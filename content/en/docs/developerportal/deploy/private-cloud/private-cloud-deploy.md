...
{{% alert color="info" %}}
If cluster manager has configured Private cloud License manager for this namespace, this option will be disabled.
{{% /alert %}}
...
{{% alert color="info" %}}
The Operator license is independent from a Mendix Runtime license. The Operator license allows you to manage Mendix apps in your cluster, while the Mendix Runtime license (configured through a [Subscription Secret/PCLM](#license-mendix)) removes trial restrictions from a Mendix App itself. 

You can get an Operator and Runtime license from [Mendix Support](https://support.mendix.com), together with instructions on how to configure it.
{{% /alert %}}
...
If PCLM is configured for the respective namespace, then the license from PCLM will be claimed by the environment.

{{% alert color="info" %}}
​​Cluster manager is supposed to install the license in the PCLM. Kindly contact your cluster manager for this. You can find more information on PCLM in the [documentation](/developerportal/deploy/private-cloud-license-manager/)
{{% /alert %}}

#### 5.1.4 Security

By default, your environment will be created as a Production environment. However, if the cluster manager has enabled the developer mode for this namespace, then you can also set the developer mode for this environment.
