...
* We fixed an issue where synchronizing objects in an offline-first Progressive Web App fails along with a "Autocommitted objects detected at end of transaction for long-lived user session" log. (Tickets #133154, #136135, and #137319)
* We fixed an issue where changed objects that have a reference to a system attribute would not be garbage collected when no longer in use (Ticket 138167)
* We fixed an issue when REST Service dialog shifts upwards when adding a new resource. (Ticket #130238)
* We made the Create Variable dialog resizable.
* We fixed an issue where a user cannot type space {Space} in a text box widget wrapped by container widget with an On click event (Ticket 139090)
* We fixed sorting by multiple attributes in Excel export of Data grid. (Ticket 136239)
* We fixed a bug where fast re-deploy at development time would not generate newly added pages correctly. This would result in bundling failures or red screens which could only be solved by a full deployment of the app. Now fast re-deploy should again correctly generate all affected pages.
* We fixed an issue where combining a "Close page" action with editable column data grid widget was causing errors to be thrown. (Ticket 140660)
* We fixed an issue where an exception is thrown when decoding Base64 binary data with special characters. We now introduce a new runtime setting com.mendix.core.UseMimeDecoderForBase64 to be able to decode MIME type of Base64. (Ticket #136429)
* We fixed a bug where some auto-generated files caused conflicting changes which needed to be resolved manually.
* We fixed an issue that could cause exceptions on non-leader cluster nodes after starting them for the first time after a domain model change (Tickets 138387 and 140298).
* We fixed an issue when an offline PWA gets an error when trying to connect to the runtime after closing the browser and re-launching the app. (Ticket 139230 , 143392)
...
* <a name="2102"></a>We fixed a [known issue](9.0#ki-2102) where you could not copy, paste, or delete using keyboard shortcuts in the **Properties** pane. (Tickets 126003, 139441, 141303)
