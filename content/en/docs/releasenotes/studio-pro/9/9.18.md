...
* We fixed an issue in workflows where previously incompatible workflows are no longer marked as **Incompatible** after the second successful upgrade.
...
* Starting with version 9.18.5, we will drop support for MariaDB 10.2, which is no longer supported by the vendor.
