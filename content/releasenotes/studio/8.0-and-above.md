...
* To prevent consistency errors, it is now impossible to create an entity, association or an attribute within the same entity with duplicated names. If you type in the same name for another entity, association or an attribute within the same entity, it will get an underscore and a numeric postfix after its name. For example, if you already name an entity named *Customer* and try to create another entity with this name, it will get a name *Customer_1*. The next duplicate will get a name *Customer_2*, etc.  

*  To improve user experience of the page editor, the **Date Picker** widget has formatting options in its properties now. You can select between **Date**, **Time**, **Date & Time**, and **Custom** formats and see  examples of how they will be displayed in the **Format Example**. 
