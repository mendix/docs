...
* Now you can add a click action in the [container widget](/refguide/container#events) for both web and native mobile pages.
...
Here’s how it works: Studio Pro generates the model for you, so it works out of the box with a single click, but you also have the flexibility to fully customize if needed. When selecting a microflow, you get the question to generate the controls, which now also includes input fields for search values and a search and reset button. When you click **Search**, the entered values are passed in the **Paging** system entity, and you can then use them in your microflow.
...
* We added documentation links to page errors CE1573, CE1574, and CE0552 as well as to navigation error CE0548 and the "No image selected" error CE0436. For more information on consistency errors, see [Consistency Errors](/refguide/consistency-errors).
...
* When you had a conditional visible widget and you deleted the object used by the conditional visibility expression, this resulted in an error. This has been fixed. (Tickets 88646, 88783, 89114, 89756)
