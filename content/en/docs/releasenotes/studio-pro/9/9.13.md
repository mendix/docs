...
* In published OData services you can now allow clients to [delete objects](/modeling/integration/published-odata-services/published-odata-resource/#deletable). This is off by default. When this feature is switched on, entity access applies, so only clients with the right credentials that are allowed to delete.
