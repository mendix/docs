...
For more details about the new flexible profiles, see [Navigation](https://docs.mendix.com/refguide7/navigation) in the Mendix Reference Guide.
...
* We fixed the bug where changes for an object were not available inside a microflow.
...
* When using the `addRefreshObjectFeedback` or `addRefreshObjectListFeedback` Java APIs, we do not redundantly refresh list views and data grids. Such refreshes should be performed only by `addRefreshClass`. (Ticket 50650)
* We fixed the fetching resource for offline mobile apps.
* We fixed the bug where changes for an object were not available inside a microflow. (Ticket 50650)
