...
#### Native Logging Improvements

* Starting now, native mobile apps can sends logs on the trace and debug levels to the Mendix Server, allowing you to analyze how your app works in detail. To enable this, set the log level to trace for log nodes that start with the “Client_” prefix. However, we recommend enabling the trace/debug log levels for only a short period of time, as it may cause a large influx of logs from all end-users. To learn more about native logging, see [Logging in Native Apps](/refguide/mobile/distributing-mobile-apps/logging/).
