...
* We added a column  to the [Open App](/refguide/open-app-dialog/ ) dialog box displaying the Mendix Studio Pro application **Version** (including for local apps that are stored on disk). [**TW - /refguide/open-app-dialog/ NEEDS UPDATE**]
...
* We improved the discoverability of exposing microflows as microflow and workflow actions by adding this option to the context menu of the microflow editor. [**TW REVIEW**]
* MxAssost Performance Bot best practice [MXP003](/refguide/performance-best-practices/#mxp003) for a non-indexed attribute used in sorting is now extended to the same attribute types as for a non-indexed attribute used in XPath.
* We updated the [Revert All Changes](/refguide/version-control-menu/#revert-all-changes) menu item to show the app selector dialog box if the app gets closed after the operation (for example, when the changes include a version change).
