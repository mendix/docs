...
* We have added support for multi-instance user tasks. It will now be possible to configure user tasks that require multiple approvals. These user tasks will be completed based on the completion criteria specified. A completion criteria can be of the **Consensus** or **Veto**. [**TW REVIEW?**]
...
* We updated the style for the following UI components: button, checkbox, checkbox list, date and time selector, drop-down menu, group box, link, radio button, splitter, stepper, label, text editor, text field.
...
* We added a new MxAssist Performance Bot Best Practice MXP016 to find the key of an OData source that is missing an index. [**TW DOC UPDATE?**]
* We added a **Category** column to the Best Practices of Best Practice Bot. [**TW - WHAT?**]
* We changed the default location for a published OData service to use lower-case.
* We added an option in the [mx command-line tool](/refguide/mx-command-line-tool/#check) to the `mx check` command to output the results to a JSON file.
...
* To make it possible for a multi instance user task to have multiple assignees, we have renamed the `WorkflowUserTask_Assignee` association to `WorkflowUserTask_Assignees` and it will be a reference set from now on.
