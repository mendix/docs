...
- We now allow changing the project's foreign key constraint configuration in the **Runtime Settings** dialog. By default this feature is turned off in apps created before Mendix 10.6.0 and is on in apps created on that version and above. For more information, see the [Foreign Key Constraints](/refguide/data-storage/#fkc) section in *Data Storage*. {TW}
...
- We now allow configuring credentials for Azure database, using credentials present in the running environment,
  without hard coding database username and password in runtime settings. For more information, see the [Microsoft Azure SQL](/refguide/custom-settings/#6-microsoft-azure-sql) section in *Runtime Customization*.
...
- We have added support for Epics and Jira to the **Stories** pane in Studio Pro. It is now possible to update the status of stories and tasks directly from Studio Pro, and to select related stories and tasks when committing changes to the Team Server. {TW}
...
- We added a consistency error for header that can only appear once, for example "Content-Type", but it was added multiple times in Consumed REST Service (Beta). {TW}
- We combined multiple headers with the same name in runtime for **Send Rest Request (Beta)**.
-  We introduced "application/json" as the default 'content-type' header for requests supporting body in Consumed REST service (beta). By having this 'content-type' header, it helps the server know the body content type sent by the client application. {TW}
