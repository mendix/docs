...
| http.client.CleanupAfterSeconds | Introduced in version 7.22.<br/> For Call REST and Call Web Service actions, the first request to a new host wil create an http client that will handle subsequent request. When there are no new requests to the host for the specified time, the http client will be cleaned up. Default value: 12 * 60 * 60 (12 hours). A value of 0 means no cleanup. |
