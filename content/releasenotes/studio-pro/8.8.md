...
## Notice in preparation of 8.9

With Mendix 8.9, we plan to roll out a more significant upgrade of the underlying technology layer, by upgrading React Native to version 0.61 and by upgrading all native libraries to their most recent versions, to benefit the latest features, fixes, and overall to guarantee the quality and stability of native mobile apps build with Mendix.
This has some significant consequences for existing users, that we would like to prepare you for early on.
We did not take this decision lightly, but we feel that it is essential to keep our third-party dependencies as up-to-date as possible.

### Make It Native

The Make It Native app published in the Apple App Store and the Google Play Store is at all times compatible with the latest version of Mendix.
Due to the changes in native libraries, the version of Make It Native that we will release alongside the release of Mendix 8.9 will not be compatible with older Mendix versions.
Our advice it to upgrade, but if you do not wish to or cannot upgrade your project to Mendix 8.9 there are several options:

- For Android we will provide a special Make It Native app version which you can use with Mendix 8.8.
- You can create a custom developer app for your project by following [these instructions](https://docs.mendix.com/howto/mobile/how-to-devapps).

We will release separate documentation to help you make this a smooth as possible.

As part of this upgrade, several third-party dependencies will updated in order to keep them compatible with React Native version 0.61.
In most cases, the APIs of these libraries remain unchanged.
However, all widgets and JS actions that rely on third-party dependencies should be tested against Mendix 8.9, once it is available.

### AsyncStorage

The storage location of AsyncStorage on iOS is moved to Application Support.
If your widget or JS action uses AsyncStorage (imported from `react-native-community/async-storage`), then this change does not affect the workings of your application, and any data found in the old location is copied to the new location.

However, some widgets or JS actions still import AsyncStorage from the React Native core library.
We strongly urge all maintainers of such JS actions or widgets to start using `react-native-community/async-storage` instead, as it might not be possible to migrate your data from the old location to the new location later on.
