...
Version 1.13.1 of Private Mendix Platform is a patch release aimed at fixing an certificate trust issue with PCLM-Operator.

#### Improvements

Fixed an issue where the Kubernetes Operator would not trust to communicate with PCLM when a self-signed CA certificate was configured to encrypt the TLS connection between different Kubernetes clusters.

(Various other improvements to Operator are part of this release, such as setting the log levels in Operator deployment, enabling customers to tailor log visibility, resolving an application scaling issue when PCLM was configured without licenses, and generating the log format in JSON now. More in Operator release notes.)
