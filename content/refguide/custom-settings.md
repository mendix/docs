...
| **ReadCommittedSnapshot** | Defines whether the`READ_COMMITTED_SNAPSHOT` option of Microsoft SQL Server must be enabled or not. For more information, see [Using Snapshot Isolation](https://docs.microsoft.com/en-us/sql/relational-databases/sql-server-transaction-locking-and-row-versioning-guide?view=sql-server-ver15). The value can be `true` or `false`. | true |
