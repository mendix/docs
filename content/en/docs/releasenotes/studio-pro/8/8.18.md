...
* We fixed an issue in offline native and hybrid profiles where startup synchronization fails due to missing schema for all or part of the syncrhonized data by creating `System.SynchronizationFailure` entries for these pieces of data so that application's administartors can process these entries and restore the data.
* We fixed an issue with the date picker not being visible in dark mode. (Tickets #120143, #122791, #122843)
* We fixed an issue with the reference selector placeholder color not correctly being set. (Ticket #133479)
