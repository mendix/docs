...
In Studio Pro 11.12.0, we introduced Change Data Capture (CDC) as a beta feature. With CDC, you can publish changes from Mendix domain model entities as Kafka events by adding the entities to Change Data Capture Service document. Each added entity will have its own Kafka topic. When the objects are created, updated, or deleted, the runtime captures these committed changes and publishes them to specified Kafka topic.
