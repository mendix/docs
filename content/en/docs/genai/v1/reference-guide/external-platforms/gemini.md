...
You can use the GenAI Commons toolbox actions to [create the required Request](/agents/agents-kit-1/reference-guide/commons/#genai-request-building) and [handle the Response](/agents/agents-kit-1/reference-guide/commons/#genai-response-handling) for your use case. 
...
Operations for chat completions focus on the generation of text based on a certain input. In this context, system prompts and user prompts are two key components that help guide the language model in generating relevant and contextually appropriate responses. For more information on the type of prompts and message roles, see the [ENUM_MessageRole](/agents/agents-kit-1/reference-guide/commons/#enum-messagerole) enumeration.

The `GeminiDeployedModel` is compatible with the two chat completion operations from GenAI Commons. While developing your custom microflow, you can drag and drop the following operations from the toolbox in Studio Pro. See category [GenAI (Generate)](/agents/agents-kit-1/reference-guide/commons/#genai-generate): 
...
For `Chat Completions with History`, you can optionally add `FileCollection` to individual user messages using [Chat: Add Message to Request](/agents/agents-kit-1/reference-guide/commons/#chat-add-message-to-request).
...
Gemini supports the following [tool choice types](/agents/agents-kit-1/reference-guide/commons/#enum-toolchoice) of GenAI Commons for the [Tools: Set Tool Choice](/agents/agents-kit-1/reference-guide/commons/#set-toolchoice) action is supported. For API mapping reference, see the table below:
