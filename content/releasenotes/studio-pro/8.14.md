...
{{% alert type="warning" %}}
We have investigated an issue related to styling that might cause breaking changes in style in your native app (web apps are not affected). The impact has been identified and there are workarounds available. Please see the [Known Issues](#known-issues) section below for details.
{{% /alert %}}
...
* The is an issue related to styling that might cause breaking changes in style in your native app (web apps are not affected). This will be fixed in 8.14.1. Until that patch is released, please use these workarounds:
	* Cards (only happens if a height is being applied) – wrap the content of the clickable container with a new container (without an action),  apply the following class, and remove the properties of alignment from the clickable container:
		```
		...
		```
	* Floating action button – this is fixed in the latest version of [Atlas UI Resources](https://appstore.home.mendix.com/link/app/104730/) available from the App Store
	* List view – wrap one container around the **Home_Communication** snippet call in the page, then add a `borderBottom` class into this new container or add a design property that adds `spacing bottom`
	* Check box – access the **AccountSettings_Edit** profile page, then show a label in the check box with the text from the left column and move the check box out from the layout grid  (by removing the layout grid)
