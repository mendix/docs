...
* We improved the startup of the runtime by not registering handlers for REST, OData and web services when the model does not contain any REST, OData or web services, respectively. This has several advantages: it makes the application startup slightly faster, there are fewer log messages at startup, and it follows the security best-practice of endpoint hardening.
...
* We fixed the issue that extra newlines would be added to the end of an expression when typing in the expression editor. (Tickets #101033, #103209, #105990, #107049, #107239, #115807, #115894, #116692, #116963, #118606, #119602 and #122468)
* We fixed the issue that temporary files and folders (like 'GPUCache') would be created in the app directory, which could lead to unnecessary merge conflicts. (Tickets 105909, 106329 and 107857)
* We fixed the issue that when multiple rows are selected in the 'Changes' window, and these rows are copied to the Windows clipboard by pressing the Ctrl+C keyboard shortcut, only some of the rows would be copied multiple times. (Ticket 123191)
* We fixed an issue where Logic Bot sends a common flow type to the server to make a suggestion instead of the exact type.
* We fixed the issue where extracting a microflow as a submicroflow with a loop that has multiple parameters would not correctly set the parameters on the extracted submicroflow. (Tickets 97103, 110679)
* We upgraded the SQL Server JDBC driver authentication DLL files to 9.2.1. (Ticket 124044)
* Fixed an error when a conditional editability expression references a non-existent attribute when the widget is non-editable due to its parent container (e.g. list view/data view) (Ticket 121857)

### Deprecations

* We dropped support for the following database versions that are no longer supported by vendors:
* We fixed a crash that would occur when conflicts after an Update or Merge were only present on the files in the app folder, and not on the app itself.
...
* We fixed an error that could occur when refreshing the list of Team Server apps in the 'Open App' dialog. (Ticket 112763)
