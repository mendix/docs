...
* The Helm chart will now use proxy and custom TLS configuration when registering Storage Plans in the Portal.
...
* We have fixed an issue where passwordless IAM or Managed Identity authentication wouldn't update tokens while an app is starting, and caused database connection errors in apps that took 15+ minutes to start. (Ticket 282784)
...
* We have updated the PodDisruptionBudget to allow eviction of unhealthy pods by default. This will prevent crashlooping pods from blocking node maintenance.
