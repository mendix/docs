...
자동화된 테스트는 테스트 유형에 따라 다양한 도구로 만들 수 있습니다. 통합 테스트는 [SoapUI](/howto8/integration/testing-web-services-using-soapui/)로, 단위 테스트는 [Unit Testing](/howto8/testing/testing-microflows-using-the-unittesting-module/) 모듈로, UI 테스트는 [Selenium IDE](/howto8/testing/testing-mendix-applications-using-selenium-ide/)로 만들 수 있습니다. Selenium IDE를 사용하면 자동화 지원 탐색적 테스팅을 돕는 스크립트를 만들 수 있습니다. 견고한 브라우저 기반 테스트를 만들고 싶다면 Selenium과 함께 테스팅 프레임워크를 사용하는 것을 살펴보아야 합니다.

TestNG는 Selenium을 구동하는 데 사용할 수 있는 Java 테스팅 프레임워크입니다. 이 사용 방법 가이드에서는 TestNG로 자동화된 테스트를 만드는 방법을 배웁니다.

이 사용 방법 가이드에서는 다음을 배우게 됩니다:

* TestNG 테스트 파일 만들기
* TestNG로 자동화된 테스트 실행하기
* TestNG로 보고서 생성하기
