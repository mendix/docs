...
* SQL Server integrated security does not work and causes an error "This driver is not configured for integrated authentication."
	* Workaround: New DLLs are needed to work with the updated JDBC driver and should be placed inside the `runtime/lib/x86` and `runtime/lib/x64` directories of the Mendix 8.7 install location. The correct dlls can be downloaded from https://github.com/microsoft/mssql-jdbc/releases/tag/v8.2.0 (select mssql-jdbc_auth.zip).
