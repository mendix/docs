...
Use input parameters and generic functions instead of hardcoding every variable or option. Also don't write actions that do everything at once. Let your actions do one specific task, e.g. the retrieval of a WebElement. This serves the readability of your actions/test cases and also makes maintenance much easier. It will go wrong, for example, when an input field requires an unique name. This will cause trouble when you rerun the test case, since ATS will use the same input. It will fail because an error occurs saying that the name already exists. You can create a test case where you delete the actions of the previous test case or you can use a data set that changes the value occasionally.  
