...
* Make sure that the security of the app must be set to Production. By default, all environments are set to Production mode when created.
...
{{% alert color="warning" %}}
All environments are defined as production environments, which means that [security in the app must be set to `Production`](/refguide/app-security/).
{{% /alert %}}
...
3. A **Internal Name** (UUID) will be generated for you. This will be used when creating your environment to ensure that all the environment names in your namespace are unique.
