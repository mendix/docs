...
* We introduced a new visual language – [workflows](/studio/workflows). Workflow is a visual language in Mendix Studios that allows you to build extendable processes. It is fully integrated with other visual languages, such as the microflow editor and page editor. Workflows is a visual way of expressing application logic and looks like a flow chart:
    ...
    A starting point for using workflows is the workflow-specific app template **Approval App** that has a preconfigured workflow as well as dashboards, admin pages, microflows, etc. This app template allows you to build an end-to-end app with a fully automated process. For more information on workflows, see [Workflows](/studio/workflows).

    A new expression editor is available for workflows, which gives you a wider choice of operations and functions you can use in expressions
