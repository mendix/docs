...
* There is an issue with the [updatable (write) capability](https://docs.mendix.com/refguide/published-odata-resource#8-capabilities) in published OData services that allows clients to change attributes values of other entities, as long as they know the ID's of the objects and their credentials give them entity access.
    * Workaround: do not give users that access the OData service write entity access to entities that are do not have the updatable (write) capability.
