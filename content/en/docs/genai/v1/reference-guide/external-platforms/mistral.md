...
Mistral provides vector embedding generation capabilities that can be invoked using this connector module. The `MistralDeployedModel` entity is compatible with the [knowledge base operations](/agents/agents-kit-1/reference-guide/genai-for-mx/commons/#genai-knowledgebase-content) from GenAI Commons.
...
Depending on the operation you use in the microflow, provide an `InputText` string or a [ChunkCollection](/agents/agents-kit-1/reference-guide/genai-for-mx/commons/#chunkcollection). The current version of this operation only supports the float representation of the resulting vector.
...
Mistral supports the following [tool choice types](/agents/agents-kit-1/reference-guide/genai-for-mx/commons/#enum-toolchoice) from GenAI Commons for the [Tools: Set Tool Choice](/agents/agents-kit-1/reference-guide/genai-for-mx/commons/#set-toolchoice) action. For API mapping reference, see the table below:
