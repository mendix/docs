...
| `LIKE` | Matches the pattern after the operator. The wildcard character '%' can be used to define any string of zero or more characters. In order to serach for special characters, like `%`, `_` and `\`, they should be escaped with `\` escape character. | `City LIKE '%dun'` returns all city's which names ends with 'dun', like 'dun' and 'Losdun'.<br> `Symbol LIKE '%\%'` returns all symbols which end with `%` special character.|
