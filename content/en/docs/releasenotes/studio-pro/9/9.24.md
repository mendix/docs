...
- We fixed an issue where changes made in the Java Script Action document can get lost. (tickets #202253)
...
- We fixed an issue where a data source with association was not visible when the surrounding data of a snippet was not coming from a direct data view parent. (Ticket 203582)

- We fixed an issue for reponsive web offline in which a data grid, template grid and list view in some cases with a datasource that is configured with an entity path over an association, did not correctly apply the constraint(s).
...
- We fixed an issue in which the consistency check for the correct ownership of an association was not applied to pluggable widgets with association properties that are not linked to a data source. (Ticket 207707)
- We fixed an issue that caused the rejection of encoded URLs in the request-body of POST/PATCH requests for published OData services (Ticket 207617).
