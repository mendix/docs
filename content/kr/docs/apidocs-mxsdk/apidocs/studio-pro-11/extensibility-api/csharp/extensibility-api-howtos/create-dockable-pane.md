---
title: "C#을 사용하여 도킹 가능한 창 확장 프로그램 만들기"
...
이 사용 방법(how-to)에서는 Studio Pro에 사용자 지정 도킹 가능한 웹 창을 추가하는 방법을 설명합니다.

이 사용 방법의 예제는 [이 GitHub 저장소](https://github.com/mendix/ExtensionAPI-Samples)에서 다운로드할 수 있습니다.

## 전제 조건

이 사용 방법을 시작하기 전에 [C#을 사용하여 메뉴 확장 프로그램 만들기](/apidocs-mxsdk/apidocs/csharp-extensibility-api-11/create-menu-extension/)를 먼저 수행하는 것이 좋습니다.

## 도킹 가능한 창 확장 클래스 생성

1. [C#을 사용하여 메뉴 확장 프로그램 만들기](/apidocs-mxsdk/apidocs/csharp-extensibility-api-11/create-menu-extension/)를 따라 이전에 생성한 프로젝트를 엽니다.
2. 프로젝트에 *MyDockablePaneExtension.cs*라는 새 클래스를 추가합니다.
3. 파일의 코드를 다음으로 바꿉니다:
...
`InitWebView` 메서드를 재정의하여 도킹 가능한 창 내의 웹 뷰 콘텐츠를 설정할 수 있습니다. 이 예제에서는 `http://mendix.com` 홈페이지를 포함합니다.
...
새 도킹 가능한 창을 표시하려면 [C#을 사용하여 메뉴 확장 프로그램 만들기](/apidocs-mxsdk/apidocs/csharp-extensibility-api-11/create-menu-extension/)를 따를 때 생성한 확장 프로그램을 수정하십시오.

1. `MyMenuExtension.cs`를 엽니다.
2. 내용을 다음 코드로 바꿉니다:
...
* `IDockingWindow` 서비스를 사용하여 창을 트리거하는 *Open My Dockable Pane*이라는 새 메뉴 항목이 추가되었습니다.

이러한 변경을 수행한 후 프로젝트를 빌드하십시오. 출력을 대상 폴더에 자동으로 복사하지 않도록 선택한 경우 프로젝트의 bin 출력을 [C#을 사용하여 메뉴 확장 프로그램 만들기](/apidocs-mxsdk/apidocs/csharp-extensibility-api-11/create-menu-extension/) 프로세스를 따를 때 생성한 확장 폴더로 수동으로 복사하십시오.

## 사용자 지정 메뉴 없이 도킹 가능한 창 표시

도킹 창을 열기 위해 별도의 메뉴 항목을 추가하지 않으려면 `DockablePaneExtension` 구현에서 `ViewMenuCaption` 속성을 재정의할 수 있습니다.
