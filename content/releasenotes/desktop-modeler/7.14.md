...
* We now support selecting multiple documents in the Project Explorer. The selection can be extended by pressing the <kbd>Ctrl</kbd> or <kbd>Shift</kbd> key while clicking. With multiple documents selected, the following operations can be performed: open, drag & drop, move, exclude/include, delete, and duplicate.
        {{% alert type="info" %}}This is based on an [upvoted idea from Wouter Kremer Weiss](https://forum.mendixcloud.com/link/ideas/24) submitted to the [Mendix Idea Forum](https://forum.mendixcloud.com/link/ideas). Nice work, Wouter!{{% /alert %}} 
...
However, we wanted to make sure you still can easily distinguish between these documents. So, we replaced the canvas colors with a nice indicator icon in the toolbar. This makes it instantly clear what the document type is that you are editing. Next to this, microflows show an indicator in the bottom-right of the editor to mark entity access.
...
We've made published REST services a lot more powerful. The biggest new feature is that you can now use microflows that return an entity or a list as well as use microflows that take entity or list parameters. You will need to specify mappings for them – an import mapping for the incoming request, and an export mapping for the response. If you use
message definitions to define the mappings, you will see that the OpenAPI documentation page contains the structure of the messages. In your microflow, you can decide which type of content the mapping should produce (JSON or XML), or you can leave it up to the client to specify it in an **Accept** header.

These are improvements we made to how published rest services behave:

* Mendix now supports more flexible content types, including specifying character sets. The only valid character set is still UTF-8, but you may specify that now. 
...
* For an import mapping in a REST operation you can now decide which commit behavior you would like. You can chose to either commit, commit without events or not commit the imported objects.
...
* We fixed the issue causing the client to return more committed objects than needed. (Ticket 54094)
...
* We fixed the issue where using multiple validation feedback actions on the same object in a nanoflow resulted in showing only the most recent one.
...
* We fixed the issue where selecting multiple rows from a reference set selector at once did not update the selector's grid properly. (Ticket 58626)
...
* <a name="62961"></a>We fixed the issue where an image widget with an on-click action configured to call a microflow was failing to execute while showing a default image. (Ticket 62961)
...
* A report with a report parameter generated using a Java action as a data set source was failing, because the report parameter was not passed correctly. That parameter is now correctly passed. (Ticket 61252)
* We fixed the handling of dates with years from 0 to 99. (Ticket 61281)
* We fixed the issue where calling a microflow threw an exception when an enclosing object was empty and was used as a parameter. (Tickets 63533, 63703)
...
* The **Generate document** microflow action now accepts expressions of the decimal type for margins, without requiring a conversion to the deprecated float type. (Ticket 55837)
...
* We fixed the issue where creating a new attribute for an entity with a unique constraint on that same attribute at the same time along with a change in one of the system members made the synchronization fail. (Ticket 59926)
* We fixed the issue where access rules were not applied strictly enough (though this error was corrected later, a warning was still generated). (Ticket 62877)
...
* We fixed the issue where high loads caused an exception while consuming a REST service. (Ticket 61802)
* We fixed the issue where a microflow required user roles to be assigned to it even when it was used in a REST operation that did not require authentication.
* We fixed the issue in REST Publish where we did not use the value of the `reason` phrase property of the HTTP response that was set in the microflow.
...
* We fixed the issue with session authentication for REST services where the user session was terminated after the first request to the REST service.
