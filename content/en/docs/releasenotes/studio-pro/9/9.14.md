...
{{% alert color="warning" %}}
Mendix Studio Pro is now using a newer JDK and switched from AdoptOpenJDK to Eclipse Adoptium JDK. This change uninstalls the AdoptOpenJDK 11 installation you already have. Studio Pro 9.14.0 and above automatically detects this and uses the Adoptium JDK. For Studio Pro versions below 9.14.0, you need to set the JDK directory in **Preferences** > [General](/refguide/preferences-dialog/#general). If you are using the Oracle JDK, your Studio Pro installation is not affected.
{{% /alert %}}
