...
* We added a new setting in **Preferences** > **Advanced** that allows you to enable software rendering mode. This can help improve performance in certain scenarios (for example, where dialog boxes close slowly). [**TW DOC UPDATE**]
...
* We fixed an issue that occurred when the **Export to image** option was not enabled for domain model, workflow, microflow, nanoflow, rule, and document template editors. (Ticket 177392)
