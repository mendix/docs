...
| Registration fails on a PWA on iOS | To receive push notifications on iOS the app must be served over https and added to the home screen | Deploy your app (or use a tool such as [ngrok](https://ngrok.com)), add your app to the home screen via the Share button in Safari, and try again |
...
| Messages are not sent (Status remains Pending) | The Push notifications connector uses scheduled events to retry failed message deliveries. Scheduled events are not supported in a free app running in the Mendix Cloud. | Run your app on a licensed node |
