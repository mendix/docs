...
description: "Mendix on Kubernetes에서 OCI 이미지 레지스트리를 구성하는 방법을 설명합니다."
weight: 11
...
앱을 실행하려면 Kubernetes에서 OCI 이미지 레지스트리에서 사용할 수 있어야 합니다.

새 버전의 앱이 배포되면 Mendix Operator는 *빌드* Pod를 실행하여 앱을 컨테이너 이미지로 패키징하고 프라이빗 OCI 레지스트리에 푸시합니다.

이를 위해 Mendix Operator는 사용할 레지스트리와 인증 방법을 알아야 합니다.

### 지원되는 컨테이너 레지스트리

[OCI Distribution Specification](https://github.com/opencontainers/distribution-spec)은 컨테이너 레지스트리가 호환되기 위해 지원해야 하는 표준과 API를 문서화합니다. Mendix Operator는 [go-containerregistry](https://github.com/google/go-containerregistry)를 사용하여 컨테이너 레지스트리에 이미지를 빌드하고 푸시합니다. 이를 통해 Mendix on Kubernetes는 대부분의 컨테이너 레지스트리를 지원할 수 있습니다. 대부분의 경우 컨테이너 레지스트리가 기본 인증(정적 사용자 이름 및 비밀번호)을 지원하면 Mendix Operator가 이를 지원합니다.

이러한 컨테이너 레지스트리의 예시는 다음과 같습니다:
...
그러나 정적 자격 증명은 종종 안전하지 않은 것으로 간주되며, 클라우드 공급자는 단기 토큰 기반의 대체 인증 방법을 제공합니다. 예를 들어 ECR에 이미지를 푸시하려면 AWS API에서 단기 토큰을 가져와야 합니다. 특정 컨테이너 레지스트리에 대한 자세한 내용은 [레지스트리 구성](#configure-registry) 섹션을 참조하십시오.

### 제한 사항

현재 여러 인증 방법을 결합하는 것은 불가능합니다. For example, Nexus uses static username/password credentials, while ECR requires using an authentication plugin that integrates with AWS IAM; if you'd like to host [air-gapped images](/developerportal/deploy/private-cloud-migrating/) of Mendix on Kubernetes in a Nexus repository, you will need to use Nexus (or another registry supporting static credentials). Using ECR or ACR as the target registry will not be possible.
...
Mendix on Kubernetes는 클러스터 내부에서 이미지를 빌드합니다. After an image is built, the Mendix Operator sets (updates) image URLs of the app's Kubernetes Deployment resource;
...
클러스터가 방화벽으로 보호되어 Mendix 레지스트리로의 경로를 열 수 없는 경우와 같이 기본 Mendix 구성 요소를 자체 레지스트리에 호스팅할 수 있습니다. 이 경우 일부 또는 전체 Mendix 구성 요소를 클러스터로 마이그레이션해야 합니다. 이를 수행하는 방법은 [자체 레지스트리로 마이그레이션](/developerportal/deploy/private-cloud-migrating/)의 지침을 참조하십시오.

### 이미지 이름 템플릿 사용자 정의 {#customize-registry-imagenametemplate}
