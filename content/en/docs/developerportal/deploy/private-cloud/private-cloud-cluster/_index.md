...
7. Enter the following information:
    ...
    1. **Installation Type**  – Choose Global Installation if you want a single operator namespace to manage multiple namespaces or Namespace Installation if only one namespace is required to be managed by an operator. For more information, see [Global Operator](/developerportal/deploy/global-operator/).
...
{{% alert color="info" %}}
If Global Operator is configured with [PCLM](developerportal/deploy/private-cloud/private-cloud-license-manager/), you will be able to view the **Runtime** and **Operator** list of licenses in the main namespace, and only the list of **claims** for the managed namespace (when configured with PCLM).
