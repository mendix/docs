...
* The first time you open MendixChat, you need to click **Sign in** so that it is available for use. This only needs to be done once after installing this version of Studio Pro.
* A finished parallel split path that is removed from a running workflow instance wrongly leads to a versioning conflict. Please note that "path" was called "branch" in earlier versions.
* Line endings in CSS files are not handled properly. Thus sometimes after Revert all changes or other version control operations the css files show up in Changes on Disk dialog.
