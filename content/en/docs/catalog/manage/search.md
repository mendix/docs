...
{{% alert color="info" %}}The **Dataset** is the name of the **Entity set** of a published **Entity** in Mendix Studio Pro, which by default, is the entity name with an "s" appended to it. For example, if an entity named `Customer` is published in a service, the **Dataset** name in the **Search Details** will be `Customers`.{{% /alert %}}
...
The Catalog is a repository of published OData and OpenAPI services (or data sources) that are deployed to a particular environment. You can have multiple versions of a service for each environment by giving them different version numbers.
...
{{% alert color="info" %}}In Mendix Studio Pro, the **Dataset** is the name of the **Entity set** of a published **Entity**. This defaults to the entity name with an "s" appended to it. For example, if an entity named `Customer` is published in a service, the **Dataset** name in the **Search Details** will be `Customers`.{{% /alert %}}
...
* **Name** – the name of the association that is exposed in the service contract.
