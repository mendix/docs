...
Mendix has developed 'git-fixer' – a cleanup tool to assist you in shrinking your repository. For more information, see the [Cleanup Tool](#cleanup-tool) section below.
...
* Ensure the Git project you are downloading is not yet on your device
* Download the branch through Studio Pro, while manually measuring how long the download takes
  
The first download of a branch on a device is a good indication of the maximum waiting time you or a colleague can experience. Subsequent branch downloads use data that is already available locally, and will therefore be a lot faster.

If the download time was acceptable, or if you have a workflow where team members don't often change so there are not a lot of 'first downloads', you can skip the cleanup.
...
#### 5.2.6 Pushing Your Results to the Server {#push}
...
* Log file (its location can be found in the command line output)
* Version of the tool version, similar to `git-fixer v1.11.2` (the version number can be found in the command line output)
