...
In order to use retrieve and generate in one operation, an internally predefined Tool can be added via the `Tools: Add Mendix Cloud Knowledge Base` action to the [Request](/appstore/modules/genai/commons/#request). The model can decide whether to use the [knowledge base retrieval](/appstore/modules/genai/commons/#knowledge-base-retrieval) tool when handling the request. This is supported by both with-history and without-history operations. You may add optional filters, such as `MaxNumberOfResults` or `MinimumSimilarity`, or pass a [MetadataCollection](/appstore/modules/genai/commons/#metadatacollection-entity). The optional `Description` can be used to help the model understand what the knowledge base content is about and to decide whether it should be called in the current chat context. 
