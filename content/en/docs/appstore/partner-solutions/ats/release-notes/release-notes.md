...
- We have added support for [Mendix Epics](https://docs.mendix.com/appstore/partner-solutions/ats/ht-two-connect-stories-to-testcases/#3-set-the-mendix-api-key).

### Fixes

- We fixed an issue that resulted in improper functioning of the action Find element by sizzle.
- We fixed an issue where Assert Validation Message would fail when negate is set to true.
- We fixed an issue that caused Open mendix application action to fail with a render timeout for Selenium 4.
...
- The built-in function _Assert Condition Fails_ was not working due to a bug . It is now fixed.
