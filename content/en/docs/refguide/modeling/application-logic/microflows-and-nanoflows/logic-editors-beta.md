...
We are redesigning the logic editors (microflows, nanoflows, and rules) to bring the user journey to the next level. In the Beta versions of the logic editors, we have enhanced the usability of the [MxAssist Logic Bot](#logic-bot), added more support for [keyboard navigation](#keyboard-navigation), and also invested in more [developer experience details](#miscellaneous) to make sure the interaction with the canvas is even more fluent. For more information on those functionalities, see the sections below. 
