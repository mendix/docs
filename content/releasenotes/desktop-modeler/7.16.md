...
Observant users will recognize the preview rendering from our Web Modeler, which is currently in beta. By introducing the view mode, we hope to facilitate collaboration between Desktop Modeler and Web Modelers users. If you find yourself enjoying the view mode experience, feel free to give the Web Modeler a try. Open your app project on [home.mendix.com](https://sprintr.home.mendix.com/) and select the **Edit in Web Modeler** option under **Edit App**.
...
* We fixed the issue where you could make the name of a published REST service empty. That is not allowed anymore, because it showed an empty link in `/rest-doc`. 
