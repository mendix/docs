...
Dates can be created with the `dateTime` and `dateTimeUTC` functions. The difference between the two is that `dateTime` uses the calendar of the session used in the function call, and `dateTimeUTC` uses the UTC calendar. The system session runs always as UTC by default, except for scheduled events which can be configured in the [project settings](project-settings).
