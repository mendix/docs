...
* We fixed a memory leak that can happen in notification dialog boxes, such as the dialog that notifies the user about running an app.
* We fixed an issue where an **Oops** dialog box apperead when opening Maia Chat.
* We fixed and issue in the logic editors where exposed microflows were not visible in the toolbox.
* We have added a consistency check to confirm compatibility between the imported Marketplace module [External Database Connector](https://marketplace.mendix.com/link/component/219862) and your Studio Pro version.
