...
* We fixed an issue where hidden aggregate columns of a datagrid would crash the page. (Ticket #145306)
* We fixed an issue in MySQL 8.0.29 and above when dropping a column with a name length over 53 characters long.
...
* We fixed a bug where button with no action attached don't trigger parent container action, when clicked.
