...
{{% button color="info" href="https://marketplace.mendix.com/link/studiopro/7.9.0" text="Go to Marketplace" title="Download version 7.9.0 from the Marketplace" %}}
...
* We won't reset application log levels to default anymore on the restart, stop, and re-deploy of your app from the Modeler.
...
* We fixed an issue where import mappings using `find by key` to find existing objects tried to also update the key attribute (or attributes) of the found object.
* <a id="58848"></a>If a text widget used an enumeration attribute as a parameter, the enum key was shown instead of its caption. This is no longer an issue. (Ticket 58848)
