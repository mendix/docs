...
A connection to an OPC-UA server is made using an IdentityToken, similar to a user role in Mendix. The server will dictate based on the response in GetEndpoints>UserIdentityToken>TokenType, which types of IdentityTokens it will support. 
...
### 4.2.2 Trust Server Certificate
...
### 4.2.3 Untrust Server Certificate
