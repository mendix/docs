...
Mendix supports functionality to extract a part of a date and time in a query. In XPath, you can use functions like `[hours-from-dateTime](/refguide6/xpath-hours-from-datetime)` and [`week-from-dateTime`](/refguide6/xpath-week-from-datetime). In OQL, you can use functions like `[DATEPART(..)](/refguide6/oql-datepart)` and `[DATEDIFF(..)](/refguide6/oql-datediff)`. In Mendix, DateTimes are stored in the UTC time zone. For a correct working of these functions, it is important that a database supports converting dates and times from the UTC time zone to another time zone. If this is not possible, the functions will operate on the date and time in the UTC time zone. That can lead to incorrect results if a user expects the date part to work in his time zone.
