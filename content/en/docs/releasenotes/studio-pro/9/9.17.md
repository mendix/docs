...
* We introduced a mechanism for app users to change the state of running workflows. It allows running workflows to jump to different activities, which are selected by the user. It also enables users to fix workflows in incompatible state, for instance, when one of the current activities was removed. For this functionality, we introduced the **System.WorkflowJumpToDetails**, **System.WorkflowCurrentActivity**, and **System.WorkflowActivityDetails** entities and [Generate Jump-to Options](/refguide/generate-jump-to-options/) and  [Apply Jump-to Option](/refguide/apply-jump-to-option/) microflow activities. You need the Workflow Commons version 2.3.0 or above to use this functionality. For more information on the jumping to different activities functionality, see [Jumping to Different Activities in a Workflow](/refguide/jump-to/).
