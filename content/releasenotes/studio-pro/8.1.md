...
We added the [mx command-line tool](/refguide8/mx-command-line-tool). This is a Windows and Linux command-line tool that can be used to convert the inputted app projects(s) to the version of Studio Pro with which the tool is bundled.
...
* Anonymous users are now localized using the user's OS language instead of the app project's default language. If the user's OS language is not recognized or not configured in the [Project Settings](/refguide8/project-settings#languages), the project's default language will be used.
...
* We fixed an issue in the Studio Pro App Store where clicking **Request info** for a Marketplace component led to an error message.
