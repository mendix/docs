...
| ---------------- | ---- | --------------------------------------------------------- | ------------------------------------------------------------ |
| URL property | This property is available both for [pages](/refguide/page-properties/#url) and [microflows](/refguide/microflow/#url). This is an out-of-the-box feature of Mendix Studio Pro and, since Mendix 10.6, the recommended approach for most users. | Mapping is clearly configured in the application model. No setup is required. | Certain complex behaviors available in DeepLink are not supported (see `IndexPage`, `LoginLocation` constants) |
