...
* We added support for [Studio Pro 8.16](../studio-pro/8.16). 
* We now show the limitations set on an external attribute. Limitations are set by the external service owner to indicate the features that are not supported.  For example, if there are limitations on an attribute, you will not be able to use it in filters and sort on it. 

#### Improvements

* We added a step introducing the **Help & Learning** feature to the guided tour. For more information on the feature, see [General Info](/studio/general).
