...
* If a role has two access rules for the same entity, one giving read only access, and another giving read and write access, do not make those access rules exclusive. Make the read only rule include the read and write rule. For example, if access level is based on a boolean attribute `Editable`, there should be two access rules: one granting read only access to all objects (no constraint) and another granting read and write access to some objects (constraint: `[Editable = true()]`). To avoid unnecessary complexity in the resulting SQL, do not add constraint `[Editable = false()]` to the first access rule.
