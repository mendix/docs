...
* [Page URLs](/refguide/page-properties/#url) got a major overhaul. Studio Pro now supports multiple page parameters, using attributes other than `Id` in the URL, and placing the variable segment(s) at any location in the URL.
...
* We updated the style for the following UI components: button, checkbox, checkbox list, date and time selector, drop-down menu, group box, link, radio button, splitter, stepper, label, text editor, text field. [**ASK TW OR DEV: stepper, splitter, date and time selector?**]
* We updated the style for the app selector screen, the tab control, the alert control (which displays alerts about merge conflicts), and the selection in text fields.
* We updated the sign-in response pages to include the new Mendix logo.
* We updated the icons of message dialog boxes.
* We added the `IMxRuntimeRequest.getRootUrl` method to the Runtime API. This method can be used to retrieve the root URL of the application.
...
* We changed the **Limit to current tab** and **Limit to uncommitted filters** options in MxAssist Performance Bot to only use the cause location when filtering. [**CLEAR? "cause location" ?**]
