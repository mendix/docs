...
{{% button color="info" href="https://marketplace.mendix.com/link/studiopro/10.12.6" text="Go to Marketplace" title="Download version 10.12.5 from the Marketplace" %}}

### Improvements
    
- We improved error handling on Delete Object action where it tries to delete a file which doesn't exist.
...
- We fixed an issue where an Oops dialog was shown when the [Data Widgets](https://marketplace.mendix.com/link/component/116540?_gl=1*jpe0m1*_gcl_au*MTg4MTk2NjAyMi4xNzIxODA1MDU2) module was updated to version 2.24.0 or later, when a Data grid 2 was configured with a column using the `owner` or `changedBy` system association.
