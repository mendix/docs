...
* <a name="101587"></a>We fixed an issue where the output of an [Import with mapping](/refguide/import-mapping-action) activity was incorrectly set to **Nothing**. (Tickets 101587, 101786, 102227, 102287, 102621, 102641)
* We fixed a rare issue where Studio Pro crashed when validating your [XPath constraints](/refguide/xpath-constraints). (Ticket 100658)
...
* We fixed a bug where users saw the mouse pointer on clickable containers, although the same users could not access the configured action. (Ticket 100744)
* We fixed an issue in [Call REST service](/refguide/call-rest-action) and [Call web service](/refguide/call-web-service-action) activities where setting the **Timeout** to empty gave an error pop-up window where it should have given a consistency error.
