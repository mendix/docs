...
In addition to the OData Service Domain Model, there is a domain model which is used by the SAP OData Connector to control the connection between your Mendix app and the SAP back-end. The domain model consists of entities and their relationships represented by associations.
...
The Create operation creates a new entity in the SAP back-end system using the OData service.
...
* Input
  * Query (required) - A URL which points to the Collection to which the entity belongs. The Collection also identifies as an Entity Set in the OData service.
  ...
  * Use SAP cloud connector - **False** if you want to use a publicly accessible service, **True** if you need to access your on-premise SAP back-end system via the SAP Cloud Connector
* Output
  * Return type - Boolean
...
**CRM_TASK.EntitySetNames.Tasks** is the name of the Tasks collection listed in the EntitySetNames enumeration of the OData Service Data Model.

The **Odata object** is an object of entity type **Task**. This can be created, by using, for example, the **Create Object** action.

#### 3.1.2 Delete

The Delete operation deletes an existing entity in the SAP back-end system using the OData service.
...
The service operations which are supported by the SAP service are listed in the enumeration **FunctionNames** which is imported as part of the data model of the OData service.
...
The Get Entry operation gets an existing single entity from the OData service.
...
```
This produces this GET request https://www.sapfioritrial.com/sap/opu/odata/sap/CRM_TASK/Tasks(guid'00505697-47E6-1EE7-BED1-6C5662A87345') and, if the task exists, returns it.
...
* Input
  * Query (required) - the query which will return a list of entities from the OData service. See section [4.1.1 Query](#Query) for more information
...
The Update operation changes the attributes of an existing entity in the SAP back-end system using the OData service.
...
This will automatically add the header to the RequestParams object.
...
Every Action will ask for a set of parameters. For example, **Get List**:
...
**@SERVICEROOT** is a constant which is created in the OData Service Data Model and has a value which is the root URL of the OData service, for example: https://www.sapfioritrial.com/sap/opu/odata/sap/CRM_TASK

**COLLECTIONNAME** can be found in the enumeration EntitySetNames which lists all the collections in the OData Service Data Model, for example: the collection Tasks will be shown as @SERVICEROOT.EntitySetNames.Tasks

**QUERYPARAMETERS** are the parameters of the OData query which identify which objects should be returned. Please note:

* The OData service will define which attributes can be used to filter and sort the result list; the metadata for the service will contain a boolean indicating whether an attribute is **filterable** and/or **sortable**
...
* **True** if you want the action to access your on-premise SAP back-end system via the SAP Cloud Connector
