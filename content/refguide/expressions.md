...
Starting with Studio Pro [8.10.0](/releasenotes/studio-pro/8.10), attributes of associated objects can be accessed using multiple slashes (for example, the `Number` attribute of a single associated `CRM.Order` is referred as `$customer/CRM.Customer_Order/CRM.Order/Number`).
