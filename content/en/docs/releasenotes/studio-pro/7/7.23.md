...
* We support the SameSite parameter in HTTPS cookies. This setting can be configured using the `com.mendix.core.SameSiteCookies` runtime setting.

### Fixes

* We added the custom setting `RequestHandling.AllowLegacyCookies` to allow violations of RFC 6265. See https://www.eclipse.org/jetty/javadoc/jetty-10/org/eclipse/jetty/http/CookieCompliance.html#RFC6265_LEGACY for more information. Be aware of the fact that enabling this custom setting exposes your app again to CVE-2023-26049.
...
* We fixed an issue in published OData services where the service feed did not have a title for the workspace, even though that is required. (Thanks to [Ronald van Puijenbroek](https://forum.mendixcloud.com/link/ideas/1357) for pointing this out on the Mendix Idea Forum!)
...
* We fixed an issue where published REST services were always be reported as unused. Now they are unused when they have no resources.
    ...
    {{% alert color="info" %}}This is based on an [upvoted idea from Edwin van Elk](https://forum.mendixcloud.com/link/ideas/1302) submitted to the Mendix Idea Forum. Thanks, Edwin!{{% /alert %}}
...
* No need to wonder what [Marketplace](https://marketplace.mendix.com/) widgets you are using in your project and to find a way to check if you have the latest version anymore! In **Project** > **Tools** we added the **Update Widgets** setting which gives you an overview of the widgets you are using, what their current version is in your project, and what the latest version is. Moreover, you can now select several widgets and update them by simultaneously clicking **Update**.
    ...
    {{% alert color="info" %}}This is based on an [upvoted idea from Nolan Ramsey](https://forum.mendixcloud.com/link/ideas/1008) submitted to the Mendix Idea Forum. Thanks, Nolan!{{% /alert %}}
