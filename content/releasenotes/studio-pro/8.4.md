...
#KI: "Not all one-to-one association values" = UICORE-461
...
* **KI: "Dragging and dropping a microflow" = fixed (#lc310) - link to fix note in old RN**
...
* You can now use SVG for static images in your native application.
* We exposed a new native **Image** widget that you can use in your native pluggable widget that will enable support for SVGs.
* In native mobile apps, you can now use custom icons in the bottom bar.
* In native mobile apps, SVG icons are now supported on button widgets.
...
Dragging documents around within your app project now provides better visual feedback on whether the selected items can be dropped. Also, when dragging a document next to the edge of the Project Explorer, the list of modules and folders is now scrolled.
...
We rebuilt the [Project Explorer](/refguide/project-explorer) from the ground up. Should you find any issues, you can switch back to the old version via **Edit** > **Preferences** > **New features**.
...
* If you ever wanted to perform some custom client-side logic in your JavaScript action and then call a nanoflow from your code, then we are happy to say that now you can. We have enabled configuring and calling nanoflows from your JavaScript action code directly. Thank you for adding your idea to the [Mendix Idea Forum](https://forum.mendixcloud.com/link/ideas).
...
* The default way to run your app is to run locally now. In addition, the <kbd>F5</kbd> shortcut key now always runs locally.
	...
	{{% /alert %}} 
* We improved performance of committing thousands of entities in a microflow. (Tickets 89267, 89279, 90357)
...
* We reverted improvements to the handling of one-to-one associations with **Owner** type **Both** to the state of [8.2.2](8.2#69877). This is to fix an issue with the synchronisation of the state of autocommitted objects between runtime and client (for example, when uploading documents and images). (Tickets 69877, 83548, 83911, 89712, 91128)
...
* Monitoring client state using the <kbd>Ctrl</kbd> + <kbd>Alt</kbd> + <kbd>G</kbd> shortcut now also works for Mac users. However, there is a limitation: it does not work for Firefox users within Parallels. In this case, you either need to use Firefox on Windows or use a different browser in Parallels.
...
* Not all [one-to-one association](/refguide/generalization-and-1-1-association) values are communicated correctly from the Mendix Runtime to the Mendix Client. This could lead to values that seem to be missing in views, but which are present after a reload.
