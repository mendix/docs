...
- We expanded the **Page URLs** query in the **Find Advanced** dialog box with support for microflow URLs. This query now finds all pages and microflows that have a URL specified. Because of this the query has been renamed to **Page and microflow URLs**. {TW}
...
* In the logic editors, we fixed centering an object selected from the problems panel or **Debugger** pane when the document was not open yet. {TW}
...
- We fixed rebase issue that occurred when there was no *user.name* and *user.email* set in *.gitconfig*.
...
* We fixed an issue where the [USERTASKSTARTED](/refguide/workflow-events/#activity-event-type) event was not triggered when a workflow user task fails during [Xpath based targeting](/refguide/user-task/#242-xpath-constraint).
