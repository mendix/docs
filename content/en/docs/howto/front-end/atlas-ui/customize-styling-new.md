...
[Cache busting](https://www.keycdn.com/support/what-is-cache-busting/) is the principle where the browser is told by the web server to re-download page resources such as images, stylesheets or JavaScript, for example, because of changes in these resources. Mendix automatically takes care of this by adding dynamic query parameters on top of the resources in login.html and index.html. Here is an example of the auto-generated cachebust query parameters in a line from **/deployment/web/index.html**:
...
```
To ensure cache busting keeps working, whenever there is a need for customisation of *index.html* or *login.html*, make sure that these files are copied according to the recommendation in paragraph 9 above. Whenever cache busting breaks, it is likely that the query parameters have become hard coded (e.g. *?638184496048312490*) instead of dynamic (e.g. *?{{cachebust}}*) due to copying ~~**/deployment/web/index.html**~~ instead of **/deployment/web/index-example.html**.
