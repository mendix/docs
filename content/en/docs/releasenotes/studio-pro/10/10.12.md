...
* We fixed the issues in the **Open App** dialog box that sometimes a Team Server app would not be listed, or that sometimes the application icon would display the default image instead of the customized image. (Tickets 205781, 205798)
...
* We fixed an issue where a microflow database retrieve action with range "first" would take longer to run than a custom range with limit of 1. (Ticket 224668)
* We fixed an issue where selecting an argument to fix the consistency error "no argument selected for parameter {Name} and no default is available" resulted in an Oops dialog.
