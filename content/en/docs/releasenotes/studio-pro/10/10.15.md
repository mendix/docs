...
X-Ray mode is generally available now for both Windows and macOS and is enabled by default. X-Ray mode is a way to better visualize the structure of your page while in Design Mode. It adds a special border around certain widgets, like layout grids and containers, while keeping most of your apps styling the same. For more information, see [X-Ray Mode](/refguide/page/#x-ray-mode).
...
- We added new support option under **Help** menu for collecting version control data. {TW}
...
- We fixed an issue where building failed when an app that was created with StudioPro 9.24.0 or below containing jar exclusions is opened with newer versions. (Ticket 227102) {TW}
...
- Disable highlighting text on right-click to text in logic editors. {TW}
