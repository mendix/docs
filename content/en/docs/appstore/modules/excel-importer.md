...
* Support reading data from Macro enabled Excel files (xlsm)
...
### 5.9 When importing larger Excel files, the JVM crashes (java.lang.OutOfMemoryError: Java heap space). How Can I work around this problem?

Generally, the OutOfMemory Error indicates that the system was unable to create new objects on the Heap. While processing large excel files you may encounter this error and eventual JVM crash. To work-around this issue, you can
* Slice the input data into manageable smaller chunks and import the file(s) individually.
* In case slicing the data is not an option, then ramping up the container size (and resources) is the way to go.

Our internal tests indicate that JVM's memory footprint peaks at 3GB while importing an Excel file ~9MB size having approximately 25,000 rows and each row containing 40 columns into a single entity. Please note that this is JVM's memory (RAM) and not container's. Also, the memory requirements will increase if associations are also getting processed as part of Excel import; so the above numbers should be taken as a *rule of thumb* and a suitable buffer should be provisioned while sizing the container.

### 5.10 After upgrading to the latest version of Excel Importer, userlib contains 2 files of the same JAR?

It is possible that you may see, for example, *commons-io.commons-io-2.11.0.jar* and *commons-io-2.11.0.jar* in the userlib folder after upgrade. This is because the naming convention of the JARs is aligned to just have the file name. In such cases, you can safely delete the JAR having package prefix; so you can remove *commons-io.commons-io-2.11.0.jar*.
