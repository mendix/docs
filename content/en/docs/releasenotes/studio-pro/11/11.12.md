...
* <a id="ki-guids-are-not-restored"></a> When a domain model entity is deleted and recreated with the same name (by either a module publisher or a consumer), the original storage GUIDs are not restored. This means that the new entity is treated as a brand-new entity rather than a continuation of the deleted one, which may result in data loss or schema drift in the consumer's database.
