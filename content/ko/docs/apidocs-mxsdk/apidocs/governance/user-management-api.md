...
description: "이 API는 지원이 중단되었습니다. 이 API를 사용하면 회사의 사용자 관리 시스템을 Mendix 플랫폼과 통합할 수 있습니다. 사용자 계정과 그룹을 생성하고 관리하며, 앱에 대한 그룹 정책을 정의할 수 있습니다."
weight: 115
restapi: true
deprecated: true
---
...
User Management API는 Mendix 계정에 바인딩된 API 키를 통한 인증이 필요합니다.
...
User Management API를 통해 작업을 수행하려면 **API 권한** 권한이 필요합니다. This can be configured from the [Permissions tab](/developerportal/deploy/node-permissions/#permissions-tab) of your app's **Environments** page.

## 오류 처리

REST 요청 수행 중 오류가 발생하면, the server will always return a JSON object with error information. For example:
...
같은 회사 내의 사용자 그룹입니다. Automated security policies can be assigned to groups. These groups typically reflect your organization's structure.

### 사용자 계정의 OpenID 조회

이메일 주소를 기반으로 사용자 계정의 OpenID를 조회합니다.
...
회사의 사용자 계정을 활성화/비활성화합니다. Deactivating a user means that the user can no longer access the Mendix Platform. However, all data of this user is retained.
