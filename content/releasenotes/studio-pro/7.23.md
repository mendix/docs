...
* We fixed an issue in published REST services where upgrading a published REST service from version 7.10 to 7.13 or above set the authentication method to No authentication.
* We fixed a drawing error in the domain model editor that displayed a big red cross if associations had a length of zero. (Tickets 85602, 85421, 85308)
...
* We fixed setting values of owner and creation date attributes when a new object created. (Ticket 83424)
* We fixed an issue where CPU usage becomes too high. If critical error messages are logged EchoAction returns only the first 10 of them. (Ticket 82162)
* We fixed an issue where pasting a Model Share into the System module resulted in an error.## 7.23.6 {#7236}
...
* We fixed an issue in XML import mappings. We no longer allow `DOCTYPE` declarations in the XML input, because that is potentially unsafe. (Ticket 83370)
...
* If a domain model is drawn which contains an *association of length 0px*, you cannot open the domain model when it is migrated to version 7.23.6. For more information see this [Forum question](https://mxforum.mendix.com/link/questions/94784). (Ticket 85976)
