...
- We have below addition to External Database Connector 
    ...
    - Supports connecting to Snowflake using Key-Pair Authentication.
    - Passing NULL values to the Parameters is now possible.
    - Connection detail constants are now saved in Active Configuration of user, also Password and Private Key are now stored as Private values.
    - Queries in the database connection document are mergeable.
    - We have added logging for Database Connection document.
    - For design time, Oracle connection string generation is improved.
