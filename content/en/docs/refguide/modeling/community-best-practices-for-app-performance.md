...
* Use retrieve over association if possible. This ensures that you have the latest version of your entity objects, including any changes which are not yet committed. In addition, where possible the runtime will [take the associated object or objects from memory](/refguide/mendix-client/#object-cache) so it can be faster. If business logic requires the database value (because you want to ignore changes to the value over association), then you will need to make a database retrieve.
