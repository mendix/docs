...
#ki: "If you create a new app" = WT-1587
...
* Native apps now support the translation of pages.
* Snippets can now be used on native pages and layouts. To enable this, set the new `Type` property to `Native`.
...
* You are now able to see enhanced error messages.
...
* We removed the following methods that do not have alternatives, as we determined there were no valid use cases for them: `MendixHashString.setInitialHash`, `IContext.addFeedback`, `IContext.getFeedbackList`, and `Configuration.getConstantValue(Object component, String key)`.
...
* We dropped support for the `Float` and `Currency` types.
* We removed the `code`, `permanent`, `eden`, `survivor`, and `tenured` fields from the `memory` section of the `runtime_statistics` admin action's result. They had already been reported as `0` on Mendix 7.
...
* If you create a new app with Mendix 8.0.0 Beta 3, you will see errors concerning missing CSS files. This is because we made several [improvements](#dom-atlas-upgrades) to the DOM and Atlas UI which affect styling. To solve these errors, update your app's Atlas UI Resource Module to the [latest version](https://appstore.home.mendix.com/link/app/104730/). If you are using a customized starter app, see [Troubleshooting Atlas UI Changes](/refguide/migration-atlas) for guidance on where to put your custom styling. To use a starting point without errors, click **File** > **New Project** and select the **Blank** starter app which is on the **last page** of **Starter Apps**.
...
### New Features (Already Released via Beta Versions)
...
The Mendix **Make It Native** app is available from [Google Play](https://play.google.com/store/apps/details?id=com.mendix.developerapp) and [Apple App Store](https://apps.apple.com/app/make-it-native/id1334081181). We will continue to improve and extend the functionality. However, you may encounter some limitations:

* Mendix native mobile apps can only be viewed using the Make It Native app. We will add support to create stand-alone apps (for example, Android APK or iOS IPA) that can be distributed using your preferred distribution channel (for example, Google Play or the Apple App Store).
...
### Improvements (Already Released via Beta Versions)
...
### Fixes (Already Released via Beta Versions)
...
### Upgrades (Already Released via Beta Versions)
...
### Deprecations (Already Released via Beta Versions)
...
### Breaking Changes (Already Released via Beta Versions)
...
### Limitations (Already Released via Beta Versions)
