...
| Not receiving push notifications on a PWA | Push notifications on PWAs are only shown when the app is not open | Close the app and try again |
| Not receiving push notifications on a PWA on iOS | iOS PWAs require https to run | Deploy your app and try again |
| Clicking on a push notification does not open the PWA | Push notifications for PWAs require a WebLink to be set. | Make sure the WebLink is set to a valid path of your application (for example "/") |
