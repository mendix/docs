...
* MxAssist Logic Bot [beta] now does the following: [**TW REVIEW AND FOR `[beta]`**]
    ...
    * Excludes recursive calls for calling MCAs, NFs, and rules (consistent with MCA). [**TW: WHAT ARE MCA AND NF?**]
...
* We changed the icons for and the background color of loops in the microflow, nanoflow, and rule editors. [**TW**]
...
* MxAssist Logic Bot [beta] bug fix: the generic recommendation for Create Variable was incorrectly creating a Change Object action activity.
* MxAssist Logic Bot [beta] bug fix: the generic recommendation for Show Message was incorrectly creating a Show Page action activity.
* MxAssist Logic Bot [beta] has minor fix for sorting of Microflow Call recommendations.
* MxAssist Logic Bot [beta] bug fix: Delete Object recommendation was excluding some variables according to their type - this constraint has been removed.
* MxAssist Logic Bot [beta] bug fix: applying a List Operation recommendation should create the List Operation with its variable set.
* MxAssist Logic Bot [beta] bug fix: the initial menu could be missing its first item.
* MxAssist Logic Editors [Beta] We fixed the output descriptions of the Aggregate list actions
* MxAssist Logic Editors (beta): We fixed updating Loop contents while the loop is being dragged.
...
#### Removal of Database Data Source
