...
*  To assist you in fixing consistency error, the link to [documentation](/studio/consistency-errors) was added to the **Checks** panel. Also, warnings and info messages are not displayed in the **Checks** panel anymore. For more information, see [Checks](/studio/checks) in the *Studio Guide*. 
...
* We fixed an issue where model versions 7.23.4 and below could no longer correctly load the project's custom widgets, hiding them from the toolbox completely.
