...
* Use the retrieve over association if possible. This [takes the associated object or objects from memory](/refguide/mendix-client/#object-cache) so it is faster and you retrieve any changes made. If business logic requires the database value (because you want to ignore changes to the value over association), then you will need to make a database retrieve.
