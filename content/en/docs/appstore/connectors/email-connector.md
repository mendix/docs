...
The [Email](https://marketplace.mendix.com/link/component/120739) connector allows you to send and receive emails on your own email server. It includes features such as sending emails using template, sending signed and encrypted emails.
...
To configure an OAuth provider for the client credentials grant flow, provide the following details, which are available on the Azure portal once you have registered your app. With EC 5.2.0 onwards, it is now possible to send emails using client credentials flow.
