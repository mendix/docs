...
* Mendix Private Cloud Architecture - The system leverages the Mendix for Private Cloud (Mx4PC) deployment model. As validated by [Mendix on Kubernetes](/developerportal/deploy/private-cloud/#standalone-architecture), all core service components, including the Mendix Operator and application Runtime, are deployed locally within the customer-managed Kubernetes environment.
