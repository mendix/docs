...
* Mircoflow actions related to [workflows](/refguide/workflows) now have help links to the documentation.
...
* You can now use the [reference selector](/refguide/reference-selector) widget inside a [containment or repeatable containment](/apidocs-mxsdk/apidocs/property-types-pluggable-widgets#widgets) pluggable widget (except when you use both [Constrained By](/refguide/reference-selector#selectable-objects) and [Select using](/refguide/reference-selector#select-using) > **Drop-down**. 
...
* We fixed an issue where extra new lines were added to the end of an expression when typing in the expression editor. (Tickets 101033, 103209, 105990, 107049, 107239, 114492, 115807, 115894, 116692, 116963, 118606, 119602, 122468)
...
* We fixed an issue where temporary files and folders (like **GPUCache**) were created in the app directory, which led to unnecessary merge conflicts. (Tickets 105909, 106329 and 107857)
...
* When you convert an app from Studio Pro 8 to 9 and merge a feature branch that has already been converted, you might see an error message when committing stating that the file inside the **theme** or **themesource** folder already exists.
	* Workaround: Commit the app conversion to version 9 before performing a merge. [**SDM VERIFIED? IF SO, ADD KI LABEL, KEEP HERE, ADD TO 9.0-9.2: https://mendix.atlassian.net/browse/SPVC-1190**]
