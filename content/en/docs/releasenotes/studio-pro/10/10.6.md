...
* We made changes to provide more error and label consistency in the microflow and workflow actions form when a variable is deleted (if it was being used). [**TW**]
...
* In the logic editors (beta), we fixed the error handler connection points on activities that have custom error handlers.
* In the logic editors (beta), we fixed showing the MxAssist Logic Bot suggestion flow with the error handler.
* In the logic editors (beta), we fixed an issue with MxAssist Logic Bot not opening on a newly created microflow, nanoflow, or rule.
...
* We fixed a bug where the data source of widgets was missing as a parameter when creating a new microflow or nanoflow for the [Call microflow](/on-click-event/#call-microflow) or [Call nanoflow](/on-click-event/#call-microflow) client actions.
...
* We removed the deprecated **Import Web Service or XML Schema** wizard that was accessible in the domain model editor when you clicked **Import web service/XML file**. [**TW: ANNOUNCED IN 10.0**]
