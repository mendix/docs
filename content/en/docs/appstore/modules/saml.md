...
You can usually leave the key store settings as the default. The SAML module will generate distinct key pairs for signing and encryption. For version 3.5.0 and above, there are separate key pairs generated for each IdP, for versions below that, the self-generated key pairs are used for all IdPs, if your app is configured to use multiple IdPs.

However, there may be a requirement to use a specific key store.

{{% alert color="warning" %}}
To use different key pairs for communication with different multiple IdPs, you will need to use Mendix version 9 and SAML module version 3.5.0 or above.
{{% /alert %}}
