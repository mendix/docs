...
*  To avoid confusion we limited the messages in the **Checks** panel to only display errors. Warnings and info messages are not displayed anymore. For more information, see [Checks](/studio/checks) in the Studio Guide. 

*  To prevent you from creating filters with consistency errors, we added validation in filters. That means that a filter with errors cannot be saved and you will immediately see what the error is. 
...
* We fixed an issue where it was no longer possible to create new custom widgets. 
* We fixed an issue where older model versions could no longer correctly load the project's custom widgets, hiding them from the toolbox completely.
