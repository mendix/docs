...
1. Configure the service by filling out the following:
    ...
    * **Event Broker Space** – the space where the bridge will operate
    * **Name** – the name you want to call the bridge
    * **Choose a data lake** – select the underlying data lake that Iceberg will use to physically store your data.
        * **Azure Blob Storage**
            * **Storage Account** – the storage account to access
            * **Storage SAS Token** – the SAS token used to access the storage account
            * **Container** – specify the container where the data will be stored
        * **AWS S3**
            * **Bucket** – specify the bucket where the data will be stored
            * **AWS Region for Bucket** – the AWS region where the bucket is located
            * **AWS Role (to Assume)** – the AWS role to assume to obtain the required permissions

    {{< figure src="/attachments/appstore/platform-supported-content/services/event-broker/bridges/iceberg/ebb_iceberg_configure.png" width="400" >}}

#### Connect Events

Select the CDC events that you want to deliver from the Event Broker to Iceberg:

1. Click **Add CDC Events** to open a dialog displaying the available CDC events managed by the Event Broker. Only events that can be subscribed to are shown because the bridge delivers events in one direction, from the Event Broker to Iceberg.

2. Select one or more CDC events to integrate. The configuration for the Iceberg connection is automatically generated based on the selected events.

    {{< figure src="/attachments/appstore/platform-supported-content/services/event-broker/bridges/iceberg/ebb_iceberg_connect_events.png" width="400" >}}

    {{< figure src="/attachments/appstore/platform-supported-content/services/event-broker/bridges/iceberg/ebb_iceberg_choose_cdc_events.png" width="400" >}}

3. Click **Next** to review and confirm the bridge configuration.

#### Overview of Iceberg Bridge

After the Mendix Event Broker Bridge has been successfully created, you can view its configuration and current status on the **Overview** page.

{{< figure src="/attachments/appstore/platform-supported-content/services/event-broker/bridges/iceberg/ebb_iceberg_overview.png" class="no-border" width="400" >}}

Once the bridge is created, check its **Bridge Status** on the **Overview** page to verify that it is ready and operating as expected. The following list describes the possible bridge statuses.
