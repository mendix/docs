...
{{% alert color="info" %}} The following page describes the steps needed to configure the connection between your Mendix app and your Teamcenter instance using Teamcenter Connector 2512.0.0 and above. For information applying to Teamcenter Connector 2506.0.0 and below, refer to [Configuring the Connection to Teamcenter 2506.0.0 and Below](/appstore/industry/teamcenter-connector/configuring-connection-2506/). {{% /alert %}}

## Connecting Your Mendix App to Teamcenter

Follow the steps in this section to connect your Mendix app to Teamcenter.
...
{{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/navigation.png" alt="Navigation menu showing the NAV_AdminHomePage button" >}}
...
2. Use the **NAV_AdminHomePage** button to navigate to the overview page of all Teamcenter configurations.
    ...
    {{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/teamcenter-new-configuration.png" alt="Teamcenter configurations overview page with the New configuration button" >}}
...
If you want to use SSO with user provisioning for anonymous users, click **Teamcenter SSO** on the login page, as described in the [User Provisioning for SSO](#user-provisioning-for-sso) section.
{{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/teamcenter-sso-button.png" alt="Login page showing the Teamcenter SSO button" >}}
...
3. Click **NAV_userLogin** to log in to Teamcenter.
  ...
  {{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/navigation-login-user.png" alt="Navigation menu showing the NAV_UserLogin button" >}}
...
Work with your CApS representative to register your application to Teamcenter X SSO.
...
These are the two scenarios you can choose from:

* Your users already have a Mendix account, and should sign in to Mendix before connecting to Teamcenter. Mendix user management and Teamcenter user management are maintained separately.     
    This scenario is implemented by the `EXAMPLE_UserProvisioningNamed` example.    
...
* Your users should be able to start from the Teamcenter SSO button on the login page, without an existing Mendix account. A Mendix account is created automatically from the Teamcenter login upon first use.     
    This scenario is implemented by the `EXAMPLE_UserProvisioningAnonymous` example.
...
If neither example fully matches your app's user management rules, Mendix recommends the following approach rather than writing a microflow from scratch:
...
2. Adjust the copy to your app's needs, such as changing username mapping, assigning roles, or adding validation.
...
The Teamcenter Connector provides an example that allows you to overwrite the `login.html` file with one that contains the SSO login button. You can find it in your `resources\TeamcenterConnector` folder.     
...
This guide provides abbreviated instructions on how to register your Mendix app. Configuring these settings requires assistance from your Teamcenter security expert. Refer to the configuration steps in the Siemens Support Center for Teamcenter Security Services. 

To register your Mendix app, follow these steps:
...
5. Scroll down to the **SSO Application Ids** section, and note the **Teamcenter SSO Application ID**.
  {{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/teamcenter-sso-application-id.png" alt="SSO Application IDs section in the Teamcenter Deployment Center" >}}

6. Scroll down to the **TcSS Login URL Settings** section, and note the **Login Service URL**.
  {{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/teamcenter-login-service-url.png" alt="TcSS Login URL Settings section showing the Login Service URL" >}}
...
7. Scroll down to the **Teamcenter Application Registry** section, and add a new line for each environment of your application. You generally need at least `localhost` for development and `production` for deployment:
    ...
    * Teamcenter Extension example:
        ...
        * `LDAP_USERNAME: uid`
  {{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/teamcenter-application-registry.png" alt="Teamcenter Application Registry section with example entries" >}}

8. Scroll down to the **TcSS Identity Service URL Settings** section, and note the **Identity Service URL**.
  {{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/teamcenter-identity-service-url.png" alt="TcSS Identity Service URL Settings section showing the Identity Service URL" >}}

You are now ready to set up your configuration in Mendix. If you encounter any issues with the SSO registration of your app, contact your Teamcenter security expert.

### Registering Your App for Teamcenter X SSO {#register-your-app-for-teamcenterx-sso}

Work with your CApS representative to register your app for Teamcenter X SSO. Your CApS representative can provide you with all the relevant information to configure your Teamcenter X SSO settings in Mendix.

## Troubleshooting

### Troubleshooting SSO

Setting up SSO can be a complex procedure. Check the following if you are unable to log in via SSO:
...
* Set up SSO on your local machine first before working on a deployed version. To do so, you need to register for local development ([Teamcenter SSO](#register-your-app-for-teamcenter-sso) or [Teamcenter X SSO](#register-your-app-for-teamcenterx-sso)).    
    ...
    If that does not solve the issue, see the [Troubleshooting SSO on Hosted Environments](#troubleshooting-sso-on-hosted-environments) section for specific deployment issues.
...
* Answer 1: Yes.    
    Solution: Continue to log in and go to Scenario 2.
...
* Answer 2: This page:
  {{< figure src="/attachments/appstore/platform-supported-content/modules/siemens/teamcenter-connector/configuration/teamcenter-error-page.png" alt="Teamcenter error page displayed after a failed SSO login" >}}    
...
The SSO setup of the Teamcenter Connector uses deep links to access the Mendix app via the following paths:
