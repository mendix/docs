...
* We fixed an issue with the new `{{themeCss}}` placeholder, where the css files would not be inserted when deploying your application. (Ticket 67200)
...
Session management is broken for applications running in a clustered environment using Mendix 7.17.0. Customers running Mendix in clustered mode should therefore not upgrade to this version. The problem was fixed in 7.17.1 (see above).
