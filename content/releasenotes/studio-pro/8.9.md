...
* We have introduced `Core.http().registerHttpRequestInterceptor(...)` in custom Java actions. You can use this method (typically during application startup) to add functionality for intercepting REST and web service calls and then, for example, logging them or adding a signature header.
...
* In the [new Changes pane](/refguide/preferences-dialog#new-changes), the columns for document **Status** and document type can now also be sorted on the icon.
* Also in the new [Changes](/refguide/changes-pane) pane, we updated the behavior and text of the **Show 25 more** button in the **History** dialog box. This button is now called **Show more**, and it gets disabled when there are no more commits to display. (Ticket 95502)
...
* We made the timestamp time for an unversioned deployment package more consistent, so the exported file name is always the same as the one shown on the **Create Deployment Package** dialog box.
...
With this release, we roll out a more significant upgrade of the underlying technology layer by upgrading [React Native](https://reactnative.dev/) to version 0.61 and by upgrading all native libraries to their most recent versions. This improves the capabilities, quality, and stability of native mobile apps built with Mendix.
...
The [Make It Native](/refguide/getting-the-make-it-native-app) app is always compatible with the latest version of Mendix. Due to the changes in native libraries, the [version](ADD CROSS-REFERENCE TO MAKE IT NATIVE RN) of Make It Native that we have published alongside the release of Studio Pro 8.9.0 is no longer compatible with older Studio Pro versions.
...
* For Android, we have made a version of the Make It Native app available that you can use with Studio Pro version 8.8 – for details, see [UPDATE TITLE/LINK](/howto/mobile/make-it-native-app-archive).
