...
* We fixed an issue where you wouldn't be redirected to the page you were initially trying to access after login in via a modeled login page. (Tickets 205091, 212002)
...
* We fixed an issue where the Merge bar did not appear after the solution upgrade, so it was impossible to commit without a Studio Pro restart. (Ticket 206204) {TW}
...
* We fixed an issue when the document switcher did not work on Windows if a microflow, nanoflow, rules or workflow editor was opened. (Ticket 211576) {TW}
...
* We changed the default setting of the Best Practice bot to disabled by default. The user can change this setting in the current session, but the bot will be disabled again in the next startup of Studio Pro. {TW}
