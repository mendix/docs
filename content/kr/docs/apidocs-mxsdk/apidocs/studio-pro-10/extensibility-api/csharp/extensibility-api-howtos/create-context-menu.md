...
Studio Pro의 `IEntity` 또는 마이크로플로우 및 페이지와 같은 `IDocument`에 컨텍스트 메뉴를 추가할 수 있습니다. 이러한 컨텍스트 메뉴는 해당 메뉴를 포함하는 확장 프로그램의 이름을 딴 메뉴(예: `MyExtension`) 아래에 나타나며 관련 항목을 수정할 수 있습니다. 이는 확장 프로그램을 생성할 때 유형을 지정하여 수행할 수 있습니다.
...
위 코드는 모든 엔티티에 대한 일련의 컨텍스트 메뉴 항목을 생성합니다. 메뉴를 통해 다음을 수행할 수 있습니다:
...
{{% alert type="info" %}}
`IEntity` 유형이 전달되므로 컨텍스트 메뉴는 엔티티에만 적용됩니다. `MenuExtension.cs`와 동일한 로직을 사용하여 메뉴를 추가합니다. 더 많은 예제는 [C#을 사용하여 메뉴 확장 프로그램 만들기](/apidocs-mxsdk/apidocs/csharp-extensibility-api-10/create-menu-extension/)를 참조하십시오.
{{% /alert %}}

## 문서에 컨텍스트 메뉴 추가

문서(Document)에도 컨텍스트 메뉴를 추가할 수 있습니다. 이렇게 하려면 컨텍스트 메뉴 확장에서 `IDocument` 유형을 지정하십시오.
