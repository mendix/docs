...
* We optimized the `$currentUser` variable in microflows, now the current user will not be always retrieved from the database before the microflow is executed. This means that the current user in a microflow might not be completely up-to-date. If you require an up-to-date current then you can retrieve it manually from the database using the `$currentUser`.
