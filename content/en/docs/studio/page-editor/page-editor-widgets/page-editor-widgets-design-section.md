...
| Wrap Options | Wrap a long text into multiple lines.                        | Possible values for **Wrap Options** are the following:<ul><li>None – text is not wrapped. Wrapping depends in what widget the text is placed, for example, when it is in a column, it wraps between words but tries to take the space it needs if the text contains long words.</li><li>Wrap – not only does line breaks between words but also breaks words into multiple lines if needed.</li><li>No Wrap – avoids wrapping by all costs and forces everything into one line, no matter if it fits and whether there are line breaks.</li></ul> |
