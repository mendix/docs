...
{{% alert type="info" %}}
The [Nodes](/developerportal/deploy/node-permissions) screen in the Developer Portal shows all the licensed apps which are returned by this request, but does not show any Free Apps, while the [My Apps](/developerportal/apps-list) screen shows both licensed apps and Free Apps.
{{% /alert %}}
