...
* We improved the native widgets by adding disabled properties and styles to give end-users better visual feedback on whether a widget is disabled or not. For more information, see [Native Mobile Styling](https://docs.mendix.com/refguide/native-styling-refguide).
...
* We moved **Enable Private Version Control** settings from the **Advanced** tab in [Preferences](/refguide/preferences-dialog) to the new **Version Control** tab.
* We moved the section for selecting your preferred file comparison tool into the new **Version Control** tab.
...
* We fixed an issue with native mobile apps where setting the size of some SVG images would not work. (Ticket 103817)
...
* When you update a consumed OData service with a new version from [Mendix Data Hub](https://hub.mendix.com) but close the document without saving, the blue arrow icon will no longer be shown to notify you about the available update for that service.
