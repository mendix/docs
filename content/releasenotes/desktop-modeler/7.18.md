...
* In this version, we introduced a means to prevent entity instances from being committed with invalid autonumber values (see the fix for ticket [63071](#63071)). This happens when the user has no access rights to the AutoNumber attribute and the entity instance has to be sent to the client before its initial commit. However, this fix causes the commit to fail in the runtime with an exception stating "Value of auto-number attribute 'AttributeName' not available, could be caused by insufficient access rights to serialize the Mendix object to the client." As a result, the Mendix application fails for end-users. (Ticket 69757)
* There is a known issue with PostgreSQL support where transactions will fail on the error 'out of shared memory'. Workaround for on premises is to increase the max_locks_per_transaction in the postgreSQL configuration. (Ticket 68224)
* There is a known issue where String values are not implicitly converted to numerical values. Among others, this leads to errors during XML imports where entities have a numeric value as key(Ticket 69811)
