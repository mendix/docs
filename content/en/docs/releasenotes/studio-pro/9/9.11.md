...
* We removed microflow information from *metamodel.json*. Microflow information is now only available in the first session request (`get_session_data`) and only contains the microflows to which the user has access.
