...
| <a id="CSPHeaderTemplate" href="#CSPHeaderTemplate">CSPHeaderTemplate</a> | The `Content-Security-Policy` header that is returned when loading HTML files. The `{{NONCE}}` template variable can be used to return a unique nonce in the header. See [Content Security Policy](/howto/security/csp/) for more details. <br /> *This setting was introduced in Mendix version 10.13.0.*  |  |
