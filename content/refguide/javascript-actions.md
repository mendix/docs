...
The code has three sections, import list, extra code block and user code block. All code that is added should be in theise blocks. Code outside the blocks will lost when re-generating the template code on deploy or update of the JavaScrot action settings. Additional imports should start with `import` and be placed above the `// BEGIN EXTRA CODE` the extra code should be place between `// BEGIN USER CODE` and `// END USER CODE` and the user implementation code should be place between `// BEGIN EXTRA CODE` and `// END EXTRA CODE`.
