...
* We added an error page in the sign in dialog when studio pro cannot open the default browser of an user.
...
* We added the custom setting `RequestHandling.AllowLegacyCookies` to allow violations of RFC 6265 which is enforced in the most recent Jetty versions. See https://www.eclipse.org/jetty/javadoc/jetty-10/org/eclipse/jetty/http/CookieCompliance.html#RFC6265_LEGACY for more information. Be aware of the fact that enabling this custom setting exposes your app to CVE-2023-26049.
...
* We fixed missing deprecations for the `Counters`, `Gauges`, `Sets` and `Timers` interfaces from the public API. These interfaces will be removed in Mendix 11. Implementations for these interfaces were only accessible through deprecated methods before.
