...
* We fixed an issue that occurred with selecting a page of the [reference selector](/refguide/reference-selector) not working properly for specialized entities when the user had no access to the generalized entity. (Ticket 128755)
...
* File documents are no longer cached by default. You can re-enable this by using the [custom runtime setting](/refguide/custom-settings) `EnableFileDocumentCaching`. Images are still cached as they were before. (Ticket 126829)
* We fixed a known issue from 9.6.0 where the main line of a Git project could be incorrectly determined. This also fixes branch switching functionality in Branch Manager.
* We fixed an issue where Branch Manager could incorrectly show branch lines of an uninitialized app.
* We fixed the issue in the access rule dialog that the selected access rights would get lost when scrolling through the list. (Tickets 131581, 131605, 131961, 131976, 132074 and 132080)
* We fixed an issue when retrieving data for a data view listening to a list view. (Ticket 129494)
