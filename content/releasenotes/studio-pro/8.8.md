...
#KI: "Calling a microflow" = OFF-309
...
* Fixed an issue where an offline microflow call would crash if the parameter contained a reference to a parameter that was not being used in the microflow.
...
* Calling a microflow from a nanoflow in an offline app with a non-persistable entity that has an association to another non-persistable entity that is not used in the microflow will cause an error. (Solved in 8.8.1)
	* Workaround: You can either use all associations in your microflow or have no associations in the non-persistable entity.
* The first deployment of an app project with a native mobile profile takes approximately one minute longer than usual, as the first deployment needs to build up a cache. On consecutive deployments, this time is reduced.
