...
* We now support calling of a Stored Procedure with no parameters for the External Database Connector: a user can browse through all the Procedures in the **Browse Database** tab. The **Refresh/Reconnect** button is introduced to provide a quick access to the updated data to the user.
- External Database Connector is now available for Studio Pro on Mac (Beta).
