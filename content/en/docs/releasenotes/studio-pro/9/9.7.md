...
* We added check boxes to show/hide unused documents in the [Module Security](/refguide9/module-security/) dialog box. (Ticket 103824)
* We added a check box named **Refresh in client** to the [Send external object](/refguide9/send-external-object/) activity dialog box to refresh the data on the UI after the change.
...
* We fixed a bug where the [Enable for Studio](/refguide9/collaborative-development/#managing-studio) button in the Branch Line Manager was always enabled, even when the app did not support collaboration in Mendix Studio. (Ticket 132361)
* We fixed an issue that occurred when adding updatable [external entities](/refguide9/external-entities/) to your domain model where the updatable associations immedately threw a consistency error.
