...
This how-to provides guidance on using the Model Access API. The Model Access API allow access to the Mendix model.
...
You can retrieve all the units managed by the `domainModels` component using the following code:
...
The snippet below loads the page named `Home_Web` in the module named `MyFirstModule`:
