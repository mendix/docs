...
* We fixed an issue where a variable containing NULL or empty would cause an error when used in string comparison functions of XPath expressions. (Ticket 78645)
* We fixed an issue where the row order could be mixed up when multiple sort directions were added using XPathBasicQuery.addSort.
