...
- Module packages now include additional content identifiers in the package manifest: a version GUID (a deterministic identifier derived from the package checksum), the module type (`source`, `add-on`, or `solution`), and the content types contained in the package (for example, `extension` or `ai-skill`). If you try to import a package created with a newer version of Studio Pro, an error is shown and the import is cancelled. For more information, see [Package Manifest](/refguide/module-settings/#package-manifest).
