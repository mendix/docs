...
Open an environment's `MendixApp` CR [for editing](private-cloud-operator#edit-cr) and set the `mode` attribute in `runtimeMetricsConfiguration` to `compatibility`:
...
If your Prometheus has a custom scrape interval (default is 1 minute), you should specify it in `duration` to ensure the correct time window is used for max and average metrics.
...
If your Prometheus setup is using a custom scrape interval, specify the interval in the `duration` attribute in ISO 8601 Duration format (e.g. 'PT1M').
