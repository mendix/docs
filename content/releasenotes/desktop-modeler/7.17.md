...
* We fixed the issue with the new `{{themeCss}}` placeholder, where the CSS files were not inserted when deploying your application. (Ticket 67200) 
...
<<<<<<< HEAD
We have decided to retract version 7.17 due to an issue with the Session manager in clustered environments and an issue that breaks the styling of your app once deployed to the Cloud. This will be fixed in 7.17.2.
=======
We have decided to retract version 7.17.0 due to an issue with the session manager in clustered environments and an issue that breaks the styling of your app once deployed to the cloud. This will be fixed in 7.17.2.
>>>>>>> rn-update

If you upgraded to 7.17.0 prior to the version being retracted and need the installer in the meantime, please request it by filing a ticket with [Mendix Support](https://support.mendix.com).
