...
* We have optimized the History dialogue so it loads significantly faster for full and partial clones. In addition, we have increased the batch size in the History Panel from 25 to 250. When viewing the history for a single document, the batch size is still 25.
