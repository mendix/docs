...
When an object is changed but there are no actual changes, the system attributes (`changedDate` and `changedBy`) were still updated. This did not make sense when there were no changes, so now the system attributes are only updated when at least one attribute or association is changed.
