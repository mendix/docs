...
{{% alert color="info" %}}
To avoid downloading too many images, there is a default limit on the number of images that will be downloaded. Starting in version 2.10.2 of `mxpc-cli`, you can check the current limit by running the following command: `./mxpc-cli registry-migration --help`
If you would like to change this limit, use a `--max-images` commandline argument, for example, `mxpc-cli registry-migration --max-images=50`.
{{% /alert %}}
