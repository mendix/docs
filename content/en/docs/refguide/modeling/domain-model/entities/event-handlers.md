...
| Commit | The microflow is executed when an object of this entity is committed. This happens when the user clicks **Save** on a page or when an object is committed in a microflow. **Committing an object only succeeds when the current user has write access for all changes.** |
