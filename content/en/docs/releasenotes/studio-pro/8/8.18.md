...
* We fixed a missing execution id in trace log lines of microflows executed by scheduled events. (Ticket 153273)
* We fixed an issue where temporary files used for image rotation were not being closed. (Ticket 154992)
* We fixed an issue with logging out in a multicluster environment. (Ticket 151734)
* We improved Logic Bot behavior when there is a network issue, reducing the number of network calls attempted.
* We introduced a workaround for decimal-pad keyboard disabled comma issue in Samsung devices, allowing the user to type a dot which will be replaced by a comma if the locale uses comma as the decimal separator. (Ticket 116581)
* We updated our image metadata extractor library. (Ticket 163987)
* We fixed an issue with our JSON reader that could cause an error when converting a project to the latest version. (Tickets 164992, 166288)

### Deprecations

* Starting with version 8.18.22, we will drop support for following databases due to them no longer being supported by vendors:
  * PostgreSQL 10
  * SQL Server 2017
