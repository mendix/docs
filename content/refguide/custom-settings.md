...
| **com.mendix.core.ScheduledEventsCleanupAge** | This setting specifies (in milliseconds) how old rows in the System.ScheduledEventInformation table can be before they are removed from the database. Only rows with the "Completed" status are cleaned up. When this setting is not specified, no cleanup is performed. | |
...
| **com.mendix.core.ProcessedTasksCleanupAge** | This setting specifies (in milliseconds) how old rows in the System.ProcessedQueueTask can be before they are removed from the database. Only rows with the "Completed" status are cleaned up. When this setting is not specified, no cleanup is performed. | |
