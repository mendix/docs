...
Mendix 애플리케이션은 기존 시스템과 자주 통신해야 합니다. 이전 플랫폼이나 레거시 시스템을 대체하기 위한 앱을 만들든, 기존 데이터베이스를 통합하려든, 기존 시스템과의 통합 기능은 필수적입니다. Mendix 앱에서는 이러한 통합을 위해 Published REST 서비스를 사용할 수 있습니다. 

이 사용 방법 문서에서는 REST 서비스에서 Wikipedia 페이지를 검색하는 앱을 만드는 예제를 통해 이를 수행하는 방법을 보여줍니다. 결과 앱은 [다운로드 가능합니다](/attachments/howto8/integration/consume-a-rest-service/WikipediaApi.mpk).
...
## CreateInput Microflow 작성하기{#createinput}
