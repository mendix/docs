...
We introduced workflows as a Beta feature in Mendix 9 and have received great feedback from our community. You were eager to test it out and provide us with your point of view on how to further improve the functionality, and we have done so with the Mendix 9.6 release.
...
* All related associations are renamed as well.
* **WorkflowAdministrator** user role is removed.
* **WorkflowContext**, **SystemTask**, **WorkflowTaskInstance**, **WorkflowVersion** entities are removed.
* It is now allowed to create specializations of `Workflow` and `WorkflowUserTask` entities.
...
* Security for **Workflow** and **WorkflowUserTask** entities are now configured on the entity level.
