...
* An action button with the synchronize action
...
{{% alert type="info" %}}
This behavior will be part of general availability release.
{{% /alert %}}
...
If synchronization is triggered using a synchronize action in a nanoflow and an error occurs, it is possible to handle the error yourself using nanoflow error handling (right click on the activity and select **Set error handling**) and displaying an error message by using a nanoflow error handler.
...
System members such as `createdDate`, `changedDate`, `owner`, and `changedBy` are not supported.
