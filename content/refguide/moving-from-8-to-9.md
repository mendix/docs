...
* If an association is [navigable from both sides](/refguide/association-properties#navigability), both entities can define access rules declaring the readability of the association. For such associations, QE9 will always use the entity on the left side in the current XPath to determine accessibility.
Example: in the query `//Customer[Customer_Address/Address/City = 'Rotterdam']` the access rules defined in `Customer` will be used for the association, while in `//Address[Customer_Address/Customer/Lastname = 'Doe']` the rules in `Address` will be used for that same association. In QE7 the behavior was not well defined.

* QE9 has been written to strictly follow the least privilege principle when retrieving data, this might cause less data to be visible to users.

* While not allowed by Studio Pro, it was possible in a Java action to use a non-boolean attribute as a constraint, like `//Address[City]`. QE7 accepts such queries, but depending on the database it may give unexpected results. QE9 will reject such queries. 

* While not supported or documented, it is possible from a Java action to use a query like `//Customer/Customer_Address/Address`. If an instance of `Address` is reachable from multiple `Customer` instances, QE7 would return the instance of `Address` multiple times. QE9 will return each matching instance of `Address` only once.
