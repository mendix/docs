...
- We fixed an issue where a data validation error triggered when an object was rolled back or deleted. We now ignore data validation errors for objects that are rolled back or deleted, as well as data validation that is manually added in a microflow. (Ticket 192090)
