...
Extended Support is available as a product for the oldest major Mendix version where support is scheduled to end upon release of a new major version. With this product, support for the oldest version is available for a maximum duration of one year from the GA release date of the newest major version. For example, support for Mx7 is scheduled to end upon the release of Mx10 GA. However, if you have purchased Extended Support, Mx7 remains supported for one year from the release date of Mx10 GA.

With Extended Support, Mendix provides commercially reasonable efforts to resolve issues and answer customer questions. However, the knowledge, skills, and development/testing environment required to resolve issues are NOT guaranteed. Our efforts will be limited to critical bugs or security vulnerabilities, for which we work with customers towards an appropriate resolution and aim to provide a solution or workaround in a timely manner, with no resolution time commitment. Under an Extended Support agreement, no new features or functionalities will be added or retrofitted, and no third-party add-ons will be supported. Mendix strongly recommends that customers migrate or upgrade to fully supported releases as soon as possible to maintain the highest level of support for their applications.
