...
|   Nanoflow   |   The nanoflow parameter type allows you to pass a nanoflow that you can call from your JavaScript action. The value of the parameter is an async function, where calling will trigger the configured nanoflow. You can specify parameters as a JavaScript object, and capture the return value of the nanoflow once execution finishes. <br> For example, you can call a nanoflow which has a string `Name` parameter and returns a `User` object with given name:<br> `const user = await nanoflowParameter({ Name: "John Doe" });`|
