...
* We added a new way to convert between nanoflows and microflows by right-clicking the microflow or nanoflow in App Explorer.
...
* The results of [Find Usages](/refguide/find-and-find-advanced/#find-usages) now also include enumeration values used for conditional visibility. (Ticket 180727)
* We fixed an issue where an application with excluded jars would fail to deploy in Studio Pro. (Ticket 181931)
* We fixed the keyboard shortcuts behavior for <key>Enter</key> and <key>Space</key> in the tree controls.
...
* Oops dialogue appears when dragging external entities which have an enum attribute into domain model. This happens only when it is consumed directly from datahub. [**VERIFY**]
