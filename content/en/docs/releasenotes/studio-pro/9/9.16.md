...
* We fixed an issue regarding start-up synchronization in offline-first PWA and native apps, where we made start-up synchronization more resilient against object changes that are no longer compatible with the new domain model changes. This fix prevents the app start-up from failing when such an incompatibility is detected. To prevent data loss, the incompatible objects are synchronized in the Runtime as *System.SynchronizationError* objects (with the corresponding files if any) and app admins can process and restore this data when necessary.
