...
To programmatically create a registration request body that you can add to the one generated by the Transform API, then you must generate an array as follows:
1. Take the values in the **metadata.json**  for `Constants` that match those given in the **dependencies.json** file for `constant`.
2. For each service, extract the values for `Name` in **metadata.json** and write that to the object `Name`  in the registration body.
3. For each `Name` take the value for  `DefaultValue` in **metadata.json** and write that to the object `Value` in the registration body.
4. Add these to the registration body as specified for the `EndpointLocationConstants` array.
5. For the example given in the screenshot of the **metadata.json** file above the resulting `"constant":"MyFirstModule.EmployeeManagement_location"` is defined with the following value of the location:
