...
The logging is cleaned up automatically. The pin icon can be used to prevent log and trap records from being deleted by the auto-cleanup mechanism. You can also delete logs or traps manually by clicking **Delete** ({{% icon name="trash-can" %}}). For more information, see the [Cleanups](/appstore/partner-solutions/apd/rg-three-apps/#cleanups) section of *Apps*.
