...
* We have significantly improved the performance of both the regular [Synchronize](/refguide8/synchronize) activity and the [Synchronize to device](https://docs.mendix.com/refguide8/synchronize-to-device) activity by around 4-5 times in most test cases. Note that this can vary depending on your network, device, size, and amount of objects. Please note that we stil recommend limiting the size and amount of files, and when working with many objects (for example, 1000+), using the **Synchronize to device** option. For questions or advice, reach out to your CSM or partner manager to discuss.
* In [published OData services](/refguide8/published-odata-services), you can no longer leave the **Version** field empty. For existing app projects that upgrade, we set the version to 1.0.0 when it is empty.
