...
* We fixed a broken URL for the how-to that describes syncing the Web Modeler with the Desktop Modeler. This link is located in the pop-up window that appears after selecting **Edit in Desktop Modeler** via the information icon in the top-right of the screen.
* We fixed an issue in the microflow editor where the Web Modeler crashed while opening the **Condition** dialog box for an exclusive split activity that is based on an enumeration.
