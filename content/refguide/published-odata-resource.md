...
A published OData resource is always readable. Use the checkbox for **Updatable (write)** to indicate that clients can update the values of attributes or associations. Only users that have write entity access to attributes can update them. [Writable OData](writable-odata) For details. 
