...
* We fixed an issue where setting multiple entities access rules at the same time resulted in an error was that no access rights were defined. (Ticket 136705)

* We fixed an issue where a user received an error when trying to update access rules if a module was project level.

* We fixed an issue where the Branch Line Manager closed by clicking the empty area and the app selector froze.

* We fixed an issue where user credentials containing spaces caused unexpected failures when committing changes for Private Server Git apps. (Ticket 136237)
