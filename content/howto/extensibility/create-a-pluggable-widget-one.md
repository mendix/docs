...
1. Open Mendix Studio Pro and create a new test project by selecting **File > New App** from the top menu bar and then **Blank App**.
...
7.  Add a test widget to the project home page:<br />
	a. To find your widget for the first time you need to refresh from the files system. Use <kbd>F4</kbd> or select **App** > **Synchronize Project Directory** from the Mendix Studio Pro menu.<br />
	...
	{{% alert type="info" %}}The widgets in Studio and Studio Pro are not automatically updated. First, run the `npm run dev` command again. To refresh your widgets, press F4 or select Project > Synchronize App Directory from the Mendix Studio Pro menu to reload the widgets from the file system. Finally, right-click the widget and select Update all widgets to update the newly-changed properties in the widget.{{% /alert %}}
