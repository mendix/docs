...
#KI: "There is an issue with deploying your app" = PAG-1254
#KI: "Changes in" = RUN-2221 (potential fix notes in TW-868)
#KI: "Retrieving objects" = 
#KI: "The Core.execute" = RUN-2212
...
* There is an issue with deployment when you use the new layout grid for web or native pages. This occurs only when using the combination of Mendix Studio Pro 8.5.0 and Atlas UI Resources 2.4.0 (to check your Atlas UI version, consult the **Atlas UI Resources** module in your app in Studio Pro). This will be fixed in Mendix Studio Pro 8.6.0.
	* Workaround: If you do not need to use the new layout grid, you can downgrade the [Atlas UI Resources](https://appstore.home.mendix.com/link/app/104730/) module in your app to version 2.3.1. However, doing this means you will not be able to use the new layout grid. Please note this cannot be fixed in Studio – you have to use Studio Pro.
* Changes in a [change object](/refguide/change-object) activity are being executed all at once and instead of being executed sequentially. In an example scenario where the input object is "Entity", "Attribute1" has the value `old value` and "Attribute2" has the value `$Entity/Attribute1`. After the change activity, the value of "Attribute1" is updated to `new value`, but the value of "Attribute2" is updated to `old value` (when it should also be updated to `new value`). This will be fixed in Mendix Studio Pro 8.6.0.
